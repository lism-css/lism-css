...
- **CSS Layer Structure** — `@layer` (lism-base → lism-primitives → lism-components → lism-custom → lism-utility) for clear specificity management. `lism-primitives` contains `trait` / `layout` / `atomic` sub-layers. `lism-components` is the layer for BEM-structured `c--` components. `lism-custom` is the layer for user-defined classes with custom prefixes. Minimizes specificity conflicts.
