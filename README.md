...
- **CSS Layer Structure** — `@layer` (lism-base → lism-trait → lism-primitive → lism-block → lism-custom → lism-utility) for clear specificity management. `lism-trait` is the layer for `is--` / `has--` trait classes. `lism-primitive` contains `layout` / `atomic` sub-layers. `lism-block` is the layer for shared base components (`b--`) whose base styles are managed in CSS. `lism-custom` is the layer for user-defined classes and overrides. Minimizes specificity conflicts.
