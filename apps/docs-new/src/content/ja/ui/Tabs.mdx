...
import { Preview, PreviewArea, PreviewTitle, PreviewCode, ImportPackage, EmbedCode, PropBadge, Callout, IconBadge } from "@/components/mdx";
...
`d--tabs_tab`では`--_isSelected`, `--_notSelected`が使えるようになっています。  
<IconBadge type='help'/> （[`set-hov`](/css/hov#set-hov)の`--_isHov`,`--_notHov`の解説を参考にしてください。）
