...
- **`'ai'`**: AI アシスタント風のチャットデモ。パネルの再生ボタンを押すと「AI に依頼 → コードが書き換わる → ヒーローが変わる」という体験を全ステップ連続で自動デモする。Lism 自体に AI アシスト機能があるわけではなく実態と差があるため現行では無効化しているが、パネルのマークアップ・`player.ts`・シナリオ文言は温存しており `DEMO_MODE` を戻すだけで復活する
...
内容はヒーローの見出し（h1）・リード文（p）・Get Started リンク（`l--flex` の a）・`data-modal-open="search-modal"` 付きの検索ボタン（`l--flex set--plain` の button。⌘K チップ入り）で構成する。検索ボタンの `set--plain` は JSX タブで `set="plain"` prop として表示され、UA スタイルのリセット機構のデモを兼ねる。h1 とリード p は 3 段階のレスポンシブ表記（`-fz:3xl -fz_sm -fz_md` + `--fz_sm: var(--fz--4xl); --fz_md: var(--fz--5xl)` 等）を持ち、SP 縮小の実装とレスポンシブの書き方のデモを兼ねる（JSX タブでは `fz={['3xl', '4xl', '5xl']}` と表示される）。リード p の本文色はカラートークンの `-c:text-2` で持ち、エディターから編集できる色トークンのデモを兼ねる。⌘K チップも同機構で SP では非表示（`-d:none -d_md` + `--d_md: inline-block`。キーボードショートカットはデスクトップ前提のため）。なお、デザインモックアップ（SVG）記載のクラスの一部（`-g:8` / `-bg:base-2` / `-fw:medium` 等）は Lism に実在しないため、ビルド済み `main.css` に存在するクラスだけで再構成している。
...
- 初期表示は SSR 済みなので、shiki 本体（非同期チャンク・gzip 約110KB）は `requestIdleCallback`（未対応ブラウザは 1.5 秒後の `setTimeout`）で遅延ロード
  - 初回操作（`pointerdown` / `focusin` / `keydown`）まで遅らせて未使用の訪問者に配らない案は、**読み込み完了前に打鍵されると全文が一度プレーンテキストへ落ちる**ため見送った。ハイライト再描画は入力・タブ切替・書き換えでしか走らず、SSR 済みの表示はそれまで残るので、触るだけなら副作用はない。しかし打鍵で再描画が走ると、その時点で未ロードなら 1 文字ぶんではなくコード全体の色が一度消える
- ロード完了後は **`highlightSync()`（同期 API）で入力のたびに即時再描画**。非同期 + debounce だと textarea とハイライトが一瞬ズレるため。入力上限が 2,000 字（後述）なので同期ハイライトの最悪コストは十分小さく、**ユーザー入力の経路には rAF スロットルを入れない**（textarea は `color: transparent` で見えている文字はハイライト層そのもの。1 フレーム遅らせると caret だけが先に進んで見えるため）
- 例外は再生アニメのフレーム（`setViewText`）で、12〜18ms ごとに来るため rAF でまとめる。プログラム的な書き換えで caret が動かないので、遅らせても見え方は変わらない
...
大量入力によるクラッシュ / フリーズ防止（毎キー入力でハイライト・変換・ヒーロー描画が走るため）。上限は `MAX_CODE_LENGTH = 2,000` 字（デモ用途の上限。初期コードは最大約 970 字（英語版。レスポンシブ表記の style 属性を含む）で、約 2 倍の編集余地を残している）。
...
- **JSX タブ**: `processInput` で `jsxToHtml()` の結果から即時に invalid 表示を付け、スナックバー用の判定はデバウンス発火時に `jsxToHtml()` で取り直す（HTML タブと同じく、評価時点のテキストだけを見る）。タブの warning 色インジケーターは即時、スナックバーはデバウンス後
...
- **変更チェック**: 表示中のコードが初期コードと 1 文字でも違えば、リセット提案（後述のボタンのみのスナックバー）を表示する。比較はアクティブタブの表示テキストと初期コードのタブ別表記（JSX タブは `htmlToJsx(initialHtml)` を初回参照時にキャッシュ）で行う。空（trim して空文字）も「変更あり」に含み、表示は変えない。手で編集を戻して初期コードと一致した場合は提案だけを閉じる（`hideAction()`。構文警告の行は残す）
- AI デモが完走した時点でも `editor.syncRestorePrompt()` を呼び、デモが書き換えたコードを戻せるようにする（最後の `snapTo` が通知を消しているため出し直す）
- 提案の出し入れは打鍵ごとではなくデバウンス評価に集約する（1 文字ごとに出し直すとちらつくため）
- タブ切替・`setCode()` 時は評価タイマーを破棄して非表示に（古いメッセージの誤表示防止）。ただしタブ切替では変更の有無を評価し直し、変更されたままならリセット提案を出し直して引き継ぐ
...
**提案型（`showAction`）** — コードが初期状態から変わっている間のリセット提案

- 初期コードから変わったままデバウンス評価を迎える（または AI デモが完走する）と `Restore initial code` ボタンを表示する。**文言もアイコンも持たず**、カードの枠・地色・影も外してボタン 1 つが浮いているように見せる（コードの上でも読めるよう、カードの地色はボタンへ移す）
- ボタンが同じなら**作り直さずそのまま出し続ける**。編集のたびに評価が走るため、積み直すと表示が点滅してしまう
- **自動クローズしない**。Restore ボタンか、手で編集を戻して初期コードと一致したとき（`hideAction()`）に閉じる
...
- 現在は仮の 5 ステップ: ①見出しを `-fw:700` → `-fw:800`（太く）②見出しを `-fw:800` → `-fw:400 -fs:italic`（細く + 斜体。可変フォントのウェイト幅を見せる）③ボタンを `-bdrs:99` + `-p:15`（ピル型）④検索ボタンも `-p:15` + `-bdrs:99` に揃え、⌘K も `-bdrs:10` → `-bdrs:99` ⑤ボタンまわりの gap を `-g:15` → `-g:20`（間隔を広げる）
...
- **ヒーロー側はサイトテーマに追従**: トークン化されていない任意色（検索ボックスの文字色 / 輪郭）のみ `:root[data-theme='dark'] .c--kvEditorHero` で上書きする。見出し色・ボタン背景は `--gray-hi-c` 変数、リード文の色は `-c:text-2` トークンのダーク値で自動対応
...
- SP の見出しサイズは言語共通の初期コードのレスポンシブ表記（`-fz:3xl -fz_sm -fz_md` + `--fz_sm` / `--fz_md`。前述）に任せる。旧静的ヒーローの SP サイズ（`2.25rem`）を CSS で固定する案は、非レイヤールールが必要になりユーザーがエディターで打った `-fz:*` を上書きしてしまうため不採用
...
`convert`・`validate`・`diff`・`scenario` には自動テストがある。DOM を扱う UI は手動で確認する。
