...
`as` には出力するHTMLタグや外部コンポーネントを指定できます。デフォルトは`div`です。
...
この時、[Prop Class](/docs/prop-class/) に対応する値が指定されれば、下記のようにLismで用意しているユーティリティクラスが出力されるようになっています。
...
`Lism` で使用できる CSS Propsは、[レスポンシブに対応した形式](/docs/responsive)で出力できるようになっています。
...
どのプロパティが対応してるかは[Prop Class](/docs/prop-class) の **BP**（
