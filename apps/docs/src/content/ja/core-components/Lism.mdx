...
`<Lism>` が生成する `class` 属性は、大きく **「ユーザー指定」「モジュールクラス群」「ユーティリティ群」** の3ブロックに分かれます。ユーティリティ群は粒度の低い（＝塊としての）順 `set--` → `u--` → Property Class で並びます。
...
| 3 | レイアウトモジュール | `layout` prop | `l--box`, `l--flow` |
| 4 | State Modules | State Props（`isWrapper` 等） | `is--wrapper`, `is--layer` |
...
2〜4 はいずれも [Module Class](/docs/css-methodology/)（CSS レイヤー `lism-modules`）に属し、5〜7 はユーティリティ系のクラスです。

:::note
`layout="flow"` の `-flow:{size}` はレイアウトモジュール固有の設定クラスとして扱われるため、`l--flow` の直後に出力されます。
:::
