...
|**State Props**|[Trait Primitives](/docs/css-methodology//#trait-primitives) に分類されるクラスを出力するためのプロパティ|
...
| 4 | トレイトプリミティブ | State Props（`isWrapper` 等） | `is--wrapper`, `is--layer` |
...
2〜4 はいずれも [Component Class / Primitive Class](/docs/css-methodology/)（CSS レイヤー `lism-components` / `lism-primitives`）に属し、5〜7 はユーティリティ系のクラスです。
