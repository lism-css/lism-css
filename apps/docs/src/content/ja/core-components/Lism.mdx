...
|**CSS Props**|[Prop Class](/docs/prop-class/) に分類されるクラスを出力するためのプロパティ| */}
...
`set` は配列やカンマ区切り・空白区切りで複数指定できます。  
`unset` を併用すると、コンポーネント内部で適用済みの set クラスを個別に除外できます。
...
<Reference>
[指定可能な Props 一覧はこちら](/docs/prop-class/)
</Reference>
...
:::note[補足]
- **BP対応プロパティ**: ブレイクポイントで値を切り替えられるプロパティです。[Prop Class](/docs/prop-class) の **BP** 欄を参照してください。
...
対応する Prop Class がない場合でも、`:` から始めることでその後ろの文字列がユーティリティクラス名として出力されます。
...
どのプロパティが対応しているかは[Prop Class](/docs/prop-class) の **BP**（<small>ブレイクポイントの略</small>）の欄をチェックしてください。
:::
