...
ユーザーが独自のコンポーネント（`.header`, `.footer`, `c--pricing` 等）を定義する場合は、`@layer lism-modules` に配置してください。
...
クラスのプレフィックスについては以下を推奨します。

| プレフィックス | 管轄 | ユーザー拡張 |
|---|---|---|
| `l--`, `a--` | Lism コア（レイアウト・装飾） | しない（コア提供のみ） |
| `c--` | コンポーネント | ユーザーも追加OK。lism-css コアには `c--` は存在せず、最初からユーザー向けの名前空間。`lism-ui` が公式プリセットとして `c--` コンポーネントを提供 |
| `z--` 等 | ゾーニング（`z--header`, `z--main`, `z--sidebar` 等） | ユーザー定義OK |
| プレフィックスなし | ユーザー独自 | 自由（`.header`, `.site-nav` 等） |
