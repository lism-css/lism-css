...
<table className="l--table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>上記のレイヤーより優先しつつ Prop Classよりも詳細度を下げたいスタイルを定義できるカスタマイズ用レイヤー。</td>
    </tr>
    <tr>
...
Lismパッケージに内蔵されているクラス群ではないものをどうするかについては、明確な定めはありません。

プレフィックスをLismに寄せて統一感を出すか、もしくは独自クラスであることを明確にす　るためにプレフィックスをつけない、というのがおすすめです。  
（コンポーネントクラスは `@lism-modules`で`c--`として定義するのを推奨しています。）


|分類の例|形式|例|
|---|---|---|
|ゾーニング|`.z--{zoneName}` または `{zoneName}` |`.z--header`,<br />`.header`|
|ページの分類|`.p--{type}-{id\|slug}` または `{slug}Page`など |`.p--front`,<br />`.p--page--{slug}`, <br />`.frontPage`,<br />`.page--{slug}`|
