...
<table className="l--table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>単一のCSSプロパティティに紐づいた **Prop Class** (`.-{prop}:{val}`) 。<br />このクラス群のみ、レイヤーを外して詳細度を高くしつつ、ある程度外部CSSとも共存できるようにしています。</td>
    </tr>
  </tbody>
</table>
...
<Reference>
詳しくは[Prop Class 一覧ページ](/docs/prop-class)を参照してください。
</Reference>
...
CSSプロパティとその値のセットの組み合わせでのユーティリティクラスは、Prop Class の`-{prop}:{value}`形式を利用します。
