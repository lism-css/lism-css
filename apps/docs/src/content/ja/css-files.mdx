...
:::info
`full.css` を読み込むだけでは、コンポーネント側はデフォルト設定のまま動作します（例: `t="20"` はインラインスタイルとして出力されます）。コンポーネントの出力も `full.css` に合わせるには、`lism.config.js` の [`isFullMode`](/docs/customize/config/#isfullmode) を有効にしてください。
:::
