...
例: `-d:none -d_sm:block` → デフォルトでは非表示、`sm` 以上で表示。

:::info
レスポンシブ対応の Property Class は、基本的には標準では`sm`,`md`までサポートされています。  
ただし`d`（display）, `g`（gap）, `cols`, グリッド系（`gta` / `gtc` / `gtr` / `ga` / `gc` / `gr`）など、大枠のレイアウト構造に関わる一部のプロパティは`lg`までサポートされています。  
それ以外のプロパティで`lg`サイズ以降を使う場合は、別途[SCSSの設定ファイルのカスタマイズ](/docs/customize/)が必要です。
:::
