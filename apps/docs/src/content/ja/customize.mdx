...
- `$common_support_bp`: 主要なProperty Classで共通してサポートするブレイクポイント
- `$is_container_query`: コンテナクエリを使用するかどうか
- `$default_important`: Property Classにデフォルトで`!important`を付与するかどうか（`0`|`1`）
- `$props`: Property Classの出力設定
...
<HelpText icon="info">`@layer` のオン・オフは変数管理ではありません。読み込む`main.scss`を切り替えてください。</HelpText>
<HelpText icon="info">また、Astro の場合は`../path-to/node_modules/`部分は不要です。</HelpText>
