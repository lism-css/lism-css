...
`@lism-css/plugin` が提供する統合プラグインを使います。これ1つで、**config alias（JS ランタイムが `lism.config.js` を読む）・動的CSSビルド（dev/build で CSS に自動反映）・型自動生成（`lism-env.d.ts`）** がまとめて有効になります。
...
Vite / Astro を使わない構成（純 SCSS 構成・他バンドラ等）で `lism.config.js` を CSS に反映させるには、`@lism-css/plugin` が提供する `lism-css build` を実行します。これは後方互換専用ではなく、そのような構成向けの**現役の正規パス**です。
...
`tokens` に値を書いておけば、`.-lts\:2xl { letter-spacing: var(--lts--2xl) }` のような **トークン参照を含むユーティリティクラス** に加えて、参照先の CSS 変数（`:root { --lts--2xl: .5em }` のような **値そのもの**）も同時に生成されます。なお値に `'-'` を指定したキーはカタログ登録のみで `:root` 宣言を出力しません（実値は手書きSCSS側で定義します。主に `color` / `palette` で使います）。
