...
| `$common_support_bp` | 主要な Property Class が共通してサポートするブレイクポイント（デフォルト: `'lg'`） |
...
@use '../path-to/node_modules/lism-css/scss/setting' with (
  ...
  $common_support_bp: 'md', // 'sm' | 'md' | 'lg'（デフォルトは 'lg'。小さくするとCSSサイズを削減できます）
...
@use '../path-to/node_modules/lism-css/scss/setting' with (
  $props: (
    ...
    'p': (
      bp: 'md',
      // 'p'のブレイクポイント対応クラスは md サイズまでに制限する
      utilities: (
        'box': '2em',
      ),
      //.-p:box{--p:2em} を追加する
...
| `bp` の値 | 意味 |
|-----------|------|
| `1` | 有効な全ブレイクポイント（最小〜 `$common_support_bp`）で出力する |
| `0` | ブレイクポイント対応クラスを出力しない |
| `'md'` など | 出力範囲の上限を個別に指定する（最小〜指定BP） |
...
デフォルトの `$breakpoints` には `sm` / `md` / `lg` に加えて `xs` / `xl` のキーも並んでいますが、サイズが `0`（無効）のため出力されません。`xs` を使いたい場合は、`$breakpoints` で `xs` のサイズを定義してください。
...
`$breakpoints` での `xs` のサイズ定義が**グローバルスイッチ**として働き、ブレイクポイント対応（範囲形式 `bp: 1`）の全 Property Class が `xs` のレスポンシブクラス（`-p_xs` など）も出力するようになります。prop ごとに個別指定する必要はありません。

これにより、コンポーネント側でもオブジェクト記法で `xs` を指定できるようになります（型・補完で `xs` を提示させるには、[Responsive ページ](/docs/responsive/#型補完で提示されるブレイクポイント)の `LismBreakpointRegistry` の拡張も必要です）。
...
:::warning
`xs` を有効化すると `bp: 1` の全 Property Class に `xs` ブロックが追加されるぶん、CSS サイズが増えます。**CSS purge との併用が前提**の設計です。purge を使わない場合は、`bp` のリスト形式（`bp: ('sm', 'md')` など）で prop ごとに出力ブレイクポイントを絞ることを検討してください。
:::
