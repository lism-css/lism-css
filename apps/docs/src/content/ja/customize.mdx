...
デフォルトの `$breakpoints` には `sm` / `md` / `lg` に加えて `xs` / `xl` のキーも並んでいますが、サイズが `0`（無効）のため出力されません。`xs` や `xl` を使いたい場合は、`$breakpoints` でサイズを定義してください。
...
`$breakpoints` でサイズを定義したブレイクポイントが有効BPになり、ブレイクポイント対応（`bp: 1`）の全 Property Class がそのBPのレスポンシブクラス（`-p_xs` / `-p_xl` など）も出力するようになります。prop ごとに個別指定する必要はありません。

これにより、コンポーネント側でもオブジェクト記法で `xs` / `xl` を指定できるようになります（型・補完で提示させるには、[Responsive ページ](/docs/responsive/#型補完で提示されるブレイクポイント)の `BreakpointRegistry` の拡張も必要です）。
