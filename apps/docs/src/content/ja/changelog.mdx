...
- 従来 px と rem が混在していた `--sz--*` トークンを全て px に統一
- 全値を 80px の倍数（×5, ×8, ×11, ×15, ×20）で再構成
- xs: 32rem(512px) → 400px
- s: 42rem(672px) → 640px
- m: 56rem(896px) → 880px
- l: 1280px → 1200px
- xl: 1600px (維持)
...
- `s25` (≒20px) / `s35` (≒32px) を追加し、`s10` 〜 `s40` を 4px 刻みの実用値で補完
- `--s-unit` を `--fz--base` 経由に再構成（`calc(var(--fz--base) * 0.5)`）
- `set--var:s` ユーティリティクラスを新設し、`--s-unit` を `0.5em` に切り替えて spacing を要素フォントサイズに追従させられるように
- `sz` トークンから `bleed` を削除し、`max-sz` 専用の preset に移動

### その他

- `input[type='range']` にフォームフィールドのデフォルトスタイルが当たらないよう除外
- カラーパレットトークン (oklch 値) を微調整
...
- `--REM` を廃止し、`--fz--base` を `1rem` に統一
- `--hl-unit` / `--s-unit` を `--fz--base` 経由に統一
...
| 旧 | 新 | 用途 |
|---|---|---|
| — | `u--divide` | 子要素**間**にのみ区切り線を表示（外周なし） |
...
- trait の SCSS を `is/` と `has/` サブディレクトリに整理（内部リファクタ）
