...
### `-max-sz:container` を `-max-sz:bleed` に差し替え (#359)

これまでの `-max-sz:container` は、`is--container` ごとに更新される `--sz--container` を参照しており、**直近の container 幅**まで広がる挙動でした。一方で、実際によく欲しいのは「本文幅やネストされた container を突き抜け、**ページ幅 / 最外側 container 幅**まで広がる full-bleed 表現」です。

そこで `-max-sz:container` を廃止し、用途に合わせた `-max-sz:bleed` に差し替えました（後方互換エイリアスなし）。
...
`-max-sz:bleed` は**最外側の `is--container` 幅**まで広がります。`@property --sz--bleed` の `initial-value` は `100svi` のため、`is--container` が祖先に存在しない場合はビューポート幅まで広がる fallback として動作します。
...
「直近 container 幅まで広がる」挙動が必要になった場合は、別途復活を検討します。
...
合わせて、上記との干渉で `has--gutter` 内の `-max-sz:full` の hang が効かなくなる問題、および `-max-sz:bleed`（旧 `-max-sz:container`）の中央配置が崩れる問題を解消するため、それぞれに `inline-size: auto` を追加しました。
