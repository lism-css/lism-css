...
## lism-css v.0.17.0 (未リリース)

**破壊的変更**を含むリリースです。

### `-max-sz:full` / `-max-sz:container` を `is--fullwide` / `is--bleed` にリネーム (#357)

実態が単一CSSプロパティの設定にとどまらず `margin-inline` の操作も伴う「役割宣言（trait）」であるため、Property Class 形式から Trait Class 形式に整理しました。
...
`is--fullwide` / `is--bleed` は他の `is--*` クラスと同様に `isFullwide` / `isBleed` Prop で利用できます。
...
旧クラス名（`-max-sz:full` / `-max-sz:container`）は廃止され、後方互換のための alias は提供されません。直接利用している箇所はすべて置き換えが必要です。

合わせて、`max-sz` Prop の値型から `'full'` / `'container'` を削除しました。
