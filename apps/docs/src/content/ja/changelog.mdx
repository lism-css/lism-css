...
### `has--gutter` の CSS 変数構造を整理 (#409)

- `:root` の基準値: `--gutter-size` → `--gutter--base` にリネーム
- 要素ローカル機能変数 `--gutter` を新設（`.has--gutter` で `var(--gutter--base)` に初期化）
- `--flow--base` / `--flow` と同じ「基準値 + 要素ローカル機能変数」の2層構造に統一
- 要素単位で `style="--gutter: 20px"` のように上書きが可能に
- `is--container` の `--sz--bleed` 計算や `-max-sz:full` の負 margin も `--gutter` を参照するように更新

### テーブルセル変数を整理

- `--td-c` / `--td-bgc` / `--td-min-sz` / `--th-c` / `--th-bgc` / `--th-min-sz` / `--th-p` を削除（未定義時の挙動と同じため不要）
- `--td-p` を `--cells-p` にリネーム（`td`, `th` 両方に適用）
- フォールバック値を `var(--s10) var(--s15)` から `0.625em 0.875em` に変更（親フォントサイズ基準に変更）
- 最終形: `td, th { padding: var(--cells-p, 0.625em 0.875em); }`

### `u--cells` を `u--enclose` にリネーム (#411)

- `u--cells` というクラス名は効果が読み取りにくいため、`u--enclose` にリネーム
- `divide`（分ける） ↔ `enclose`（囲う）の動詞対比で機能の対比を明確化
- 後方互換 alias は提供しないため、利用箇所はすべて置き換えが必要
