...
navtitle: 'l--tileGrid'
description: 'l--tileGrid は、列数と行数を指定して均等なタイル型グリッドレイアウトを構成するレイアウトプリミティブです。'
---

import { TileGrid, Box, Center } from 'lism-css/astro';


`l--tileGrid` は、`cols`（列数）と`rows`（行数）を指定して、**均等なタイル型グリッドレイアウト**を構成するためのレイアウトプリミティブです。
...
- [`<FluidCols>`](/docs/primitives/l--fluidcols/): auto-fit / auto-fill による流動的列数
