...
description: 'is--vertical は、要素に縦書きモード（writing-mode）を適用するプリミティブクラスです。'
---
import { Flow } from 'lism-css/astro';
import { DummyText } from '@lism-css/ui/astro';

`is--vertical` は、要素に縦書きモード（`writing-mode`）を適用するプリミティブクラスです。
...
<SrcCode path="src/scss/modules/trait/_vertical.scss" />
