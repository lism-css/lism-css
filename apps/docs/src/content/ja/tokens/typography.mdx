...
Lism CSSでは、`line-height` をハーフレディング（行間の上下余白）の値で管理する独自の仕組みを採用しており、`--hl` 変数にセットした値から `line-height` が算出されます。
...
`--fz--base` を `clamp()` 等を使って流体値にしているサイトで、行間もフォントサイズ基準にして流体的に変化させたい場合は、`:root` で `--hl-unit: calc(var(--fz--base) * 0.125);` のように上書きしてください。

この設計の背景は [ハーフレディング（line-height）](/docs/half-leading/) を参照してください。
...
実際の `line-height`（unitless 比率）を管理する `--lh` 用のトークンです。`--lh` をセットすると、上記の全要素ルールのハーフレディング計算（fallback）よりも優先され、比率が子要素にも継承されます。
...
各値は、基準フォントサイズ（16px）におけるデフォルトの `hl` スケールと一致するように設定されており、`-lh:m` がデフォルト本文の行送りとほぼ対応します。
...
`hl` との棲み分けは [ハーフレディング（line-height）](/docs/half-leading/) を参照してください。
