...
`set--var:bxsh` クラスを指定した要素では、シャドウを構成する `--bxsh--` 変数が再定義されます。
...
<Preview>
  ...
  <PreviewArea p="30" nogrid>
    <Stack g="10">
      ...
      <Spacer h="40" />
      <Text> set--var:bxsh で 変数を上書きした例: </Text>
...
詳しくは[スペーシングトークン](/docs/tokens/spacing/)を参照してください。
...
活用例は [`hover`の解説ページ](/docs/property-class/hov#set--varhov) を参照してください。
...
`p`, `bdrs`はブレイクポイントで値を変えても変数管理されているため、`set--var:bdrsInner`の角丸もレスポンシブ対応できます。
