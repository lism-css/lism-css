...
<Reference>
活用例は [`hover`の解説ページ](/docs/props/hov#set--hov) を参照してください。
</Reference>
...
<Reference>
活用例は [`hover`の解説ページ](/docs/props/hov#set--transition) を参照してください。
</Reference>
...
[ブレイクポイント対応](/docs/responsive/)していないプロパティを変化させたい時のハック的手段として活用できます。
...
## `unset` で Set Class を除外する

コンポーネント内部でデフォルト適用されている `set` クラスを、使用側で除外したい場合は `unset` prop を利用できます。

例えば `Accordion.Button` は内部で `set="plain"` が適用されていますが、`unset="plain"` を渡すことでリセットを解除できます。

```jsx
{/* デフォルトの set="plain" を解除 */}
