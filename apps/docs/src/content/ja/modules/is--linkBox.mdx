...
`<Lism isLinkBox>` のエイリアスとして `<LinkBox>` が用意されています。
...
2. `<LinkBox>`(`.is--linkBox`）を`<div>`にして、内部に`a.is--coverLink`（旧 `u--expandedLink`）を配置する。
