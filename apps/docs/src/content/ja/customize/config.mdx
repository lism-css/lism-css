...
追加されたユーティリティクラスに対するスタイルは読み込まれません（[追加スタイルを読み込ませる方法](#追加スタイルを読み込ませる方法)を参照）。
:::
...
:::info
[`full.css`](/docs/customize/build/#全部入りビルドfullcssを使う)には、あらかじめ `xs`（仮値 360px）が有効化された状態のクラスが含まれています。
:::
