...
`isFullMode: true` にすると、コンポーネントの props 設定にも full 用の設定が適用され、`t="20"` がインラインスタイルではなく `-t` クラス＋`--t` 変数として出力されるようになります。ブレイクポイント指定（`ta={['start', 'center']}` など）も全 Property で警告なく使えるようになります。設定は「デフォルト設定 → full 用設定 → `lism.config.js` の `props`」の順でマージされ、後のものほど優先されます。
