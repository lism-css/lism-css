...
`isFullMode: true` にすると、コンポーネントの props 設定にも full 用の設定が適用され、`t="20"` がインラインスタイルではなく `-t` クラス＋`--t` 変数として出力されるようになります。ブレイクポイント指定（`ta={['start', 'center']}` など）も、上で挙げたブレイクポイント対応クラスが出力される Property であれば警告なく使えるようになります。変数出力専用（`isVar` 系。ただし `bds` / `bdc` は除く）と border ショートハンド系は対象外のままで、これらにブレイクポイント指定を渡すと警告が出ます。設定は「デフォルト設定 → full 用設定 → `lism.config.js` の `props`」の順でマージされ、後のものほど優先されます。
