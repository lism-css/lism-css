...
本番ビルド時に未使用の Lism CSS クラスを取り除き、CSS ファイルを軽量化するパージプラグインを `lism-css` パッケージに同梱しています。Astro なら `lism-css/purge/astro` の `lismPurgeAstro`、Vite なら `lism-css/purge/vite` の `lismPurge` を import して組み込みます。
