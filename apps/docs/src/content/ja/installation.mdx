...
本番ビルド時に未使用の Lism CSS クラスを取り除き、CSS ファイルを軽量化するパージ機能を `lism-css` パッケージに同梱しています。`lism-css/vite` の統合プラグイン（`lismCss()` / `lismCssAstro()`）を使っている場合は、`purge: true` オプションを渡すだけで有効化できます。
...
:::info
単体プラグイン（`lism-css/purge/astro` の `lismPurgeAstro` / `lism-css/purge/vite` の `lismPurge`）も後方互換で引き続き利用できます。
:::
