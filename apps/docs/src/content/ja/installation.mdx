...
:::info
`main.css` のほかに、`@layer` を使わない `main_no_layer.css` や、CSS purge ツールとの併用を前提にした全部入りビルドの `full.css` もあります。詳しくは[CSSファイルの種類](/docs/css-files/)をご覧ください。
:::
...
本番ビルド時に未使用の Lism CSS クラスを取り除き、CSS ファイルを軽量化するパージ機能は `@lism-css/plugin` で提供しています。Vite / Astro の統合プラグイン（`lismCss()`）に `purge: true` を渡すだけで有効になります。セットアップ手順・オプション・safelist の指定方法は [CSS Purge](/docs/customize/purge/) を参照してください。
