...
詳しくは [タイポグラフィトークン](/docs/tokens/typography/#lh-line-height) を参照してください。
...
クラスを持たないリスト、または Prop Classしかクラスを持たない（class属性が `-` で始まる）リストに対して、ブラウザデフォルトスタイルを復活させています。（余白はスペーシングトークンで管理）
