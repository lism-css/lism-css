...
`listProps`で`.c--tabs_list`にプロパティを渡せます。
...
`c--tabs_tab`では`--_isSelected`, `--_notSelected`が使えるようになっていますので、活用していただくと便利です。
<HelpText icon="info" >（[`set--hov`](/docs/property-class/hov#set--hov)の`--_isHov`,`--_notHov`と同じようにして使用できます。）</HelpText>
