---
title: 'ShapeDivider'
description: '@lism-css/ui で提供する ShapeDivider コンポーネントについて解説します。'
---

import { Box, Wrapper, Container, Divider, Frame, Spacer, Layer, Media, Stack } from 'lism-css/astro';
import { ShapeDivider } from '@lism-css/ui/astro/ShapeDivider';
import { DummyText } from '@lism-css/ui/astro/DummyText';


svgで境界線を描画するコンポーネントです。


## Overview

<Preview>
  <PreviewTitle>Preview</PreviewTitle>
  <PreviewArea resize layout="">
    <Stack>
      <ShapeDivider isEmpty level={2} />
      <DummyText p="30" />
      <ShapeDivider viewBox="0 0 100 10">
        <path d="M100 6C89.3 3.3 82.7 9 70 9S48.4 3 38 3 24.5 6 17 6C7.4 6 0 0 0 0v10h100V6z" />
      </ShapeDivider>
    </Stack>
  </PreviewArea>
  <PreviewCode slot="tab" label="JSX">
    ```jsx
    <Stack>
      <ShapeDivider isEmpty level={2} />
      <DummyText p="30" />
      <ShapeDivider viewBox="0 0 100 10">
        <path d="M100 6C89.3 3.3 82.7 9 70 9S48.4 3 38 3 24.5 6 17 6C7.4 6 0 0 0 0v10h100V6z" />
      </ShapeDivider>
    </Stack>
    ```
  </PreviewCode>
  <PreviewCode slot="tab" label="HTML">
```html
<div class="l--stack">
  <div class="c--shapeDivider -max-sz:full" style="--level:2" aria-hidden="true"></div>
  <p class="-p:30">...Contents...</p>
  <div class="c--shapeDivider -max-sz:full" style="--level:5" aria-hidden="true">
    <div class="c--shapeDivider_inner">
      <svg class="c--shapeDivider_svg" viewBox="0 0 100 10" width="100%" height="100%" fill="currentColor" focusable="false" preserveAspectRatio="none">
        <path d="M100 6C89.3 3.3 82.7 9 70 9S48.4 3 38 3 24.5 6 17 6C7.4 6 0 0 0 0v10h100V6z"></path>
      </svg>
    </div>
  </div>
</div>
```
  </PreviewCode>
</Preview>


## Styles

ShapeDivider のベーススタイルは、以下のCSSで定義されています。

<SrcCode dir="lism-ui" path="src/components/ShapeDivider/_style.css" title="_style.css" />

:::note
ソースコード全体は [GitHub](https://github.com/lism-css/lism-css/tree/main/packages/lism-ui/src/components/ShapeDivider) で公開しています。
:::


## How to use

`@lism-css/ui` パッケージで`ShapeDivider`(`c--shapeDivider`)として提供しています。

### Import

<ImportPackage component="ShapeDivider" packageName="@lism-css/ui" css="style.css" />


### Props

| プロパティ | 説明 |
|---|---|
| `viewBox` | 出力するsvgのviewBox。|
| `level` | シェイプの高さレベルを正の数で指定できます。デフォルトは`5`です。|
| `offset` | シェイプを水平方向にずらすための変数を出力します。 |
| `stretch` | シェイプを水平方向に引き伸ばして表示するための変数を出力します。|
| `flip` | `"X"`, `"Y"`, `"XY"` でシェイプの反転方向を指定できます。|
| `isEmpty` | シェイプを表示せず、Spacerのように使用することができます。|
| `isAnimation` | アニメーションをONにします。`data-has-animation="true"`が出力されます。|


## Examples

### シェイプをセクションの上下に配置する

flipを指定して反転させることで、同じsvgパスを使って上下に利用できます。（パス自体を上下用に分けて用意してももちろんOKです）

<Preview>
  <PreviewTitle>Preview</PreviewTitle>
  <PreviewArea resize p="0" layout="">
    <Wrapper hasGutter py="40">
      <DummyText />
    </Wrapper>
    <ShapeDivider viewBox="0 0 10 10" level={4}>
      <path d="M10,10V0L0,9.5V10L10,10z"></path>
    </ShapeDivider>
    <Wrapper hasGutter bgc="text" c="base">
      <Box py="40">
        <DummyText length="l" />
      </Box>
    </Wrapper>
    <ShapeDivider viewBox="0 0 10 10" level={4} flip="XY">
      <path d="M10,10V0L0,9.5V10L10,10z"></path>
    </ShapeDivider>
    <Wrapper hasGutter py="40">
      <DummyText />
    </Wrapper>
  </PreviewArea>
  <PreviewCode slot="tab" label="JSX">
    ```jsx "\<ShapeDivider" "\/ShapeDivider\>" 'flip="XY"'
    <Wrapper hasGutter py="40">
      <DummyText />
    </Wrapper>
    <ShapeDivider viewBox="0 0 10 10" level="4">
      <path d="M10,10V0L0,9.5V10L10,10z"></path>
    </ShapeDivider>
    <Wrapper hasGutter bgc="text" c="base">
      <Box py="40">
        <DummyText length="l" />
      </Box>
    </Wrapper>
    <ShapeDivider viewBox="0 0 10 10" level="4" flip="XY">
      <path d="M10,10V0L0,9.5V10L10,10z"></path>
    </ShapeDivider>
    <Wrapper hasGutter py="40">
      <DummyText />
    </Wrapper>
    ```
  </PreviewCode>
</Preview>


### 空のShapeDividerをスペーサーとして利用する

pathを用意せず、空のisEmptyを指定してスペーサーとしても利用できます。

空の場合でも、`level`で高さをコントロールできるので、上下片方にシェイプを配置した時の逆側の余白コントロールに便利です。

<Preview>
  <PreviewTitle>Preview</PreviewTitle>
  <PreviewArea resize p="0" layout="">
    <Wrapper hasGutter bgc="base">
      <ShapeDivider isEmpty level={5} />
      <Box py="20">
        <DummyText length="l" />
      </Box>
      <ShapeDivider viewBox="0 0 10 10" level={5}>
        <path d="M10,10V0L0,9.5V10L10,10z"></path>
      </ShapeDivider>
    </Wrapper>
    <Wrapper hasGutter bgc="text" c="base">
      <Box py="40">
        <DummyText length="l" />
      </Box>
    </Wrapper>
  </PreviewArea>
  <PreviewCode slot="tab" label="JSX">
    ```jsx "isEmpty" 'level="5"'
    <Wrapper hasGutter bgc="base">
      <ShapeDivider isEmpty level="5" />
      <Box py="20">
        <DummyText length="l" />
      </Box>
      <ShapeDivider viewBox="0 0 10 10" level="5" >
        <path d="M10,10V0L0,9.5V10L10,10z"></path>
      </ShapeDivider>
    </Wrapper>
    <Wrapper hasGutter bgc="text" c="base">
      <Box py="40">
        <DummyText length="l" />
      </Box>
    </Wrapper>
    ```
  </PreviewCode>
</Preview>


<Spacer h="1rem"/>

### アニメーションをオンにする例

`isAnimation`でアニメーションをONにした時の例です。

<Preview>
  <PreviewTitle>Preview</PreviewTitle>
  <PreviewArea p="15" layout="" resize>
    <Container pos="relative" hasGutter>
      <ShapeDivider isEmpty level={7} />
      <Frame isLayer z="-1" max-w="100%">
        <Media src="https://cdn.lism-css.com/img/a-1.jpg" alt="" width="960" height="640" />
        <Layer bgc="black:50%" />
      </Frame>
      <Wrapper contentSize="s" py="40" c="white">
        <DummyText length="l" />
      </Wrapper>
      <ShapeDivider viewBox="0 0 100 10" isAnimation c="base" level={7}>
        <path opacity=".3" d="M100,5.5C91.5,3,79.5,7.5,71.5,8C58,9,49,0.5,36,2c-9,1-13.5,3.5-20.5,3.5C8.5,6,0,2,0,2v8h100V5.5z"></path>
        <path opacity=".3" d="M100,1.5C87.8,6.6,79.3,2.1,67.8,2.6S53,6.5,42.5,5.5S23,0,15,0S1,3,0,4v6h100V1.5z"></path>
        <path d="M100,7.5C91.5,5,77.1,11.8,54,8C34.6,4.8,14.4,4.1,0,8.5V10h100V7.5z"></path>
      </ShapeDivider>
    </Container>
    <Container isWrapper="s" hasGutter bgc="base" py="40">
      <DummyText length="l" />
    </Container>
  </PreviewArea>
  <PreviewCode slot="tab" label="JSX">
    ```jsx "isAnimation"
    <Container pos="relative">
      <ShapeDivider isEmpty level="7" />
      <Frame isLayer z="-1" max-w="100%">
        <Media src="https://cdn.lism-css.com/img/a-1.jpg" alt="" width="960" height="640" />
        <Layer bgc="black:50%" />
      </Frame>
      <Wrapper contentSize="s" hasGutter py="40" c="white">
        <DummyText length="xl" />
      </Wrapper>
      <ShapeDivider viewBox="0 0 100 10" isAnimation c="base" level="7">
        <path opacity=".3" d="M100,5.5C91.5,3,79.5,7.5,71.5,8C58,9,49,0.5,36,2c-9,1-13.5,3.5-20.5,3.5C8.5,6,0,2,0,2v8h100V5.5z"></path>
        <path opacity=".3" d="M100,1.5C87.8,6.6,79.3,2.1,67.8,2.6S53,6.5,42.5,5.5S23,0,15,0S1,3,0,4v6h100V1.5z"></path>
        <path d="M100,7.5C91.5,5,77.1,11.8,54,8C34.6,4.8,14.4,4.1,0,8.5V10h100V7.5z"></path>
      </ShapeDivider>
    </Container>
    <Container isWrapper="s" hasGutter bgc="base" py="40">
      <DummyText length="l" />
    </Container>
    ```
  </PreviewCode>
</Preview>

<Spacer h="1rem"/>

### 内部のSVGをずらして配置する例

上下に同じシェイプを使いまわす時、そのまま反転させると少し違和感がでてくるかもしれません。  
そういう場合は、`stretch`,`offset`でsvgをずらすことで、同じシェイプでも自然な表示にすることができます。

<Preview>
  <PreviewTitle>`stretch`,`offset`の活用</PreviewTitle>
  <PreviewArea resize p="0" layout="">
    <Container isWrapper="s" hasGutter bgc="base" py="40">
      <ShapeDivider isEmpty level={4} />
      <DummyText length="l" />
    </Container>
    <Container pos="relative" hasGutter c="base" bgc="text">
      <ShapeDivider viewBox="0 0 100 10" c="base" flip="Y" level={8} stretch="1.1" offset="-5%">
        <path d="M100 6C89.3 3.3 82.7 9 70 9S48.4 3 38 3 24.5 6 17 6C7.4 6 0 0 0 0v10h100V6z"></path>
      </ShapeDivider>
      <Wrapper contentSize="s" py="40">
        <DummyText length="xl" />
      </Wrapper>
      <ShapeDivider viewBox="0 0 100 10" c="base" level={8} stretch="1.1">
        <path d="M100 6C89.3 3.3 82.7 9 70 9S48.4 3 38 3 24.5 6 17 6C7.4 6 0 0 0 0v10h100V6z"></path>
      </ShapeDivider>
    </Container>
    <Container isWrapper="s" hasGutter bgc="base" py="40">
      <DummyText length="l" />
      <ShapeDivider isEmpty level={4} />
    </Container>
  </PreviewArea>
  <PreviewCode slot="tab" label="JSX">
    ```jsx "stretch=" "offset="
    <Container isWrapper="s" hasGutter bgc="base" py="40">
      <ShapeDivider isEmpty level="4" />
      <DummyText length="l" />
    </Container>
    <Container pos="relative" hasGutter c="base" bgc="text">
      <ShapeDivider viewBox="0 0 100 10" c="base" flip="Y" level="8" stretch="1.1" offset="-5%">
        <path d="M100 6C89.3 3.3 82.7 9 70 9S48.4 3 38 3 24.5 6 17 6C7.4 6 0 0 0 0v10h100V6z"></path>
      </ShapeDivider>
      <Wrapper contentSize="s" py="40">
        <DummyText length="xl" />
      </Wrapper>
      <ShapeDivider viewBox="0 0 100 10" c="base" level="8" stretch="1.1">
        <path d="M100 6C89.3 3.3 82.7 9 70 9S48.4 3 38 3 24.5 6 17 6C7.4 6 0 0 0 0v10h100V6z"></path>
      </ShapeDivider>
    </Container>
    <Container isWrapper="s" hasGutter bgc="base" py="40">
      <DummyText length="l" />
      <ShapeDivider isEmpty level="4" />
    </Container>
    ```
  </PreviewCode>
</Preview>

