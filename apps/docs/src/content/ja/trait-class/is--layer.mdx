...
`is--layer` は、absoluteで親要素の上に被せて配置するオーバーレイ用のクラスです。
...
`isLayer` プロパティで**どのコンポーネントからでも呼び出せるようになっています。**

| プロパティ | 出力 |
|---|---|
| `isLayer`| `is--layer` |

さらに、`<Lism isLayer>` のエイリアスとして、`<Layer>` も用意しています。
...
`style` プロパティで `backdropFilter` を直接指定できます。
