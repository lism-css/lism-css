...
The default `$breakpoints` map includes `xs` and `xl` keys alongside `sm`, `md`, and `lg`, but their size is `0` (disabled), so they produce no output. To use `xs` or `xl`, define its size in `$breakpoints`:
...
Any breakpoint with a size in `$breakpoints` becomes active: all Property Classes with responsive support (`bp: 1`) will automatically start outputting responsive classes for that breakpoint (such as `-p_xs` / `-p_xl`) — no per-prop configuration is needed.

This also allows you to specify `xs` / `xl` in object notation on the component side (to also show them in type completion, you need to extend `BreakpointRegistry` as described on the [Responsive page](/en/docs/responsive/#breakpoints-offered-by-type-completion)).
