...
:::info
The legacy subpath `lism-css/vite-plugin` (config alias only) remains available for backward compatibility, but we recommend using the `lism-css/vite` integrated plugin for new projects.
:::
