...
This is all it takes. Every Property Class that supports responsive output (`bp: 1`) will then also emit responsive classes for `xs` / `xl` (e.g., `-p_xs` / `-p_xl`) — no per-prop configuration is needed. When you are using the `lism-css/vite` integrated plugin, these changes are automatically reflected in the dev server and production build CSS.
...
To have `xs` / `xl` appear in type completion as well, see the [Responsive page](/en/docs/responsive/#breakpoints-offered-by-type-completion). If you are using the integrated plugin (`lism-css/vite`), `lism-env.d.ts` is generated automatically and no hand-written type extension is needed.
...
Use the integrated plugin provided by `lism-css/vite`. A single registration enables **config alias (so the JS runtime reads `lism.config.js`), dynamic CSS build (changes are automatically reflected in dev/build CSS), and automatic type generation (`lism-env.d.ts`)** all at once.
...
If you have registered the `lism-css/vite` integrated plugin (`lismCss()` / `lismCssAstro()`), any additions or changes to `props` / `tokens` in `lism.config.js` are **automatically reflected in the CSS, both in the dev server and in production builds**. No manual CLI build is needed.
...
For projects that do not use Vite or Astro (plain SCSS setups, other bundlers, etc.), run `npx lism-css build` to reflect `lism.config.js` into the CSS. This is not a legacy-only path — it is the **primary route** for that type of setup.
