...
:::info
In the [`<Lism>` component](/docs/core-components/Lism/), passing a string to the `hov` prop automatically outputs the corresponding `-hov` class.
:::
...
:::info
In the [`<Lism>` component](/docs/core-components/Lism/), passing an object like `hov={{ prop:value }}` outputs a `-hov:prop` class paired with a `--hov-prop:value` CSS variable.
:::
