...
| Variable setup | Setting up CSS variables only | `set--var:sh` / `set--var:hov` / `set--var:bdrsInner` |

The `{name}` in `set--var:{name}` corresponds to the name of the CSS variable each class sets up (e.g. `--sh--*` → `var:sh`, `--_isHov` / `--_notHov` → `var:hov`, `--bdrs--inner` → `var:bdrsInner`).
