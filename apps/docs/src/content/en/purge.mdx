...
The Lism CSS purge feature scans your build output and **keeps only the Lism CSS classes you actually use**. It ships with the `lism-css` package, so no extra install is required.


## Recommended setup (via the integrated plugin)

If you are using the `lism-css/vite` integrated plugin, just pass `purge: true` to enable it.
...
:::info
You can also use the standalone plugins directly: `lismPurge` from `lism-css/purge/vite` and `lismPurgeAstro` from `lism-css/purge/astro`.
:::
