...
Once purge is enabled and you switch to `full.css`, you gain access to token classes and extended breakpoints that `main.css` does not include, while the final CSS is narrowed down to **only the classes actually used on each page**.
...
- CSS sourcemaps are not supported. Purge removes rules, so an existing sourcemap no longer lines up. Disable purge if you need CSS sourcemaps (production builds usually ship CSS without sourcemaps anyway).
