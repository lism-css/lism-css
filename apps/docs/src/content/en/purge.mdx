...
`lism-css/purge` is a purge plugin that scans your build output and **keeps only the Lism CSS classes you actually use**. It ships with the `lism-css` package, so no extra install is required.

Two flavors are provided: a Vite plugin and an Astro integration.
...
import { lismPurge } from 'lism-css/purge';
