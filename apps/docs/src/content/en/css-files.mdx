...
description: 'Learn the differences between the main.css / main_no_layer.css / full.css / full_no_layer.css files bundled with lism-css, and what to know before using the @layer-free or full builds.'
---
...
:::info
Importing `full.css` alone does not change how components behave — they still use the default configuration (for example, `t="20"` is output as an inline style). To align the component output with `full.css`, enable [`isFullMode`](/en/docs/customize/config/#isfullmode) in your `lism.config.js`.
:::
