...
|`sz`|`inline-size`|-|-|
|`max-sz`|`max-inline-size`|`xs`,`s`,`m`,`l`,`xl`,`container`,`full`|-|
...
<Preview>
  ...
  <PreviewArea p="0" resize>
    <Flow set="gutter" py="20" w="100%" h="20em" wm={[null, 'vertical-rl']}>
      <DummyText length="s" />
  ...
  <PreviewCode slot="tab" label="HTML">
  ```html "-wm_sm" "--wm_sm:"
  <div class="l--flow set--gutter -py:20 -w:100% -h -wm_sm" style="--h: 20em; --wm_sm: vertical-rl">
...
In Lism, styles for the `:hover` state are defined with Property classes that start with `-hov:`.

However, `-hov` classes work with a slightly different mechanism compared to regular Property classes.
