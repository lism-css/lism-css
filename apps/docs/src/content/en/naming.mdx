...
`--{target}-{prop}` takes the same form whether the origin selector is `:root`, `.b--*`, or `.c--*`. A variable whose origin is not `:root` must be initialized on the origin selector so it never inherits a same-named value from ancestors or other components (e.g. `.b--list { --icon-size: 1em; }`). To change the value, set it on the origin element via inline style or props rather than on `:root`.
