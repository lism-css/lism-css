...
Use `--{target}-{prop}` in the same form whether the starting selector is `:root`, `.b--*`, or `.c--*`. However, when a component initializes a variable on its own selector, that declaration may take precedence over the value from `:root`. Use this form even when state changes the final property value.
