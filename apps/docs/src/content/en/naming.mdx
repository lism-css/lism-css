...
🎵 The opacity tokens (`--o--mp` / `--o--p` / `--o--pp` / `--o--ppp`) use semantic names derived from musical dynamic markings (the piano family). The more times `p` (piano / soft) is repeated, the higher the transparency. Using the number of repeated letters to indicate a stage is an exception unique to opacity within Lism.
:::
...
| `u--` | Decorative effects (including child-element decoration) | `@lism-utility` | `u--trim`, `u--cbox`, `u--divide`, `u--cells`, `u--clipText` |
