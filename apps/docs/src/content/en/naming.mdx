...
| `-10`, `-20`, `-30`... | When values decrease stepwise from `0` (`none`) | `--o--n10`, `--o--n20` |
...
As an exception, tokens whose value is `-{NUM}` are written as `n{NUM}` in the variable, but the class still uses `-{NUM}`.
