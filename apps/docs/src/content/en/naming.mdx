...
| `base` | Values set as the initial value of `:root` / `body` | `--fz--base`, `--hl--base` |
...
|------|------|-----|
| `--{prop}` | The same abbreviation as the class's `{prop}` portion | `--p`, `--bgc`, `--bdrs`, `--m` |
...
| Form | Use | Example |
| --- | --- | --- |
| `--{target}-{prop}` | Property set on a child element from a specific selector | `--link-td`, `--headings-ff`, `--icon-size` |
...
| `--_{varName}` | Variables for internal processing or state checks that are not intended to be overridden | `--_panelH`, `--_flipX`, `--_isHov` |

`--{target}-{prop}` takes the same form whether the origin selector is `:root`, `.b--*`, or `.c--*`. A variable whose origin is not `:root` follows these rules so that it is closed off from outside influence:

- Always initialize it on the origin selector (e.g. `.b--list { --icon-size: 1em; }`) so it never inherits a same-named value from ancestors or other components.
- To change the value, set it on the origin element via inline style or props rather than on `:root`.
...
Note that only `b--` and `c--` classes can have a BEM structure (Block / Modifier / Element). See [CSS Methodology](/en/docs/css-methodology/) for the notation details.
...
|---|---|---|---|
| `set--` | Base styling of HTML elements / variable setup | `lism-base` | `set--plain`, `set--hov`, `set--bxsh`, `set--bdrsInner` |
...
| `b--` | Common site-wide base parts whose base styles are managed in CSS (Block) | `lism-block` | `b--btn`, `b--badge` |
| `c--` | Free-form custom classes beyond the core definitions | `lism-custom` | `c--pricing`, `c--siteHeader` |
...
| `u--` | Decorative effects (including child-element decoration) | `lism-utility` | `u--trim`, `u--trimAll`, `u--cbox`, `u--srOnly`, `u--clipText` |
...
#### Consistent group abbreviations, no collisions

Properties in the same group must always start with the same group abbreviation.
Also, an abbreviation used by one group must not be reused by another group.

Incorrect examples:
- Abbreviating `flex` to `fx` but `flex-shrink` to `fsh` (this breaks the consistency of the `fx` group; the correct form is `fxsh`)
- Using `f` as the group abbreviation for `flex-*` (`f` is already the `font` group abbreviation, so `flex-wrap` would collide with `font-weight` as `fw`, and `flex-shrink` with `font-style` as `fs`. To avoid this, the `flex` group uses `fx`, giving `fxw` and `fxsh`)
...
For **single-letter properties that have no group**, the letter can be reused as the abbreviation for the first half of a different hyphenated property (for example, `t` for `top` is reused as the `t` of `text-*`).
...
:::note
The `g` in `grid-*` is an exception. Lism provides no Property Class for the `grid` property itself, so it does not collide with `g` for `gap` and is allowed.
:::
...
If a matching token exists, use the token value (the `{token}` portion of `--{prop}--{token}`).
