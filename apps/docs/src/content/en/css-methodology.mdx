...
|---|---|---|
|`-{prop}:{value}`| Classes for applying the primary values or token values for each property |`-fz:l`,<br/>`-d:none`|
|`-{prop}`| A class that receives the `--{prop}` variable |`-p`,<br/>`-fz`|
|`-{prop}_{bp}`| A class that receives the `--{prop}_{bp}` variable |`-p_sm`,<br/>`-p_md`|
