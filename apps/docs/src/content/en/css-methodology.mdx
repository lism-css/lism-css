...
When defining your own components (`.header`, `.footer`, `c--pricing`, etc.), place them inside `@layer lism-modules`.
...
| Prefix | Scope | User extension |
|---|---|---|
| `l--`, `a--` | Lism core (layout / atomic) | No (core only) |
| `c--` | Components | Users can add freely. The `lism-css` core does not define any `c--` classes — this namespace is reserved for users. `lism-ui` provides official preset `c--` components |
| `z--`, etc. | Zoning (`z--header`, `z--main`, `z--sidebar`, etc.) | User-defined OK |
| No prefix | User-defined | Free (`.header`, `.site-nav`, etc.) |
