...
A purge plugin that removes unused Lism CSS classes from your production build is bundled with the `lism-css` package. Import `lismPurgeAstro` from `lism-css/purge/astro` for Astro, or `lismPurge` from `lism-css/purge/vite` for Vite-based projects.
