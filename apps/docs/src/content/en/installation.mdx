...
:::info
Besides `main.css`, Lism also provides `main_no_layer.css` (without `@layer`) and `full.css`, a full build intended for use with CSS purge tools. See the [Customize page](/en/docs/customize/) for details.
:::
...
A purge plugin that removes unused Lism CSS classes from your production build is bundled with the `lism-css` package. If you are using the `lism-css/vite` integrated plugin (`lismCss()` / `lismCssAstro()`), just pass the `purge: true` option to enable it.
...
:::info
The standalone plugins (`lismPurgeAstro` from `lism-css/purge/astro` / `lismPurge` from `lism-css/purge/vite`) are still available for backward compatibility.
:::

See [CSS Purge](/en/docs/purge/) for the full setup, options, and safelist usage.
