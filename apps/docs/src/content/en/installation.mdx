---
title: 'Installation'
description: 'Learn how to install Lism CSS and get started with the basics.'
---

This guide explains how to set up and use Lism CSS.


There are two main approaches to getting started.

## CSS-only setup

For simple HTML sites or other cases where you only need the CSS file, you can use it via CDN.


<PreviewTitle>Load via CDN</PreviewTitle>
```html
<link href="https://cdn.jsdelivr.net/npm/lism-css@0.12.0/dist/css/main.css" rel="stylesheet" />
```

You can also download the CSS file and load it locally.


## Using dedicated components

React (.jsx) and Astro (.astro) components are distributed as an npm package.

### 1. Install the package

<Preview>
  <PreviewTitle>Install the package</PreviewTitle>
  <PreviewCode slot='tab' label='npm'>
  ```bash frame="none"
  npm i lism-css
  ```
  </PreviewCode>
  <PreviewCode slot='tab' label='pnpm'>
  ```bash frame="none"
  pnpm add lism-css
  ```
  </PreviewCode>
  <PreviewCode slot='tab' label='yarn'>
  ```bash frame="none"
  yarn add lism-css
  ```
  </PreviewCode>
</Preview>




### 2. Import the CSS

<PreviewTitle>Import it as a global style.</PreviewTitle>
```js
import 'lism-css/main.css';
```

For example, in Next.js, import it in `_app.js` or `layout.js`.



### 3. Import components

<Preview>
<PreviewTitle>Import individual components</PreviewTitle>
<PreviewCode slot='tab' label='.jsx'>
```jsx
import { Box, Flex, ... } from 'lism-css/react';
```
</PreviewCode>
<PreviewCode slot='tab' label='.astro'>
```jsx
import { Box, Flex, ... } from 'lism-css/astro';
```
</PreviewCode>
</Preview>



:::warning
::title[Note when importing .astro files]

Directly importing `.astro` files from inside `node_modules` may not work and could require additional configuration.
If you encounter a "**Directory import '...' is not supported resolving ES modules** ..." error, add `"lism-css"` to `vite.ssr.noExternal` in your `astro.config.js`.

<PreviewTitle>`astro.config.js`</PreviewTitle>
```js
import { defineConfig } from 'astro/config';

export default defineConfig({
  // ...other settings
  vite: {
    ssr: {
      noExternal: ['lism-css'],
    },
  },
});
```


<small>(We are investigating whether this can be resolved by improving the package distribution method...)</small>
:::



## Installing Lism UI

**UI components** such as accordions, tabs, and modals are provided as a separate package, `@lism-css/ui`.

### Use as a package

<Preview>
  <PreviewCode slot='tab' label='npm'>
  ```bash frame="none"
  npm i @lism-css/ui
  ```
  </PreviewCode>
  <PreviewCode slot='tab' label='pnpm'>
  ```bash frame="none"
  pnpm add @lism-css/ui
  ```
  </PreviewCode>
</Preview>


```js frame="none"
// Example: importing React components
import { Accordion, Tabs, Button } from '@lism-css/ui/react';

// Example: importing in .astro
import { Accordion, Tabs, Button } from '@lism-css/ui/astro';
```


### Copy to your project with the CLI

Using `@lism-css/cli`, you can copy UI component source code directly into your project and customize it freely.

```bash frame="none"
# Add a component (interactive setup on first run)
npx @lism-css/cli add accordion

# Add multiple components at once
npx @lism-css/cli add accordion modal tabs

# Add all components
npx @lism-css/cli add --all
```


## AI Tool Integration

Set up AI coding tools (Claude Code, Cursor, Codex, etc.) to work effectively with Lism CSS.

- [MCP Server](/docs/mcp/) — AI tools can reference component, Props, and token information in real time
- [Skills File](/docs/skills/) — Place a file in your project to communicate design rules to AI agents
