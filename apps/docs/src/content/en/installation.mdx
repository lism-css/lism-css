...
:::info
Besides `main.css`, Lism also provides `main_no_layer.css` (without `@layer`) and `full.css`, a full build intended for use with CSS purge tools. See [CSS File Types](/en/docs/css-files/) for details.
:::
...
The purge feature — which removes unused Lism CSS classes from your production build to keep the CSS lightweight — is provided by `@lism-css/plugin`. Just pass `purge: true` to the Vite / Astro integrated plugin (`lismCss()`) to enable it. See [CSS Purge](/en/docs/customize/purge/) for the full setup steps, options, and how to configure `safelist`.
