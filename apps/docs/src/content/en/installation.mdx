...
## Quick start (create-lism)

The fastest way to start from scratch is using `create-lism`.
...
This is equivalent to `lism create` from `@lism-css/cli`. If you want to add Lism CSS to an existing project, follow the sections below instead.
...
The legacy `lism-ui` command (`npx lism-ui add accordion`) continues to work via an internal redirect.
