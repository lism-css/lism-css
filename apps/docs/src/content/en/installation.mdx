...
The purge feature that removes unused Lism CSS classes from your production build is provided by `@lism-css/plugin`. For Vite / Astro projects, install `@lism-css/plugin` and pass `purge: true` to the integrated plugin (`lismCss()` / `lismCssAstro()`).
