...
| <PropBadge>`<Accordion.Heading>`</PropBadge><br/>`tag` | Specifies the HTML tag for the heading. Defaults to `div` (with `role="heading"` added automatically). If `h2`–`h6` is specified, `role` is not added. |
...
`<Accordion.Heading>` outputs a `<div role="heading">` by default, but you can specify a heading tag like `h3` via the `tag` prop to render it as an actual heading element.
...
<div class="c--accordion l--stack" data-allow-multiple>
	<div class="c--accordion_item" style="--duration: .15s">...</div>
	<div class="c--accordion_item" style="--duration: .15s">...</div>
	<div class="c--accordion_item" style="--duration: .15s">...</div>
</div>
```
