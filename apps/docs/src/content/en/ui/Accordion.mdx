---
title: 'Accordion'
description: 'Documentation for the Accordion component provided by @lism-css/ui.'
---

import { Divider, Stack, Center, Flex, Icon } from 'lism-css/astro';
import { Accordion } from '@lism-css/ui/astro/Accordion';
import { DummyText } from '@lism-css/ui/astro/DummyText';

A UI component for creating accordion elements.

It is built with `<div>` / `<button>` elements rather than `<details>` / `<summary>`, and uses JavaScript to control the open/close animation.
The panel uses `hidden="until-found"`, so its content can be discovered by the browser's in-page search even when the accordion is closed, and the panel will automatically expand when a match is found.


## Overview

<Preview>
  <PreviewTitle>Preview</PreviewTitle>
  <PreviewArea p="20">
    <Accordion.Root bd>
      <Accordion.Item>
        <Accordion.Heading>
          <Accordion.Button p="20">Accordion Example 1</Accordion.Button>
        </Accordion.Heading>
        <Accordion.Panel p="20" pt="5">
          <DummyText length="l" />
        </Accordion.Panel>
      </Accordion.Item>
      <Accordion.Item bd-t>
        <Accordion.Heading>
          <Accordion.Button p="20">Accordion Example 2</Accordion.Button>
        </Accordion.Heading>
        <Accordion.Panel p="20" pt="5">
          <DummyText length="l" />
        </Accordion.Panel>
      </Accordion.Item>
    </Accordion.Root>
  </PreviewArea>
  <PreviewCode slot="tab" label="JSX">
    ```jsx
    <Accordion.Root>
      <Accordion.Item>
        <Accordion.Heading>
          <Accordion.Button p="20">Accordion Example 1</Accordion.Button>
        </Accordion.Heading>
        <Accordion.Panel p="20" pt="5">
          <DummyText length="l" />
        </Accordion.Panel>
      </Accordion.Item>
      <Accordion.Item bd-t>
        <Accordion.Heading>
          <Accordion.Button p="20">Accordion Example 2</Accordion.Button>
        </Accordion.Heading>
        <Accordion.Panel p="20" pt="5">
          <DummyText length="l" />
        </Accordion.Panel>
      </Accordion.Item>
    </Accordion.Root>
    ```
  </PreviewCode>
  <PreviewCode slot="tab" label="HTML">
```html
<div class="b--accordion l--stack">
  <div class="b--accordion_item">
    <div class="b--accordion_heading set--plain" role="heading">
      <button class="b--accordion_button l--flex set--plain -g:10 -w:100% -ai:center -jc:between -p:20" aria-controls="{id}" aria-expanded="false">
        Accordion Example 1
        <span class="b--accordion_icon a--icon -pi:center -fxsh:0" aria-hidden="true"></span>
      </button>
    </div>
    <div class="b--accordion_panel -pos:relative -ov:hidden" id="{id}" hidden="until-found">
      <div class="b--accordion_content l--flow -p:20 -pt:5">
        ...Contents...
      </div>
    </div>
  </div>
  <div class="b--accordion_item -bd-t">
    ...
  </div>
</div>
```
  </PreviewCode>
</Preview>


:::note
`<Accordion.Button>` automatically includes `<Accordion.Icon>` at the end.
The icon is rendered using CSS pseudo-elements (`::before` / `::after`), so no SVG specification is needed.
:::


## Styles

The Accordion base styles are defined in the following CSS.

<SrcCode dir="lism-ui" path="src/components/Accordion/_style.css" title="_style.css" />

:::note
The full source code is available on [GitHub](https://github.com/lism-css/lism-css/tree/main/packages/lism-ui/src/components/Accordion).
:::


## Usage

Provided as `Accordion` from the `@lism-css/ui` package.


### Import

<ImportPackage component="Accordion" script="accordion.js" packageName="@lism-css/ui" />

The following components are available:

- `<Accordion.Root>`: The root element wrapping multiple accordion items (based on `Stack`)
- `<Accordion.Item>`: An individual accordion element
- `<Accordion.Heading>`: Wrapper for the heading area (defaults to `<div role="heading">`)
- `<Accordion.Button>`: The toggle button (automatically includes `<Accordion.Icon>` at the end)
- `<Accordion.Icon>`: The open/close state indicator icon (rendered via CSS pseudo-elements)
- `<Accordion.Panel>`: The content panel that opens and closes (uses `hidden="until-found"`)


### Props

| Prop | Description |
|---|---|
| <PropBadge>`<Accordion.Root>`</PropBadge><br/>`allowMultiple` | Allows multiple accordions to be expanded simultaneously. By default, only one can be open at a time. |
| <PropBadge>`<Accordion.Item>`</PropBadge><br/>`isOpen` | Expands the item by default (adds `data-opened`). Use together with `isOpen` on `<Accordion.Button>` / `<Accordion.Panel>`. |
| <PropBadge>`<Accordion.Heading>`</PropBadge><br/>`as` | Specifies the HTML tag for the heading. Defaults to `div` (with `role="heading"` added automatically). If `h2`–`h6` is specified, `role` is not added. |
| <PropBadge>`<Accordion.Button>`</PropBadge><br/>`isOpen` | Sets `aria-expanded` to match the initial expanded state. |
| <PropBadge>`<Accordion.Panel>`</PropBadge><br/>`flow` | Flow layout setting passed to the content area (`_content`) inside the panel. |
| <PropBadge>`<Accordion.Panel>`</PropBadge><br/>`isOpen` | Shows the panel by default (removes `hidden`). |


## Examples

### Changing the heading tag

`<Accordion.Heading>` outputs a `<div role="heading">` by default, but you can specify a heading tag like `h3` via the `as` prop to render it as an actual heading element.

<Preview>
  <PreviewTitle>Example</PreviewTitle>
  <PreviewArea p="20">
    <Accordion.Root bd>
      <Accordion.Item>
        <Accordion.Heading as="h3">
          <Accordion.Button p="20">Accordion Label</Accordion.Button>
        </Accordion.Heading>
        <Accordion.Panel p="20" pt="5">
          <DummyText length="l" />
        </Accordion.Panel>
      </Accordion.Item>
      <Accordion.Item bd-t>
        <Accordion.Heading as="h3">
          <Accordion.Button p="20">Accordion Label</Accordion.Button>
        </Accordion.Heading>
        <Accordion.Panel p="20" pt="5">
          <DummyText length="l" />
        </Accordion.Panel>
      </Accordion.Item>
    </Accordion.Root>
  </PreviewArea>
  <PreviewCode slot="tab" label="JSX">
    ```jsx 'as="h3"'
    <Accordion.Root bd>
      <Accordion.Item>
        <Accordion.Heading as="h3">
          <Accordion.Button p="20">Accordion Label</Accordion.Button>
        </Accordion.Heading>
        <Accordion.Panel p="20" pt="5">
          <DummyText length="l" />
        </Accordion.Panel>
      </Accordion.Item>
      <Accordion.Item bd-t>
        <Accordion.Heading as="h3">
          <Accordion.Button p="20">Accordion Label</Accordion.Button>
        </Accordion.Heading>
        <Accordion.Panel p="20" pt="5">
          <DummyText length="l" />
        </Accordion.Panel>
      </Accordion.Item>
    </Accordion.Root>
    ```
  </PreviewCode>
  <PreviewCode slot="tab" label="HTML">
  ```html "h3"
  <div class="b--accordion l--stack">
    <div class="b--accordion_item">
      <h3 class="b--accordion_heading set--plain">
        <button class="b--accordion_button l--flex set--plain -g:10 -w:100% -ai:center -jc:between" aria-controls="{id}" aria-expanded="false">
          Accordion Label
          <span class="b--accordion_icon a--icon -pi:center -fxsh:0" aria-hidden="true"></span>
        </button>
      </h3>
      ...
    </div>
  </div>
  ```
  </PreviewCode>
</Preview>


### Changing the animation duration

The speed of the open/close animation is controlled by the `--duration` variable, which defaults to `0.25s`.
You can change it by setting the CSS variable on `<Accordion.Root>` (for all items) or on an individual `<Accordion.Item>`.

<Preview>
  <PreviewTitle>Preview</PreviewTitle>
  <PreviewArea p="20">
    <Accordion.Root bd style={{ '--duration': '.15s' }}>
      <Accordion.Item>
        <Accordion.Heading>
          <Accordion.Button p="20">Accordion Example 1</Accordion.Button>
        </Accordion.Heading>
        <Accordion.Panel p="20" pt="5">
          <DummyText length="l" />
        </Accordion.Panel>
      </Accordion.Item>
      <Accordion.Item bd-t>
        <Accordion.Heading>
          <Accordion.Button p="20">Accordion Example 2</Accordion.Button>
        </Accordion.Heading>
        <Accordion.Panel p="20" pt="5">
          <DummyText length="l" />
        </Accordion.Panel>
      </Accordion.Item>
    </Accordion.Root>
  </PreviewArea>
  <PreviewCode slot="tab" label="JSX">
```jsx "--duration"
<Accordion.Root style={{ '--duration': '.15s' }}>
  <Accordion.Item>
    <Accordion.Heading>
      <Accordion.Button p="20">Accordion Example 1</Accordion.Button>
    </Accordion.Heading>
    <Accordion.Panel p="20" pt="5">
      ...
    </Accordion.Panel>
  </Accordion.Item>
  ...
</Accordion.Root>
```
  </PreviewCode>
  <PreviewCode slot="tab" label="HTML">
```html "--duration"
<div class="b--accordion l--stack" style="--duration: 0.15s">
  <div class="b--accordion_item">...</div>
  <div class="b--accordion_item">...</div>
</div>
```
  </PreviewCode>
</Preview>


### Opening the first item by default

To have an item expanded from the start, specify `isOpen` on each of `<Accordion.Item>`, `<Accordion.Button>`, and `<Accordion.Panel>`.

:::note
Because `isOpen` plays a different role on each component, specify it on all three.
On `<Accordion.Item>` it adds `data-opened` to set the panel height and icon to the open state, on `<Accordion.Button>` it controls `aria-expanded`, and on `<Accordion.Panel>` it toggles `hidden`.
:::

<Preview>
  <PreviewTitle>Preview</PreviewTitle>
  <PreviewArea p="20">
    <Accordion.Root bd>
      <Accordion.Item isOpen>
        <Accordion.Heading>
          <Accordion.Button p="20" isOpen>Accordion Label 1</Accordion.Button>
        </Accordion.Heading>
        <Accordion.Panel p="20" pt="5" isOpen>
          <DummyText length="l" />
        </Accordion.Panel>
      </Accordion.Item>
      <Accordion.Item bd-t>
        <Accordion.Heading>
          <Accordion.Button p="20">Accordion Label 2</Accordion.Button>
        </Accordion.Heading>
        <Accordion.Panel p="20" pt="5">
          <DummyText length="l" />
        </Accordion.Panel>
      </Accordion.Item>
    </Accordion.Root>
  </PreviewArea>
  <PreviewCode slot="tab" label="JSX">
```jsx "isOpen"
<Accordion.Root bd>
  <Accordion.Item isOpen>
    <Accordion.Heading>
      <Accordion.Button p="20" isOpen>Accordion Label 1</Accordion.Button>
    </Accordion.Heading>
    <Accordion.Panel p="20" pt="5" isOpen>
      ...
    </Accordion.Panel>
  </Accordion.Item>
  <Accordion.Item bd-t>
    <Accordion.Heading>
      <Accordion.Button p="20">Accordion Label 2</Accordion.Button>
    </Accordion.Heading>
    <Accordion.Panel p="20" pt="5">
      ...
    </Accordion.Panel>
  </Accordion.Item>
</Accordion.Root>
```
  </PreviewCode>
  <PreviewCode slot="tab" label="HTML">
```html "data-opened"
<div class="b--accordion l--stack">
  <div class="b--accordion_item" data-opened>
    <!-- button: aria-expanded="true" / panel: no hidden -->
    ...
  </div>
  <div class="b--accordion_item">...</div>
</div>
```
  </PreviewCode>
</Preview>


### Allowing multiple panels to be open

:::note
By default, only one accordion within the same `<Accordion.Root>` can be expanded at a time.
Specifying `allowMultiple` allows multiple panels to be open simultaneously.
:::

<Preview>
  <PreviewTitle>Preview</PreviewTitle>
  <PreviewArea p="20">
    <Accordion.Root allowMultiple bd>
      <Accordion.Item>
        <Accordion.Heading>
          <Accordion.Button p="20">Accordion Label 1</Accordion.Button>
        </Accordion.Heading>
        <Accordion.Panel p="20" pt="5">
          <DummyText length="l" />
        </Accordion.Panel>
      </Accordion.Item>
      <Accordion.Item bd-t>
        <Accordion.Heading>
          <Accordion.Button p="20">Accordion Label 2</Accordion.Button>
        </Accordion.Heading>
        <Accordion.Panel p="20" pt="5">
          <DummyText length="xl" />
        </Accordion.Panel>
      </Accordion.Item>
      <Accordion.Item bd-t>
        <Accordion.Heading>
          <Accordion.Button p="20">Accordion Label 3</Accordion.Button>
        </Accordion.Heading>
        <Accordion.Panel p="20" pt="5">
          <DummyText length="l" />
        </Accordion.Panel>
      </Accordion.Item>
    </Accordion.Root>
  </PreviewArea>
  <PreviewCode slot="tab" label="JSX">
```jsx "allowMultiple"
<Accordion.Root allowMultiple bd>
  <Accordion.Item>
    <Accordion.Heading>
      <Accordion.Button p="20">Accordion Label 1</Accordion.Button>
    </Accordion.Heading>
    <Accordion.Panel p="20" pt="5">
      ...
    </Accordion.Panel>
  </Accordion.Item>
  <Accordion.Item bd-t>
    <Accordion.Heading>
      <Accordion.Button p="20">Accordion Label 2</Accordion.Button>
    </Accordion.Heading>
    <Accordion.Panel p="20" pt="5">
      ...
    </Accordion.Panel>
  </Accordion.Item>
  ...
</Accordion.Root>
```
  </PreviewCode>
  <PreviewCode slot="tab" label="HTML">
```html "data-allow-multiple"
<div class="b--accordion l--stack" data-allow-multiple>
  <div class="b--accordion_item">...</div>
  <div class="b--accordion_item">...</div>
  <div class="b--accordion_item">...</div>
</div>
```
  </PreviewCode>
</Preview>
