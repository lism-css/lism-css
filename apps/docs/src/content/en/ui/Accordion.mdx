---
title: 'Accordion'
description: 'A UI component that creates accordion elements with animated expanding and collapsing panels.'
---
...
:::note
The full source code is available on [GitHub](https://github.com/lism-css/lism-css/tree/main/packages/lism-ui/src/components/Accordion).
:::
...
| <PropBadge>`<Accordion.Item>`</PropBadge><br/>`isOpen` | Expands the item by default (adds `data-opened`). Use together with `isOpen` on `<Accordion.Button>` / `<Accordion.Panel>`. |
...
| <PropBadge>`<Accordion.Button>`</PropBadge><br/>`isOpen` | Sets `aria-expanded` to match the initial expanded state. |
| <PropBadge>`<Accordion.Panel>`</PropBadge><br/>`as` | Specifies the HTML tag for the outer panel element (`.b--accordion_panel`). Defaults to `div`. |
| <PropBadge>`<Accordion.Panel>`</PropBadge><br/>Lism Props | Props such as `p`, `pt`, and `flow` are passed to the inner content area (`.b--accordion_content`). `as`, `class` (Astro) / `className` (React), `accID`, and `isOpen` are applied to the outer panel element. |
| <PropBadge>`<Accordion.Panel>`</PropBadge><br/>`isOpen` | Shows the panel by default (removes `hidden`). |
...
<div class="b--accordion -bd">
  ...
  <!-- Closed items keep aria-expanded="false" / hidden="until-found" -->
  <div class="b--accordion_item -bd-t">...</div>
  <div class="b--accordion_item -bd-t">...</div>
</div>
```
