...
If your site uses a fluid `--fz--base` (e.g. with `clamp()`) and you want line heights to change fluidly along with the font size, override the unit on `:root`, e.g. `--hl-unit: calc(var(--fz--base) * 0.125);`.
...
Tokens for `--lh`, which manages the actual `line-height` as a unitless ratio. When `--lh` is set, it takes precedence over the half-leading fallback in the universal rule above, and the ratio is inherited by child elements.
...
The values are chosen to match the default `hl` scale at the base font size (16px), so `-lh:m` roughly corresponds to the default body line spacing.
