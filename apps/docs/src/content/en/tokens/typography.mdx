...
Lism CSS takes a [distinctive approach](/en/docs/half-leading/) to `line-height`, managing it on every element through **half-leading values** (the spacing above and below each line): `line-height` is computed from the `--hl` variable.
