...
This page covers Lism's spacing tokens — a Fibonacci-based harmonic scale combined with practical 4px-step values — and the property classes that use them.
...
Because Fibonacci-only steps would leave the lower end too coarse, `--s5` (4px), `--s15` (12px), `--s25` (20px), and `--s35` (32px) are added so that everything up to `--s40` (40px) is filled in with practical 4px-step values.
