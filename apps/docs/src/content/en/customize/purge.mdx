...
The Lism CSS purge feature scans your build output and **keeps only the Lism CSS classes you actually use**. Plugins for Vite and Astro are available from the `@lism-css/plugin` package.
...
## Recommended setup (using the integrated plugin)
...
In SSR / hybrid setups (`output: 'server'`, or when some routes have `prerender = false`), the server output (`dist/server/`) is also scanned and reference-rewritten in addition to the client output (`dist/client/`). This ensures that classes used only on on-demand rendered pages are also kept.


## Pairing with full.css

`lism-css/full.css` is a **full build** that adds breakpoint-aware classes and color-token classes not included in `main.css` (see [CSS File Types](/en/docs/css-files/#using-the-full-build-fullcss) for what's included). It is large when loaded as-is, so it ships as a build **meant to be paired with purge**.
...
| Option | Description |
|---|---|
| <span class="-whs:nowrap">`safelist`</span> | Keep classes that static analysis cannot detect |
| `report` | Log the file size before and after purging |
| `known` | Override the set of classes purge treats as "Lism classes" (rarely needed) |
...
The set of classes purge treats as "Lism classes." When not specified, it's automatically built from the config-reflected `full.css` at the start of the build, so unused classes derived from props / tokens you added in `lism.config.js` are correctly purged too. Since `full.css` includes every selector in `main.css`, nothing is missed regardless of which one you load. Passing a function calls it at build time and uses the return value as the list.


## Caveats

- When you build class names dynamically, make sure the complete class name remains in your source (write it as `` `-p:${size}` ``; do not concatenate `'p-'` and `size` separately). If that is not practical, add the class to `safelist`.
- Purge keeps **every class used anywhere in the entire build output** (not per page). A class is retained as long as it is used on any page, even if it is unused on a given page. This is intentional so that a single CSS shared across multiple pages is never broken.
- Only CSS emitted by Lism CSS is purged. Your own CSS and styles from other libraries are not changed.
