...
If you are using `lism.config.js`, specifying `breakpoints` there is the easiest approach (see [lism.config.js](/en/docs/customize/config/#enabling-xs--xl-breakpoints)). For projects using SCSS directly, you can enable `xs` / `xl` by overriding the `$breakpoints` SCSS variable:
