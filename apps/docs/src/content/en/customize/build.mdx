...
- Classes for every color token on the color Property Classes (`bgc` / `c` / `bdc`), such as `-bgc:red`. `main.css` only outputs classes for a curated set of semantic colors, while `full.css` covers all tokens including the [palette colors](/en/docs/tokens/colors/)
