...
:::info
For Next.js, use `withLism()` from `@lism-css/plugin/next` instead of the Vite / Astro plugin. See [Setting up with Next.js](/en/docs/installation/#setting-up-with-nextjs) for the setup steps.
:::
...
:::warning
Note that adding props in `lism.config.js` only changes the HTML output from components — the styles for the added utility classes are not generated on their own. See [How to load additional styles](#how-to-load-additional-styles) for ways to apply them (automatic via the plugin / CLI / SCSS).
:::
...
To have `xs` / `xl` appear in type completion as well, see the [Responsive page](/en/docs/responsive/). If you are using the integrated plugin (`@lism-css/plugin`), `lism-env.d.ts` is generated automatically and no hand-written type extension is needed.
...
For projects using SCSS directly, you can also enable `xs` / `xl` via the `$breakpoints` SCSS variable override (see [SCSS Customization](/en/docs/customize/scss/)).
...
You can also override SCSS variables in line with your `lism.config.js` configuration before loading the styles. See [SCSS Customization](/en/docs/customize/scss/) for details on SCSS variables.
