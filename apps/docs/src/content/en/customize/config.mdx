...
Placing a `lism.config.js` (or `lism.config.ts` / `lism.config.mjs`) file in your project root lets you extend the props / tokens / traits and breakpoints your components accept.

Using `lism.config.js` requires `@lism-css/plugin`. With Vite, Astro, or Next.js, the bundler integration (the integrated plugin) reflects it automatically in both the components and the CSS. For any other setup, only the CSS is built, using the bundled CLI (see [Applying config without the integrated plugin](#applying-config-without-the-integrated-plugin)).


## Setup

Register the `lismCss()` integrated plugin from `@lism-css/plugin`. A single registration enables config loading (`lism.config.js`), automatic CSS reflection, and automatic generation of the `lism-env.d.ts` type definitions — all at once.

:::warning
This plugin is what loads `lism.config.js`. If it isn't registered, the file is ignored and Lism falls back to its default settings.
:::
...
:::warning
Enabling `xs` / `xl` adds a block for each active breakpoint to every Property Class that uses `bp: 1`, which increases the CSS size. **This is designed to be paired with [CSS Purge](/en/docs/customize/purge/).** If you are not using purge, consider using the list form of `bp` (e.g., `bp: ['sm', 'md']`) to restrict which breakpoints are output on a per-prop basis.
:::
...
Importing the [full build (`full.css`)](/en/docs/css-files/#using-the-full-build-fullcss) alone does not change how components behave — they still use the default configuration (for example, `t="20"` is output as an inline style). To align the component output with `full.css`, enable `isFullMode`.
...
Since components also don't read `lism.config.js` in this setup, additional values such as `p="box"` are still processed with the default configuration. To use them from a component, force the class output with the Lism Props [`:value` notation](/en/docs/core-components/lism-props/#output-utility) (`p=":box"`). For components rendered client-side, you can also merge the same settings at runtime by defining them in the browser via `window._LISM_CSS_CONFIG_` (except for `isFullMode`).
...
When you give a value in `tokens`, the build generates both the Property Class that references the token (such as `.-lts\:2xl { letter-spacing: var(--lts--2xl) }`) and the referenced CSS variable itself (such as `:root { --lts--2xl: .5em }`). A key whose value is `'-'` only registers the name and doesn't output a value — use it for tokens with no CSS variable (such as `palette.keycolor`) or tokens whose real value is defined in hand-written SCSS (such as `bdrs.inner`).
