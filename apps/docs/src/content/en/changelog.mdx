...
### Replace `-max-sz:container` with `-max-sz:bleed` (#359)

The previous `-max-sz:container` referenced `--sz--container`, which was updated by every `is--container`, so it expanded only up to the **nearest container width**. In practice, what is far more useful is a **full-bleed** that breaks through content widths and nested containers all the way out to the **page width / outermost container width**.

For that reason, `-max-sz:container` has been removed and replaced with `-max-sz:bleed` (no backward-compatible alias).

| Old | New |
|---|---|
| `-max-sz:container` / `max-sz="container"` | `-max-sz:bleed` / `max-sz="bleed"` |
| `--sz--container` | `--sz--bleed` |

`-max-sz:bleed` expands up to the **outermost `is--container`** width. The `@property --sz--bleed` `initial-value` is `100svi`, so when no `is--container` ancestor exists it falls back to the viewport width.

```scss
@property --sz--bleed {
  syntax: '<length-percentage>';
  initial-value: 100svi;
  inherits: true;
}

.is--container:not(:where(.is--container .is--container)) > * {
  --sz--bleed: 100cqi;
}
.is--container.has--gutter:not(:where(.is--container .is--container)) > * {
...
If the previous "expand up to the nearest container" behavior is needed again, we will consider reintroducing it separately.
...
To prevent this from breaking the negative-margin "hang" of `-max-sz:full` inside `has--gutter` and the centered placement of `-max-sz:bleed` (formerly `-max-sz:container`), both classes now also set `inline-size: auto` to reset the inherited `inline-size: 100%`.
