...
- **Breaking:** Icon presets now use strokes on a 24-unit grid and support `weight` (light 1 / regular 1.5 / bold 2) and `strokeWidth`. Set the color with `c`. Previously bold caret and arrow presets now default to regular; use `weight="bold"` for thicker strokes.
- Removed `book`, `cart`, `star`, `star-half`, `heart`, `lock`, `lock-open`, and fill icons from core presets. Use components from `@lism-css/icons` when needed. Removed `dot`. The default stroke width of `menu-2` is now 1.5; use `weight="light"` for a thinner appearance.
