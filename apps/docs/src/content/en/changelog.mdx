...
### Reorganized the CSS variable structure of `has--gutter` (#409)

- Renamed the root baseline value from `--gutter-size` to `--gutter--base`
- Added the element-local functional variable `--gutter`, initialized by `.has--gutter` with `var(--gutter--base)`
- Aligned gutter variables with the same two-layer structure as `--flow--base` / `--flow`
- You can now override the gutter per element with values like `style="--gutter: 20px"`
- Updated the `--sz--bleed` calculation in `is--container` and the negative margin behavior of `-max-sz:full` to reference `--gutter`

### Simplified table cell variables

- Removed `--td-c` / `--td-bgc` / `--td-min-sz` / `--th-c` / `--th-bgc` / `--th-min-sz` / `--th-p` (their fallback values matched the default behavior when undefined, so they were redundant)
- Renamed `--td-p` to `--cells-p`, now applied to both `td` and `th`
- Changed the fallback from `var(--s10) var(--s15)` to `0.625em 0.875em` (now scales with the parent font size)
- Final form: `td, th { padding: var(--cells-p, 0.625em 0.875em); }`

### Renamed `u--cells` to `u--enclose` (#411)

- The name `u--cells` poorly conveyed its effect, so it has been renamed to `u--enclose`
- The verb pair `divide` (split) ↔ `enclose` (wrap) now clearly contrasts the two utility classes
- No backward-compatible alias is provided; all usages must be replaced
...
### Redesigned `--sz--*` tokens with unified px and 80px increments (#364)

- Unified `--sz--*` tokens (previously mixed px and rem) to px only
- Restructured all values as multiples of 80px (×5, ×8, ×11, ×15, ×20)
...
### Expanded spacing tokens and new `set--var:s` utility (#377)

- Added `s25` (~20px) / `s35` (~32px), filling `s10`–`s40` with practical 4px-step values
- Restructured `--s-unit` to derive from `--fz--base` (`calc(var(--fz--base) * 0.5)`)
- Added `set--var:s` utility class that switches `--s-unit` to `0.5em`, allowing spacing to scale with element font size
- Removed `bleed` from `sz` tokens, moved to `max-sz`-specific preset

### Other

- Excluded `input[type='range']` from default form-field styles
- Fine-tuned oklch values for color palette tokens
...
- Negative margins can now be specified using spacing tokens. (e.g., `my-s="-20"`, `mt="-10"`)
