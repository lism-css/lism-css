...
Props corresponding to Trait Primitives classes (`is--*`).
...
For an overview of [Primitives classes](/en/docs/primitives/), see the following.

<DocsLink path="/docs/primitives/" />
...
```
[className] [a--] [l--] [is--] [set--] [u--] [-]
```

| # | Prop | Example |
|---|---|---|
| 1 | User-defined class (`className`) | `c--box`, `z--header`, `hoge` |
| 2 | `atomic` | `a--icon`, `a--divider` |
| 3 | `layout` | `l--flex`, `l--columns` |
| 4 | `isXxxx` | `is--wrapper`, `is--layer` |
| 5 | `set` | `set--hov`, `set--transition` |
| 6 | `util` | `u--cbox`, `u--trim` |
| 7 | `prop={value}` | `-p:20`, `-bgc:base-2` |
