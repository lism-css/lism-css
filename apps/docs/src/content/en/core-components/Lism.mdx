...
The `class` attribute generated by `<Lism>` is broadly divided into three blocks: **"user-specified", "module classes", and "utilities"**. The utility block is ordered from coarser to finer granularity: `set--` → `u--` → Property Class.
...
| 3 | Layout module | `layout` prop | `l--box`, `l--flow` |
| 4 | State Modules | State Props (`isWrapper`, etc.) | `is--wrapper`, `is--layer` |
...
Items 2–4 all belong to [Module Class](/en/docs/css-methodology/) (CSS layer `lism-modules`), while 5–7 are utility-type classes.

:::note
The `-flow:{size}` class produced by `layout="flow"` is treated as a layout-module-specific configuration class, so it is emitted directly after `l--flow`.
:::
