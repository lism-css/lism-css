...
| `isLinkBox` | `is--linkBox` | Make entire box clickable |
...
`set` accepts arrays, comma-separated, or space-separated values for multiple classes.  
Use `unset` to remove specific set classes applied internally by a component.

```jsx
<Lism set={['hov', 'transition']}>...</Lism>
<AccordionButton unset="plain">...</AccordionButton>
```
