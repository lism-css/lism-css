...
`lism-base` includes a `reset` sub-layer, and `lism-primitive` is further divided into `layout` and `atomic` sub-layers. `lism-trait` is the layer for Trait classes — `is--` (role declaration) and `has--` (feature attachment). `lism-component` is the layer for BEM-structured UI components prefixed with `c--`.
