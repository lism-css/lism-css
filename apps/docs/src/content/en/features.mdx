...
`lism-base` includes a `reset` sub-layer, and `lism-primitives` is further divided into `trait`, `layout`, and `atomic` sub-layers. `lism-components` is the layer for BEM-structured UI components prefixed with `c--`.
