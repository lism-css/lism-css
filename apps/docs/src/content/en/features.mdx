...
`lism-base` includes a `reset` sub-layer, and `lism-primitive` is further divided into `trait`, `layout`, and `atomic` sub-layers. `lism-component` is the layer for BEM-structured UI components prefixed with `c--`.
