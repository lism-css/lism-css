...
navtitle: 'l--sideMain'
description: 'l--sideMain is a two-column layout primitive that automatically switches between horizontal and vertical arrangements based on the content width you want to preserve.'
---
import { SideMain, Grid, Box, Stack, Spacer } from 'lism-css/astro';
import { DummyText } from '@lism-css/ui/astro';

`l--sideMain` is a two-column layout primitive that, without relying on media queries or container queries, **automatically switches between horizontal and vertical arrangements based on the content width you want to preserve**.
...
`SideMain` (`.l--sideMain`) is not dependent on breakpoints, but you can also build a similar layout using [`Grid`](/en/docs/primitives/l--grid) where the switch point is controlled by a breakpoint.
