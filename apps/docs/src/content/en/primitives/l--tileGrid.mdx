...
navtitle: 'l--tileGrid'
description: 'l--tileGrid is a layout primitive for creating uniform tile-style grid layouts by specifying cols and rows.'
---

import { TileGrid, Box, Center } from 'lism-css/astro';


`l--tileGrid` is a layout primitive for creating **uniform tile-style grid layouts** by specifying `cols` (number of columns) and `rows` (number of rows).
...
- [`<FluidCols>`](/en/docs/primitives/l--fluidcols): Fluid column count with auto-fit / auto-fill
