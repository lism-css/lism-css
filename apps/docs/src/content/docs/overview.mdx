...
<IconBadge /> [Every Layout](https://every-layout.dev/) + [Bootstrap](https://getbootstrap.com/) + [Tailwind CSS](https://tailwindcss.com/) を 混ぜ合わせたような設計になっています。  
...
<IconBadge /> ただし、React, Astroで**専用コンポーネント**を使った方が便利です。
...
<IconBadge type='warning' /> コンテナ要素の中では position: fixed の挙動が変わったり、サイズ関係の挙動が少し変わったりすることがあるため注意が必要となっています。
