...
`<Lism>`をベースとした`lism-css`で提供する各コンポーネントで受け取れるプロパティを**Lism Props**と呼んでいます。
...
[Layout State](/css/module-class/) クラスに対応するプロパティ群です。
...
<Reference>
[Layout State クラスについての解説と使用例はこちら](/css/module-class/)
</Reference>
