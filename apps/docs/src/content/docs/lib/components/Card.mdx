---
title: 'Card'
description: ...
---

import { Dummy, Box, Flex, Stack, Grid, GridItem, Frame, Text, LinkBox, Link, Layer, Columns, Media, WithSide } from 'lism-css/react';
import { Preview, PreviewArea, PreviewTitle, PreviewCode, ImportSource, ImportExComponent } from '~/components';
import Card from '~/components/ex/Card/index.jsx';


カード型コンテンツの作成例を紹介します。

<Preview>
	<PreviewTitle>Previews</PreviewTitle>
	<PreviewArea resize bg='u:stripe' p='0' isFlow={null}>
	<Box p={['30', null,'40']}>
		<Columns cols={[1, 2]} g={['30', null, '40']} colSize='16rem' ai='s'>
			<WithSide isLinkBox href='###' hov={{bxsh: '40'}} trs sideW='50%' bgc='base' bdrs='20' bxsh='20' ov='h' gridItem={{gc:'1/-1'}}>
				<Frame data-is-side ar='16/9' >
					<Media src='/img/img-4.jpg' alt='' width='960' height='640' />
				</Frame>
				<Stack g='30' p='30'>
					<h3 className='u--trimHL -fz:l'>Card Title</h3>
					<Text className='u--trimHL' fz='s' lh='s' op='high'>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。</Text>
					<Text d='if' mis='auto' mbs='auto' c='inherit' td='n' bd='b' lh='1' p='10'>View More →</Text>
				</Stack>
			</WithSide>
			<Frame pos='r' ar='3/4' bdrs='30' bxsh='30'>
				<Media src='/img/img-1.jpg' alt='' width='960' height='640' />
				<Layer bgc='rgb(0 0 0 / 20%)'/>
				<Layer layout={Stack} c='white' p='30' g='30'>
					<h3 className='u--trimHL -fz:l -mbs:a'>Card Title</h3>
					<Text className='u--trimHL' fz='s' lh='s' op='high'>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。</Text>
					<Link href='#' d='f' c='black' bgc='white' td='n' bd='b' lh='1' p='20' jc='c' bdrs='99' hov='fade'>Link Text</Link>
				</Layer>
			</Frame>
			<Stack p='20' g='20' c='text' bgc='base' bdrs='30' bxsh='30' className='u--outerR'>
				<Frame data-is-side ar='16/9' >
					<Media src='/img/img-1.jpg' alt='' width='960' height='640' className='u--innerR'/>
				</Frame>
				<Stack g='30' p='10'>
					<h3 className='u--trimHL -fz:l'>Card Title</h3>
					<Text className='u--trimHL' fz='s' lh='s' op='high'>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。</Text>
					<Link href='#' d='if' mis='auto' c='base' bgc='text' td='n' bd='b' lh='1' py='20' px='30' jc='c' hov='fade' className='u--innerR'>Link Text</Link>
				</Stack>
			</Stack>
			
		</Columns>
		</Box>
	</PreviewArea>
</Preview>

## Examples

### 画像の上にコンテンツを配置するカードの例

<Preview>
	<PreviewTitle>Preview</PreviewTitle>
	<PreviewArea resize p='30'>
		<Frame pos='r' ar='3/4' bdrs='30' bxsh='30' mx='a' maxW='xs'>
			<Media src='/img/img-1.jpg' alt='' width='960' height='640' />
			<Layer bgc='rgb(0 0 0 / 20%)'/>
			<Layer layout={Stack} c='white' p='30' g='30'>
				<h3 className='u--trimHL -fz:l -mbs:a'>Card Title</h3>
				<Text className='u--trimHL' fz='s' lh='s' op='high'>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。</Text>
				<Link href='#' d='f' c='black' bgc='white' td='n' bd='b' lh='1' p='20' jc='c' bdrs='99' hov='fade'>Link Text</Link>
			</Layer>
		</Frame>
	</PreviewArea>
	<PreviewCode slot='tab' label='JSX'>
		```jsx
		<Frame pos='r' ar='3/4' bdrs='30' bxsh='30' mx='a' maxW='xs'>
			<Media src='/img/img-1.jpg' alt='' width='960' height='640' />
			<Layer bgc='rgb(0 0 0 / 20%)'/>
			<Layer layout={Stack} c='white' p='30' g='30'>
				<h3 className='u--trimHL -fz:l -mbs:a'>Card Title</h3>
				<Text className='u--trimHL' fz='s' lh='s' op='high'>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。</Text>
				<Link href='#' d='f' c='black' bgc='white' td='n' bd='b' lh='1' p='20' jc='c' bdrs='99' hov='fade'>Link Text</Link>
			</Layer>
		</Frame>
		```
	</PreviewCode>
	<PreviewCode slot='tab' label='HTML'>
		```html
		<div class='l--frame -pos:r -ar -bdrs:30 -bxsh:30 -mx:a -maxW' style='--ar:3/4;--maxW:var(--size-xs)'>
			<img src='/img/img-1.jpg' width='960' height='640' />
			<div class='is--layer -bgc' style='--bgc:rgb(0 0 0 / 50%)'></div>
			<div class='l--stack is--layer -c -p:30 -g:30' style='--c:var(--white)'>
				<h3 class='u--trimHL -fz:l -mbs:a'>Card Title</h3>
				<p class='u--trimHL -fz:s -lh:s -op:high'>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。</p>
				<a class='-d:f -c -bgc -td:n -bd:b -lh:1 -p:20 -jc:c -bdrs:99 -hov:fade' style='--c:var(--black);--bgc:var(--white)' href='#'>Link Text</a>
			</div>
		</div>
		```
	</PreviewCode>
</Preview>



### u--innerR のカードへの活用例

<Preview>
	<PreviewTitle>Preview</PreviewTitle>
	<PreviewArea resize p='30'>
		<Stack p='10' g='20' c='text' bgc='base' bdrs='30' bxsh='30' className='u--outerR' maxW='xs' mx='a'>
			<Frame data-is-side ar='16/9' >
				<Media src='/img/img-1.jpg' alt='' width='960' height='640' className='u--innerR'/>
			</Frame>
			<Stack g='30' p='20'>
				<h3 className='u--trimHL -fz:l'>Card Title</h3>
				<Text className='u--trimHL' fz='s' lh='s' op='high'>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。</Text>
				<Link href='#' className='u--innerR' d='if' mis='auto' c='base' bgc='text' td='n' bd='b' lh='1' py='20' px='30' jc='c' hov='fade'>Link Text</Link>
			</Stack>
		</Stack>
	</PreviewArea>
	<PreviewCode slot='tab' label='JSX'>
		```jsx
		<Stack p='10' g='20' c='text' bgc='base' bdrs='30' bxsh='30' className='u--outerR' maxW='xs' mx='a'>
			<Frame data-is-side ar='16/9' >
				<Media src='/img/img-1.jpg' alt='' width='960' height='640' className='u--innerR'/>
			</Frame>
			<Stack g='30' p='10'>
				<h3 className='u--trimHL -fz:l'>Card Title</h3>
				<Text className='u--trimHL' fz='s' lh='s' op='high'>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。</Text>
				<Link href='#' className='u--innerR' d='if' mis='auto' c='base' bgc='text' td='n' bd='b' lh='1' py='20' px='30' jc='c' hov='fade'>Link Text</Link>
			</Stack>
		</Stack>
		```
	</PreviewCode>
	<PreviewCode slot='tab' label='HTML'>
		```html
		<div class='l--stack u--outerR -p:10 -g:20 -c:text -bgc:base -bdrs:30 -bxsh:30 -mx:a -maxW' style='--maxW:var(--size-xs)'>
			<div class='l--frame -ar:16/9' data-is-side='true'>
				<img class='u--innerR' src='/img/img-1.jpg' width='960' height='640' />
			</div>
			<div class='l--stack -g:30 -p:10'>
				<h3 class='u--trimHL -fz:l'>Card Title</h3>
				<p class='u--trimHL -fz:s -lh:s -op:high'>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。</p>
				<a class='u--innerR -d:if -mis:a -c:base -bgc:text -td:n -bd:b -lh:1 -py:20 -px:30 -jc:c -hov:fade' href='#'>Link Text</a>
			</div>
		</div>
		```
	</PreviewCode>
</Preview>


{/* ## Create Component

ここからは`Card`(`.c--card`)というコンポーネントを作って利用する例を紹介していきます。

### Source Code

<ImportSource name='Card' react />

### Import
<ImportExComponent name='Card' react />

### Props

| プロパティ | 説明 |
|---|---|
|`layout`| 展開するレイアウトコンポーネントを指定します。デフォルトは`<WihSide>`です。|
|`href`| この例では、hrefを指定すると自動で`a.is--linkBox`として展開されるようになっています。|
 */}



### 縦向き・横向きのレイアウトを切り替える例

[`WithSides`](/layout/withside/)を使って構築することで、表示領域が狭くなったら縦向きレイアウトにすることができます。

さらに、この例では[`is--linkBox`](/css/layout-state/#is--linkbox)を適用してカードコンテンツ全体をリンクにしてみます。

<Preview>
	<PreviewTitle>Preview</PreviewTitle>
	<PreviewArea resize p='40'>
		<WithSide tag='a' isLinkBox href='#_' hov={{bxsh: '40'}} trs sideW='50%' bgc='base' bdrs='20' bxsh='20' ov='h'>
			<Frame data-is-side ar='16/9' >
				<Media src='/img/img-4.jpg' alt='' width='960' height='640' />
			</Frame>
			<Stack g='30' p='30'>
				<h3 className='u--trimHL -fz:l'>Card Title</h3>
				<Text className='u--trimHL' fz='s' lh='s' op='high'>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。</Text>
				<Text d='if' mis='auto' mbs='auto' c='inherit' td='n' bd='b' lh='1' p='10'>View More →</Text>
			</Stack>
		</WithSide>
	</PreviewArea>
	<PreviewCode slot='tab' label='JSX'>
		```jsx "WithSide" "tag='a' isLinkBox"
		<WithSide tag='a' isLinkBox href='#_' hov={{bxsh: '40'}} trs sideW='50%' bgc='base' bdrs='20' bxsh='20' ov='h'>
			<Frame data-is-side ar='16/9' >
				<Media src='/img/img-4.jpg' alt='' width='960' height='640' />
			</Frame>
			<Stack g='30' p='30'>
				<h3 className='u--trimHL -fz:l'>Card Title</h3>
				<Text className='u--trimHL' fz='s' lh='s' op='high'>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。</Text>
				<Text d='if' mis='auto' mbs='auto' c='inherit' td='n' bd='b' lh='1' p='10'>View More →</Text>
			</Stack>
		</WithSide>
		```
	</PreviewCode>
</Preview>


カード全体をリンクにしつつ別のリンクを含めたい場合、 `u--overlayLink` を使用してください。

<Preview>
	<PreviewTitle>`u--overlayLink`を使う例</PreviewTitle>
	<PreviewArea resize p='40'>
		<WithSide isLinkBox href='#_' hov={{bxsh: '40'}} trs sideW='50%' bgc='base' bdrs='20' bxsh='20' ov='h'>
			<Frame data-is-side ar='16/9' pos='r'>
				<Media src='/img/img-5.jpg' alt='' width='960' height='640' />
				<Link href='#badge-link' pos='a' z='1' t='0' r='0' fz='s' bgc='black' c='white' m='20' px='20' bdrs='99' td='n' hov='fade'>Category</Link>
			</Frame>
			<Stack g='30' p='30'>
				<h3 className='u--trimHL -fz:l'>
					<Link className='u--overlayLink' href='#card-link'>Card Title (overlayLink)</Link>
				</h3>
				<Text className='u--trimHL' fz='s' lh='s' op='high'>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。</Text>
				<Text d='if' mis='auto' mbs='auto' c='inherit' td='n' bd='b' lh='1' p='10'>View More →</Text>
			</Stack>
		</WithSide>
	</PreviewArea>
	<PreviewCode slot='tab' label='JSX'>
		```jsx "isLinkBox" "\<Link " "u--overlayLink"
		<WithSide isLinkBox href='#_' hov={{bxsh: '40'}} trs sideW='50%' bgc='base' bdrs='20' bxsh='20' ov='h'>
			<Frame data-is-side ar='16/9' pos='r'>
				<Media src='/img/img-5.jpg' alt='' width='960' height='640' />
				<Link href='#badge-link' pos='a' z='1' t='0' r='0' fz='s' bgc='black' c='white' m='20' px='20' bdrs='99' td='n' hov='fade'>Category</Link>
			</Frame>
			<Stack g='30' p='30'>
				<h3 className='u--trimHL -fz:l'>
					<Link className='u--overlayLink' href='#card-link'>Card Title (overlayLink)</Link>
				</h3>
				<Text className='u--trimHL' fz='s' lh='s' op='high'>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。</Text>
				<Text d='if' mis='auto' mbs='auto' c='inherit' td='n' bd='b' lh='1' p='10'>View More →</Text>
			</Stack>
		</WithSide>
		```
	</PreviewCode>
</Preview>



### 画像比率を指定しながらコンテンツに合わせる

<Preview>
	<PreviewTitle>Flex時にメディア側のアスペクト比を指定すると、コンテンツ側の高さが低い場合にその比率で固定されます。</PreviewTitle>
	<PreviewArea resize p='30'>
		<LinkBox layout={Flex}
			href='#_' c='text' bgc='base' p='10' bdrs='20'
			bd bdw='3px' bdc='base-2'
			hov={['set', {bdc: 'link'}]}
		>
			<Frame w={['25%','30%']} ar='1/1' fxsh='0' bdrs='10'>
				<Media src='/img/img-2.jpg' alt='' width='960' height='640' />
			</Frame>
			<Stack fxg='1' g='30' p='30'>
				<h3 className='u--trimHL -fz:l'>Card Title</h3>
				<Text className='u--trimHL' fz='s' lh='s' op='high'>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。ブラインド行うにはいくつかの重要な事柄が流れます。</Text>
				<Text d='if' mis='auto' mbs='auto' c='var(--_isHov, var(--c-link)) var(--_notHov, inherit)' td='n' bd='b' lh='1' p='10'>View More →</Text>
			</Stack>
		</LinkBox>
	</PreviewArea>
	<PreviewCode slot='tab' label='JSX'>
		```jsx "ar='1/1'"
		<LinkBox layout={Flex}
			href='#_' c='text' bgc='base' p='10' bdrs='20'
			bd bdw='3px' bdc='base-2'
			hov={['set', {bdc: 'link'}]}
		>
			<Frame w={['25%','30%']} ar='1/1' fxsh='0' bdrs='10'>
				<Media src='/img/img-2.jpg' alt='' width='960' height='640' />
			</Frame>
			<Stack fxg='1' g='30' p='30'>
				<h3 className='u--trimHL -fz:l'>Card Title</h3>
				<Text className='u--trimHL' fz='s' lh='s' op='high'>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。ブラインド行うにはいくつかの重要な事柄が流れます。</Text>
				<Text d='if' mis='auto' mbs='auto' c='var(--_isHov, var(--c-link)) var(--_notHov, inherit)' td='n' bd='b' lh='1' p='10'>View More →</Text>
			</Stack>
		</LinkBox>
		```
	</PreviewCode>
</Preview>


### 横並びと横並びを切り替え、メディア比率も変更する

`Grid`でブレイクポイント指定で横並びと縦並びを切り替え、メディア比率も変更する例です。

<Preview>
	<PreviewTitle>ブレイクポイントで切り替える場合</PreviewTitle>
	<PreviewArea resize p='30'>
		<LinkBox layout={Grid}
			href='#_'
			gd={[`'img' '.'`, `'img .' / 200px 1fr`]}
			hov='set'
		>
			<GridItem layout={Frame} ga='img' aslf='stretch' ar={['ogp', '1/1']} maxW='100%' pos='r' bdrs='20' bxsh='10'>
				<Media css={{scale: 'var(--_isHov, 1.25)'}} trs src='/img/img-1.jpg' alt='' width='960' height='640' />
			</GridItem>
			<Stack g='30' py='30' px={['20','30']} op='var(--_isHov, 0.75)'>
				<h3 className='u--trimHL -fz:l'>Card Title</h3>
				<Text className='u--trimHL' fz='s' lh='s' op='high'>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。ブラインド行うにはいくつかの重要な事柄が流れます。</Text>
				<Text d='if' mis='auto' mbs='auto' c='inherit' td='n' bd='b' lh='1' p='10'>View More →</Text>
			</Stack>
		</LinkBox>
	</PreviewArea>
	<PreviewCode slot='tab' label='JSX'>
		```jsx
		<LinkBox layout={Grid}
			href='#_'
			gd={[`'img' '.'`, `'img .' / 200px 1fr`]}
			hov='set'
		>
			<GridItem layout={Frame} ga='img' aslf='stretch' ar={['ogp', '1/1']} maxW='100%' pos='r' bdrs='20' bxsh='10'>
				<Media css={{scale: 'var(--_isHov, 1.25)'}} trs src='/img/img-1.jpg' alt='' width='960' height='640' />
			</GridItem>
			<Stack g='30' py='30' px={['20','30']} op='var(--_isHov, 0.75)'>
				<h3 className='u--trimHL -fz:l'>Card Title</h3>
				<Text className='u--trimHL' fz='s' lh='s' op='high'>ロレムイプサムの座り雨、トマ好き学習だったエリット、しかし時と活力はそのような木々と楽しみ。ブラインド行うにはいくつかの重要な事柄が流れます。</Text>
				<Text d='if' mis='auto' mbs='auto' c='inherit' td='n' bd='b' lh='1' p='10'>View More →</Text>
			</Stack>
		</LinkBox>
		```
	</PreviewCode>
</Preview>



### ホバーでテキストを出現させる例

以下の例では、`.-hov\:get\:slide`クラスを別途用意する必要があります。

{/* Note: PreviewAreaにresize つけると Filterの挙動変わることに注意。 */}
<Preview>
	<PreviewTitle>Preview</PreviewTitle>
	<PreviewArea p='30'>
		<LinkBox href='#card-a' pos='r' bdrs='20' bxsh='20' ov='h' hov='set' maxW='xs' mx='a'>
			<Frame ar='1/1'>
				<img src='/img/img-5.jpg' alt='' width='960' height='640' />
			</Frame>
			<Layer bgc='rgba(0 0 0 / 10%)'/>
			<Stack pos='a' b='0' w='100%' p={['20', '30']} bgc='rgb(255 255 255 / 20%)'>
				<Layer blur='6px' brightness='1.1'/>
				<Box pos='relative' z='1' c='white' lh='s' css={{textShadow: '0 0 1em rgb(0 0 0 / 20%)'}}>
					<h3 className='-fz:l'>Card Title</h3>
					<Grid trs hov='get:slide'>
						<Box ov='h'>
							<Dummy py='5' fz='s' lh='xs'/>
						</Box>
					</Grid>
					<Text fz='xs' op='high'>2000.01.01 / Author</Text>
				</Box>
			</Stack>
		</LinkBox>
	</PreviewArea>
	<PreviewCode slot='tab' label='JSX'>
	```jsx "hov='set'" "hov='get:slide'"
	<LinkBox href='#card-a' pos='r' bdrs='20' bxsh='20' ov='h' hov='set' maxW='xs' mx='a'>
		<Frame ar='1/1'>
			<img src='/img/img-5.jpg' alt='' width='960' height='640' />
		</Frame>
		<Layer bgc='rgba(0 0 0 / 10%)'/>
		<Stack pos='a' b='0' w='100%' p={['20', '30']} bgc='rgb(255 255 255 / 20%)'>
			<Layer blur='6px' brightness='1.1'/>
			<Box pos='relative' z='1' c='white' lh='s' css={{textShadow: '0 0 1em rgb(0 0 0 / 20%)'}}>
				<h3 className='-fz:l'>Card Title</h3>
				<Grid trs hov='get:slide'>
					<Box ov='h'>
						<Dummy py='5' fz='s' lh='xs'/>
					</Box>
				</Grid>
				<Text fz='xs' op='high'>2000.01.01 / Author</Text>
			</Box>
		</Stack>
	</LinkBox>
	```
	</PreviewCode>
<PreviewCode slot='tab' label='追加CSS'>
```css
.-hov\:get\:slide {
	opacity: var(--_notHov, 0);
	grid-template-rows: var(--_isHov, 1fr) var(--_notHov, 0fr);
	transition-property: margin-block, padding-block, opacity, grid-template-rows;
}
```
</PreviewCode>
</Preview>



{/* <Preview>
	<PreviewTitle>rtl言語でのテスト</PreviewTitle>
	<PreviewArea p={20} maxW='40em'>
		<Card isFlex dir='rtl'>
			<Frame w='40%'>
				<img src='/img/img-1.jpg' alt='' width='960' height='640' />
			</Frame>
			<CardBody>
				<h3 className='-fz:l'>ما هو 'لوريم إيبسوم' ؟</h3>
				<p>هذا نص وهمي أنا أكتب جمل ليس لها معنى معين.هذا نص وهمي أنا أكتب جمل ليس لها معنى معين.هذا نص وهمي أنا أكتب جمل ليس لها معنى معين.</p>
				<Lism fz='s' aslf='fe' mbs='auto'>النص السفلي</Lism>
			</Stack>
		</Card>
	</PreviewArea>
	<PreviewCode slot='tab' label='JSX'>
		```jsx
		<Card isFlex dir='rtl'>
			<Frame w='40%'>
				<img src='/img/img-1.jpg' alt='' width='960' height='640' />
			</Frame>
			<CardBody>
				<h3 className='-fz:l'>ما هو 'لوريم إيبسوم' ؟</h3>
				<p>هذا نص وهمي أنا أكتب جمل ليس لها معنى معين.هذا نص وهمي أنا أكتب جمل ليس لها معنى معين.هذا نص وهمي أنا أكتب جمل ليس لها معنى معين.</p>
				<Lism fz='s' aslf='fe' mbs='auto'>النص السفلي</Lism>
			</Stack>
		</Card>
		```
	</PreviewCode>
</Preview> */}
