...
import { Preview, PreviewArea, PreviewTitle, PreviewCode, ImportPackage, EmbedCode, PropBadge, Callout, IconBadge } from "~/components";
...
<EmbedCode dirPath='packages/lism-css' srcPath='src/scss/modules/dynamic/_tabs.scss' />
...
|<PropBadge>`<Tabs.Root>`</PropBadge><br/>`listProps`|タブボタンを囲むリスト要素(`d--tabs_list`)へ渡すpropsを指定できます。|
...
`listProps`で`.d--tabs_list`にプロパティを渡せます。
...
`<Tabs.Root>`(`.d--tabs`)に`grid-template:'list panel' auto / auto 1fr;` を指定すると、タブリストとコンテンツが横並びになります。
...
<Preview>
	...
	<PreviewArea p={30} resize>
		...
		>
			<Tabs.Item>
				<Tabs.Tab px='20' px-s='10' py='10' py-s='5'>Tab 1</Tabs.Tab>
				<Tabs.Panel p='5'>
					<Dummy pre='Tab 01: '/>
				</Tabs.Panel>
			</Tabs.Item>
			<Tabs.Item>
				<Tabs.Tab px='20' px-s='10' py='10' py-s='5'>Tab 2</Tabs.Tab>
				<Tabs.Panel p='5'>
					<Dummy length='l' offset={1} pre='Tab 02: ' />
				</Tabs.Panel>
			</Tabs.Item>
...
`d--tabs_tab`では`--isSelected_`, `--notSelected_`が使えるようになっています。  
<IconBadge type='help'/> （[`set-hov`](/css/hover#set-hov)の`--isHov_`,`--notHov_`の解説を参考にしてください。）
...
:where(.d--tabs--line) {
	...
	.d--tabs_list {
		box-shadow: var(--sh-size) 0 var(--c--line);
	}
	.d--tabs_tab {
		font-size: var(--fz--s);
		padding: 0.5em 0.875em;
		box-shadow: var(--isSelected_, var(--sh-size) 0 0 currentColor);
	}
}
```
	</PreviewCode>
...
<Preview>
	...
	<PreviewArea p={30}>
		<Tabs.Root variant='emboss' g='20' ji='center'>
			<Tabs.Item>
				<Tabs.Tab>Tab 1</Tabs.Tab>
				<Tabs.Panel>
					<Dummy pre='Tab 01: '/>
...
:where(.d--tabs--emboss) {
	...
	.d--tabs_tab {
		...
		background-color: var(--isSelected_, var(--c--base));
		box-shadow: var(--isSelected_, var(--bxsh--10));
	}
}
```
	</PreviewCode>
