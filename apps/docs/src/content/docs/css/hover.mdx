...
[`set--transition`](/css/layout-state#set--transition)クラスを用意しています。
...
<div class='u--hidden' id='set--hover'></div>
## `.set--hover` と特殊変数


<IconBadge /> Lism では、[`set--hover`](/css/layout-state#set--hover)というクラスを定義しており、このクラスでは`--isHov_`変数と`--notHov_`変数を定義しています。
...
<Preview>
	...
	<PreviewArea p='40'>
		<div className='-p:40 -bd -fz:l set--hover -hov:test'>BOX</div>
	</PreviewArea>
...
さて、この`set--hover`が便利なのはここからです。
親要素に`set--hover`を付与してその子要素で`--isHov_`変数と`--notHov_`変数をうまく使うことで、hover時の子要素の挙動をコントロールできるようになります。
...
/* 親の set--hover がhoverされたら非表示にする */
.-hov\:to\:hide {
	opacity: var(--isHov_, 0);
}

/* 親の set--hover がhoverされたら表示する */
.-hov\:to\:show {
...
<Preview>
	...
	<PreviewArea p='30'>
		<Frame isLinkBox setHover href='#banner-link' ar='16/9' bdrs='30'>
			<Media isLayer src='/img/img-1.jpg' alt='' width='960' height='640' loading='lazy' />
			<Layer setTransition hov='get:show' bgc='rgb(0 0 0 / 40%)' blur='6px'>
				<Center h='100%' c='#fff'>
	...
	<PreviewCode slot='tab' label='HTML'>
		```html "set--hover" "-hov:to:show" "set--transition"
		<a class="l--frame is--linkBox set--hover -ar:16/9 -bdrs:30" href="#banner-link">
	...
	<PreviewCode slot='tab' label='JSX'>
		```jsx "setHover" "hov='get:show'" "setTransition"
		<Frame isLinkBox setHover href='#banner-link' ar='16/9' bdrs='30'>
			<Media isLayer src='/img/img-1.jpg' alt='' width='960' height='640' loading='lazy' />
			<Layer setTransition hov='get:show' bgc='rgb(0 0 0 / 40%)' blur='6px'>
				<Center h='100%' c='#fff'>
...
<Preview>
	...
	<PreviewArea p='30'>
		<LinkBox
			href='#banner-link'
			setHover ov='hidden' ar='16/9' bdrs='30'
			style={{'--transition-duration':'.6s'}}
		>
			<Frame isLayer>
				<Media setTransition hov='get:zoom' src='/img/img-2.jpg' alt='' width='960' height='640' loading='lazy'/>
			</Frame>
			<Layer setTransition hov='get:hide' grayscale='70%'/>
			<Center isLayer c='#fff' bgc='rgb(0 0 0 / 40%)'>
				<Text setTransition hov='get:letterSpread' fz='2xl'>LinkBox</Text>
			</Center>
		</LinkBox>
	</PreviewArea>
	...
	<PreviewCode slot='tab' label='HTML'>
		```html "set--hover" "-hov:to:zoom" "-hov:to:letterSpread" "--transition-duration"
		<a
			href="#banner-link"
			class="is--linkBox set--hover -ar:16/9 -ov:hidden -bdrs:30"
			style="--transition-duration:.6s"
		>
	...
	<PreviewCode slot='tab' label='JSX'>
		```jsx "setHover" "hov='get:zoom'" "hov='get:letterSpread'" "--transition-duration"
		<LinkBox
			href='#banner-link'
			setHover ov='hidden' ar='16/9' bdrs='30'
			style={{'--transition-duration':'.6s'}}
