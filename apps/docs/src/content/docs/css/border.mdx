...
- どの方向にボーダーを表示するかを `.-bd\:{side}`クラスで指定します。
- 全方向表示には `-bd` クラスを使用します。
- 細かなスタイルは `--bdc`, `--bdw`, `--bds` の3つの変数で指定します。
...
<Preview>
	...
	<PreviewCode label='HTML' slot='tab'>
		...
		<div class="l--box -bd -bds:ds -p:20" style="--bdw:2px;--bdc:var(--red)">
			<p> Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor. </p>
		</div>
		<div class="l--box -bd:l -p:20" style="--bdc:var(--blue);--bdw:0.4em">
			<p> Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor. </p>
		</div>
		```
	</PreviewCode>
