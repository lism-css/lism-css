...
<table className="l--table">
	...
	<tbody>
		...
		<tr>
			...
			<td>トークン定義や変数セット、HTML要素の初期スタイルをセットするレイヤー。<br />Base Set (`.set--{syle}`)
			クラスもここに入ります。</td>
		</tr>
		<tr>
...
<table className="l--table">
	...
	<tbody>
		<tr>
			...
			<td>
				レイアウトを組む上で特定の役割を持ち、複数のレイアウトモジュールに対してつけ外しできるような汎用的な機能を持つクラス。(`.is|has--{state}`)
			</td>
		</tr>
		<tr>
			<td class='-whitespace:nowrap'>Layout Modules<br/>`@lism.modules.layout`</td>
			<td>
				レイアウトの構成単位となるようなモジュール群（`.l--{module}`）を定義するレイヤー。
			</td>
		</tr>
		<tr>
			<td class='-whitespace:nowrap'>Dynamic Modules<br/>`@lism.modules.dynamic`</td>
			<td>JavaScriptと組み合わせて動的な機能を持つ特殊なモジュール群 (`.d--{module}`) 。</td>
		</tr>
	</tbody>
</table>
