...
```jsx
// ① Lism コンポーネントを使う
<Box p='15' bgc='base-2' fz='xl'>text</Box>

// ② ただの div タグを使う
<div {...getLismProps({layout:'box', p:'20', fz:'xl'})}>text</div>
```
...
const returnProps = getLismProps({className:'foo', fz:'xl', p:['20', '30']});
```

<PreviewTitle>返り値</PreviewTitle>
```js
{
	className: 'foo -fz:l -p:20 -p_sm',
	style: {'--p_sm':'var(--s30)'},
}
```
...
<Note title='注意点' type='warning' flow='s'>
