...
| `blur`,`contrast`,`brightness`,`grayscale`,`hueRotate`,`invert`,`saturate`,`sepia` | これらをまとめて連結し、 `bdfltr` に受け渡します。|
...
Lismコンポーネントでは、`bdfltr` に指定した値を`backdrop-filter`として出力できるようになっていますが、`<Layer>`コンポーネントでは、`blur`などの主要成分を個別に指定できるように追加調整されており、この機能をより活用しやすいようになっています。
