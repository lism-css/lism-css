...
2. `<LinkBox>`(`.is--linkBox`）を`<div>`にして、内部に`a.set--clickExpand`を配置する。
...
<Preview>
	...
	<PreviewArea p='30'>
		<LinkBox href='#linkbox-demo01' p='40' bgc='base' bdc='line' bd bdrs='30'>
			...
			<Dummy lh='s' my-s='20' />
			<Text fz='s' c='text-2' ta='right' lh='1' my-s='10'>MORE →</Text>
		</LinkBox>
	</PreviewArea>
...
### 2. 内部に`set--clickExpand`を配置する

内部のアンカー要素に`.set--clickExpand`を併用してもボックス全体をリンクにすることができます。
...
<Preview>
	...
	<PreviewArea p='30'>
		<LinkBox tag='section' p='40' bgc='base' bdc='line' bd bdrs='30' setTransition hov='bxsh'>
			<Text fz='xl' fw='bold'>
				<Link setClickExpand href='#linkbox-demo02'>Heading link text</Link>
			</Text>
			<Dummy c='text-2' lh='s' my-s='20' />
			<Text id='inner-link02' my-s='10'>
				<a href='#inner-link02' className='-hov:o'>Inner Link</a>
			</Text>
		</LinkBox>
	</PreviewArea>
	<PreviewCode slot='tab' label='JSX'>
		```jsx "setClickExpand" "set--clickExpand"
	...
	<PreviewCode slot='tab' label='HTML'>
		...
		<div class="is--linkBox -hov:bxsh -p:40 -bgc:base -bdc:divider -bd -bdrs:30 set--transition">
			<p class="-fz:xl -fw:bold">
				<a class="set--clickExpand" href="#linkbox-demo02">Heading Link</a>
			</p>
			<p class="-c:text-2 -lh:s -my-s:20">
				Lorem ipsum dolor sit, amet consectetur adipisicing elit, sed do eiusmod tempor. Non facere laudantium ex eos doloribus aut dolore nisi provident.
			</p>
			<p id="inner-link02" class="-my-s:10" >
...
`set--clickExpand`でリンクを設置すると、Tabキーでフォーカスした時はアウトラインがボックス全体ではなくリンクテキスト部分のみになります。

もしこれをボックス全体に広げたい場合、以下のようなCSSを追記してください。

```css
@supports selector(:has(*)) {
	.is--linkBox:has(.set--clickExpand:focus-visible) {
		outline: auto 1px;
		outline: auto 1px -webkit-focus-ring-color;
	}
	.set--clickExpand:focus {
		outline: 0;
	}
}
```
...
- `set--clickExpand`のようにリンク要素の擬似要素を広げてボックス全体をクリックできるようにする手法では、altキーを押しながらのリンク内テキスト選択ができなくなるという問題があります。
