...
In `main.css`, the default breakpoints are `xs: 0` (disabled) / `sm: 480px` / `md: 800px` / `lg: 1120px` / `xl: 0` (disabled). A value of `0` means "disabled — no CSS query is output".
...
:::info
In `full.css`, `xs` is enabled at `360px`, while `xl` is disabled. The same defaults apply to `main.css` generated with `isFullMode: true`. Values specified in `breakpoints` take precedence. To use `xs` in components, use the configuration above or `isFullMode: true` with type generation through the integrated plugin.
:::
...
Note that adding values to an existing prop (such as `ta="justify"`) also reflects only the added values in the types, as `CustomPropValueRegistry` augmentations, and adds them to the completion candidates. Existing props already accept arbitrary strings, though, so they never raise a type error even without adding the value.
