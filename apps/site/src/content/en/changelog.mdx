...
- `Icon` supports `weight` (light 1 / regular 1.5 / bold 2) and `strokeWidth`. Stroke icons in `@lism-css/icons` use a 24-unit grid and default to a stroke width of 1.5; set their color with `c`. Use `weight="bold"` for previously bold caret and arrow icons, or `weight="light"` for a thinner `menu-2`. Removed `dot`.
