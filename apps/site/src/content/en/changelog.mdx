...
- **Breaking:** `Icon` no longer converts `weight` to a stroke width. You can use components such as `CheckIcon` from `@lism-css/icons@0.3.0`. For SVG strings or native elements, set the stroke width with `strokeWidth` (or `stroke-width` in Astro).
...
- All icons are now original designs published as `@lism-css/icons` from the separate [lism-css/icons](https://github.com/lism-css/icons) repository. See the [`@lism-css/icons` page](/en/docs/packages/icons/) for the full list.
