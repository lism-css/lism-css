...
React（.tsx）形式と Astro（.astro）形式のコンポーネントを、npm パッケージ `lism-css` で配布しています。

### 1. パッケージ(`lism-css`)のインストール

<Preview>
  <PreviewTitle>パッケージのインストール</PreviewTitle>
  <PreviewCode slot="tab" label="npm">
  ```bash frame="none"
  npm i lism-css
  ```
...
:::info
`main.css` のほかに、`@layer` を使わない `main_no_layer.css` や、CSS Purge との併用を前提にした全部入りビルドの `full.css` もあります。詳しくは[CSSファイルの種類](/docs/css-files/)をご覧ください。
:::
...
`withLism()` は `lism.config.js` を読み込み、設定を反映した CSS を `.lism-css/css/` 以下へ事前生成します。そのうえで `lism-css/main.css` などの import を生成済み CSS へ差し替えます（alias）。Next.js には Vite / Astro のような「import をその場で変換する仕組み」が無いため、この方式を取っています。既存の `nextConfig` は壊さずにマージされます。
...
設定できる項目の詳細は[カスタマイズページ](/docs/customize/config/)を参照してください。
...
:::info
Next.js 向けの **CSS Purge（未使用クラスの削除）は現在未対応**です（Vite / Astro のみ対応）。Next.js では `lism.config.js` を反映した CSS がそのまま読み込まれます。
:::
...
`@lism-css/ui/react` / `@lism-css/ui/astro` から複数コンポーネントを一括で import することもできますが、ビルド環境によっては未使用コンポーネントのコードや CSS まで bundle に含まれてしまう可能性があります。**本番ビルドでは、上記のようにコンポーネント単位の個別パスから import することを推奨します。**
...
`lism-cli ui add` を使うと、UIコンポーネントのソースコードを自分のプロジェクトにコピーし、自由にカスタマイズできます。コンポーネント名は `import` するときと同じ PascalCase での指定を推奨します。
...
名前の大文字・小文字、ハイフン、アンダースコアは区別されないため、`NavMenu` / `navmenu` / `nav-menu` / `nav_menu` のどの表記も同じコンポーネントとして扱われます。


## 未使用CSSの削除（CSS Purge）

本番ビルド時に未使用の Lism CSS クラスを取り除き、CSS ファイルを軽量化する CSS Purge は `@lism-css/plugin` で提供しています。Vite / Astro の統合プラグイン（`lismCss()`）に `purge: true` を渡すだけで有効になります。セットアップ手順・オプション・safelist の指定方法は [CSS Purge](/docs/customize/purge/) を参照してください。
