---
title: 'Hero'
description: 'Lism CSS で実装するヒーローセクションの実装例です。'
draft: true
---

{/* memo: #566 でヒーローの作例は /patterns/hero/ へ移した。
    このページは「is--layer / ShapeDivider との組み合わせ」の作例を
    General（section）カテゴリへ移植するまでの退避用に draft で残している。 */}

import { Spacer, Box, Flow, Flex, Media, Layer, Stack, Center, Frame, Heading, Text } from 'lism-css/astro';
import { ShapeDivider } from '@lism-css/ui/astro/ShapeDivider';
import { DummyText } from '@lism-css/ui/astro/DummyText';

ヒーローコンテンツの作成例を紹介します。


{/* `<Hero>`内に`<HeroContent>`を使うことで、上・中央・下寄せでコンテンツを配置できます。 */}
{/* <p>親ブロックからbodyブロックを上・中央・下寄せをコントロールできるように。(marginを変える)</p>
<p>初期展開ブロックを選べるようにする。内部gridパターンも。</p> */}


## Examples

<Preview>
  <PreviewTitle>例 1</PreviewTitle>
  <PreviewArea resize>
    <Stack isContainer max-sz="full" min-h="50vh" bgc="base-2">
      <Flow isWrapper hasGutter my="auto">
        <Text fw="bold" fz="xl">
          <Fragment>DEMO 01</Fragment>
        </Text>
        <DummyText length="l" lang="ja" />
      </Flow>
    </Stack>
  </PreviewArea>
  <PreviewCode>
    ```jsx
    <Stack isContainer max-sz="full" min-h="50vh" bgc="base-2">
      <Flow isWrapper hasGutter my="auto">
        <Text fw="bold" fz="xl">
          DEMO 01
        </Text>
        <p>ロレム・イプサムの座り雨。目まぐるしい文章の流れの中で、それは静かに歩く仮の言葉です。Elitも穏やかに続いていきますが、積み重ねられてきた「LiberroyとFoogの取り組み」は、余白のようなものです。作業が進むにつれて、工夫や考えとともに関心が折り重なりながらも、必要以上に主張せず彼らの作品は私たちに一定の示唆を与えてくれます。</p>
      </Flow>
    </Stack>
    ```
  </PreviewCode>
</Preview>

### ヘッダー・フッター・レイヤー要素を追加

<Preview>
  <PreviewTitle>例2: ヘッダー・フッター・レイヤー要素を追加</PreviewTitle>
  <PreviewArea resize>
    <Stack isContainer max-sz="full" min-h="68vh" g="20" c="white" pos="relative">
      <Frame isLayer>
        <Media isLayer src="https://cdn.lism-css.com/img/KRQPW50jXaLu.webp" alt="" width="960" height="640" />
        <Layer style={{ backdropFilter: 'blur(4px)' }} bgc="black:50%" />
      </Frame>
      <Flex jc="between" p="20" z="0" g="20">
        <Box>Header</Box>
        <Flex g="20">
          <div>XXX</div>
          <div>XXX</div>
        </Flex>
      </Flex>
      <Flow isWrapper hasGutter my="auto" z="0">
        <Text fw="bold" fz="xl">
          <Fragment>DEMO 02</Fragment>
        </Text>
        <DummyText length="l" lang="ja" />
      </Flow>
      <Center p="15" z="0">
        <Fragment>Footer</Fragment>
      </Center>
    </Stack>
  </PreviewArea>
  <PreviewCode>
    ```jsx
    <Stack isContainer max-sz="full" min-h="68vh" g="20" c="white" pos="relative">
      <Frame isLayer>
        <Media isLayer src="https://cdn.lism-css.com/img/KRQPW50jXaLu.webp" alt="" width="960" height="640" />
        <Layer style={{ backdropFilter: 'blur(4px)' }} bgc="black:50%" />
      </Frame>
      <Flex jc="between" p="20" z="0" g="20">
        <Box>Header</Box>
        <Flex g="20">
          <div>XXX</div>
          <div>XXX</div>
        </Flex>
      </Flex>
      <Flow isWrapper hasGutter my="auto" z="0">
        <Text fw="bold" fz="xl">
          DEMO 02
        </Text>
        <p>ロレム・イプサムの座り雨。目まぐるしい文章の流れの中で、それは静かに歩く仮の言葉です。Elitも穏やかに続いていきますが、積み重ねられてきた「LiberroyとFoogの取り組み」は、余白のようなものです。作業が進むにつれて、工夫や考えとともに関心が折り重なりながらも、必要以上に主張せず彼らの作品は私たちに一定の示唆を与えてくれます。</p>
      </Flow>
      <Center p="15" z="0">
        Footer
      </Center>
    </Stack>
    ```
  </PreviewCode>
</Preview>

{/* Stack直下なので、position:relativeがなくてもz-indexが効きます。 */}


### `is--layer`, `ShapeDivider` との組み合わせ


{/* <Spacer h="10"/> */}

以下の例では、オプトインコンポーネントの[`<ShapeDivider>`](/ui/ShapeDivider/)も使用しています。
<Preview>
  <PreviewTitle>With ShapDivide & Layers</PreviewTitle>
  <PreviewArea resize>
    <Stack isContainer max-sz="full" hasGutter pos="relative" min-h="50vh" c="white" g="15">
      <ShapeDivider viewBox="0 0 10 10" c="base" level="6" flip="XY" z="1">
        <path d="M10,10V0L0,9.5V10L10,10z" />
      </ShapeDivider>
      <Frame isLayer>
        <Media isLayer src="https://cdn.lism-css.com/img/2SvHdIk1KOOR.webp" alt="" width="960" height="640" />
        <Layer style={{ backdropFilter: 'blur(4px)' }} bgc="black:50%" />
      </Frame>
      <Flow isWrapper py="30" my="auto" z="0">
        <Heading level="2" fw="bold" fz="xl">
          <Fragment>ShapeDivider & Layers</Fragment>
        </Heading>
        <DummyText length="l" lang="ja" />
      </Flow>
      <ShapeDivider viewBox="0 0 10 10" c="base" level="6" z="1">
        <path d="M10,10V0L0,9.5V10L10,10z" />
      </ShapeDivider>
    </Stack>
  </PreviewArea>
  <PreviewCode>
    ```jsx "\<Frame isLayer" "\<ShapeDivider"
    <Stack isContainer max-sz="full" hasGutter pos="relative" min-h="50vh" c="white" g="15">
      <ShapeDivider viewBox="0 0 10 10" c="base" level="6" flip="XY" z="1">
        <path d="M10,10V0L0,9.5V10L10,10z" />
      </ShapeDivider>
      <Frame isLayer>
        <Media isLayer src="https://cdn.lism-css.com/img/2SvHdIk1KOOR.webp" alt="" width="960" height="640" />
        <Layer style={{ backdropFilter: 'blur(4px)' }} bgc="black:50%" />
      </Frame>
      <Flow isWrapper py="30" my="auto" z="0">
        <Heading level="2" fw="bold" fz="xl">
          ShapeDivider & Layers
        </Heading>
        <p>ロレム・イプサムの座り雨。目まぐるしい文章の流れの中で、それは静かに歩く仮の言葉です。Elitも穏やかに続いていきますが、積み重ねられてきた「LiberroyとFoogの取り組み」は、余白のようなものです。作業が進むにつれて、工夫や考えとともに関心が折り重なりながらも、必要以上に主張せず彼らの作品は私たちに一定の示唆を与えてくれます。</p>
      </Flow>
      <ShapeDivider viewBox="0 0 10 10" c="base" level="6" z="1">
        <path d="M10,10V0L0,9.5V10L10,10z" />
      </ShapeDivider>
    </Stack>
    ```
  </PreviewCode>
</Preview>


<Preview>
  <PreviewTitle>clipPathと組み合わせ、背景に画像があるコンテンツの境界を斜めにつなぐ</PreviewTitle>
  <PreviewArea resize p="20">
    <Box max-sz="full" style={{ '--slopeH': 'clamp(2rem, 4cqw, 4rem)' }}>
      <Stack isContainer max-sz="full" hasGutter pos="relative" min-h="50vh" c="white" g="15">
        <ShapeDivider viewBox="0 0 10 10" h="var(--slopeH)" c="base" flip="Y" z="1">
          <path d="M10,10V0L0,9.5V10L10,10z"></path>
        </ShapeDivider>
        <Frame isLayer>
          <Media isLayer src="https://cdn.lism-css.com/img/KRQPW50jXaLu.webp" alt="" width="960" height="640" />
          <Layer bgc="black:50%" />
        </Frame>
        <Flow isWrapper py="30" pe={['0', '25%', '33%']} my="auto" z="0">
          <Heading level="2" fw="bold" fz="xl">
            <Fragment>ShapeDivider & Layers</Fragment>
          </Heading>
          <DummyText length="l" lang="ja" />
        </Flow>
        <Spacer h="var(--slopeH)" />
      </Stack>
      <Stack isContainer max-sz="full" hasGutter pos="relative" min-h="50vh" c="white" mbs="calc(0px - var(--slopeH))">
        <Frame
          isLayer
          style={{
            clipPath: 'polygon(0 var(--slopeH), 0 100%, 100% 100%, 100% 0)',
          }}
        >
          <Media isLayer src="https://cdn.lism-css.com/img/hwrIQ2ajud1n.webp" alt="" width="960" height="640" />
          <Layer bgc="black:50%" />
        </Frame>
        <Spacer h="var(--slopeH)" />
        <Flow isWrapper py="30" ps={['0', '25%', '33%']} my="auto" z="0" ta="right">
          <Heading level="2" fw="bold" fz="xl">
            <Fragment>ShapeDivider & Layers</Fragment>
          </Heading>
          <DummyText length="l" lang="ja" />
        </Flow>
        <ShapeDivider viewBox="0 0 10 10" h="var(--slopeH)" c="base" flip="X" z="1">
          <path d="M10,10V0L0,9.5V10L10,10z"></path>
        </ShapeDivider>
      </Stack>
    </Box>
  </PreviewArea>
  <PreviewCode>
    ```jsx "--slopeH"
    <Box max-sz="full" style={{ '--slopeH': 'clamp(2.5rem, 1.25rem + 5cqw, 5rem)' }}>
      <Stack isContainer max-sz="full" hasGutter pos="relative" min-h="50vh" c="white" g="15">
        <ShapeDivider viewBox="0 0 10 10" h="var(--slopeH)" c="base" flip="Y" z="1">
          <path d="M10,10V0L0,9.5V10L10,10z"></path>
        </ShapeDivider>
        <Frame isLayer>
          <Media isLayer src="https://cdn.lism-css.com/img/KRQPW50jXaLu.webp" alt="" width="960" height="640" />
          <Layer bgc="black:50%" />
        </Frame>
        <Flow isWrapper py="30" pe={['0', '25%', '33%']} my="auto" z="0">
          <Heading level="2" fw="bold" fz="xl">
            <Fragment>ShapeDivider & Layers</Fragment>
          </Heading>
          <p>ロレム・イプサムの座り雨。目まぐるしい文章の流れの中で、それは静かに歩く仮の言葉です。Elitも穏やかに続いていきますが、積み重ねられてきた「LiberroyとFoogの取り組み」は、余白のようなものです。作業が進むにつれて、工夫や考えとともに関心が折り重なりながらも、必要以上に主張せず彼らの作品は私たちに一定の示唆を与えてくれます。</p>
        </Flow>
        <Spacer h="var(--slopeH)" />
      </Stack>
      <Stack isContainer max-sz="full" hasGutter pos="relative" min-h="50vh" c="white" mbs="calc(0px - var(--slopeH))">
        <Frame
          isLayer
          style={{
            clipPath: 'polygon(0 var(--slopeH), 0 100%, 100% 100%, 100% 0)',
          }}
        >
          <Media isLayer src="https://cdn.lism-css.com/img/hwrIQ2ajud1n.webp" alt="" width="960" height="640" />
          <Layer bgc="black:50%" />
        </Frame>
        <Spacer h="var(--slopeH)" />
        <Flow isWrapper py="30" ps={['0', '25%', '33%']} my="auto" z="0" ta="right">
          <Heading level="2" fw="bold" fz="xl">
            <Fragment>ShapeDivider & Layers</Fragment>
          </Heading>
          <p>ロレム・イプサムの座り雨。目まぐるしい文章の流れの中で、それは静かに歩く仮の言葉です。Elitも穏やかに続いていきますが、積み重ねられてきた「LiberroyとFoogの取り組み」は、余白のようなものです。作業が進むにつれて、工夫や考えとともに関心が折り重なりながらも、必要以上に主張せず彼らの作品は私たちに一定の示唆を与えてくれます。</p>
        </Flow>
        <ShapeDivider viewBox="0 0 10 10" h="var(--slopeH)" c="base" flip="X" z="1">
          <path d="M10,10V0L0,9.5V10L10,10z"></path>
        </ShapeDivider>
      </Stack>
    </Box>
    ```
  </PreviewCode>
</Preview>





## 別ページで確認する
<PreviewFrame url="/demo/hero" />
