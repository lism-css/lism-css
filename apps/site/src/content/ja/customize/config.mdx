...
`main.css` のデフォルトのブレイクポイントは `xs: 0`（無効）/ `sm: 480px` / `md: 800px` / `lg: 1120px` / `xl: 0`（無効）です。値 `0` は「無効（CSS クエリを出力しない）」を表します。
...
:::info
`full.css` では、`xs` は `360px` で有効、`xl` は無効です。`isFullMode: true` で生成する `main.css` にも同じ既定値が適用されます。`breakpoints` を指定すると、その値が優先されます。コンポーネントで `xs` を使う場合は、上記の設定または `isFullMode: true` と統合プラグインによる型生成を利用してください。
:::

:::warning
`xs` / `xl` などを有効化すると `bp: 1` の全 Property Class に該当ブレイクポイントのブロックが追加されるぶん、CSS サイズが増えます。**[CSS Purge](/docs/customize/purge/) との併用が前提**の設計です。CSS Purge を使わない場合は、`bp` のリスト形式（`bp: ['sm', 'md']` など）で prop ごとに出力ブレイクポイントを絞ることを検討してください。
:::
...
なお、既存 prop への値追加（`ta="justify"` など）も、追加した値だけが `CustomPropValueRegistry` の拡張として型に反映され、補完候補に追加されます。既存 prop はもともと任意の文字列も受け付けるため、値を追加しなくても型エラーにはなりません。
