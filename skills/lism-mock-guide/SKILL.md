---
name: lism-mock-guide
description: 'デザインモックを@lism-css/mockのデータディレクトリ（pages/*.jsx|tsx・tokens.json・mock.config.json）として作成・編集する時の作業ガイド。lism-mock init→実装→lism-mock checkの自己検証ワークフローと、ページID・import許可リスト・トークン上書きのデータ契約に従う。マークアップの書き方自体はlism-css-guideを併用する。lism-mock / @lism-css/mock / デザインモックの作成・修正で使う。'
---

# Lism CSSデザインモック作成ガイド（@lism-css/mock）

デザインモックを`@lism-css/mock`の**データディレクトリ**（1画面=1ファイルの`pages/`＋小さな設定ファイル）として組むためのガイドです。プレビューアプリ（ビューア）はCLIに同梱されており、モック側にビルド設定や`package.json`は不要です。このスキルが扱うのは「モックとしてどんなファイルを置けるか（データ契約）」と「initからcheckまでのワークフロー」だけです。

> **前提（必須）: `lism-css-guide`を併用する。**
> このスキルは`lism-css-guide`が同じ階層に導入されていることを前提にします。マークアップ自体の書き方 — Primitive選定・トークン照合・Property Class・レスポンシブ設計 — はすべて`lism-css-guide`の実装フローに従ってください。無い場合は、`lism-cli skill add`（引数なしで全スキル一括導入）での追加をユーザーに案内してください。guideなしで、推測だけでモックを書かないでください。

> **バージョン情報:** このガイドは`@lism-css/mock@0.1.0`時点の情報に基づきます。
...
- **契約の正本**: `lism-mock init`が生成する`README.md`（契約説明書）。このスキルの要約と食い違う場合は生成物側を優先する

## コマンド

| コマンド | 用途 |
| --- | --- |
| `npx @lism-css/mock init [dir]` | ひな形＋契約説明書の生成（初手） |
| `npx @lism-css/mock check [dir]` | 非対話の検証。エージェントの自己確認はこれを使う |
| `npx @lism-css/mock dev [dir]` | devサーバー起動（常駐）。人間のブラウザ確認用 |

`[dir]`省略時はカレントディレクトリ。ローカルにインストール済みならbin名`lism-mock`でも実行できる。Node要件は`^20.19.0 || >=22.12.0`。

## ワークフロー（厳守）

1. **init**: データディレクトリがまだ無ければ、必ず`npx @lism-css/mock init <dir>`から始める。既にデータディレクトリがある場合はinitを実行せず、既存の生成物を読む
   - initは生成先に既存ファイルがあると無変更で非0終了する。`--force`は上書きオプションなので、ユーザーの明示的な指示なしに使わない
...
4. **自己検証**: `npx @lism-css/mock check <dir>`を実行する。**checkが非0で終了する間は、完成と報告してはいけない。** エラーには対象ファイルと原因が表示されるので、修正して再実行する
...
```
mock/
├── mock.config.json      # 必須 — schemaVersion＋表示メタデータ
...
- `useState`等のローカルUI状態とイベントハンドラは書いてよい。**API通信・認証・永続化・業務ロジックは書かない**（モックは画面の絵であり、アプリではない）
...
- 違反は`dev` / `check`とも非0終了する（警告にはならない）
...
bare importは許可リストのパッケージのみ。CLI側のnode_modulesで解決されるため、モック用にユーザープロジェクトへ依存を追加する必要はない（かつ、リスト外のパッケージは追加しても使えない）。
...
1. `mock.config.json` / `tokens.json`のスキーマ（`schemaVersion`含む）
2. import境界（上記の規則）
...
「check成功」は「モックが整形式でビルドできる」であり「画面が正しい」ではない。逆に、**checkが失敗しているモックを完成と報告することは禁止。**

## モック特有の注意

- **Astroプロジェクト向けのモックでもReact（`.jsx` / `.tsx`）で書く。** モックの目的は「デザインと、それを実装するクラス・コンポーネントが把握できること」であり、import文を実プロジェクトと厳密に揃える必要はない
- ビューアはダークモード切替を備える。色をトークンで書いていれば両モードの確認がそのままできる（直書き色を避けるのは`lism-css-guide`の通常ルール通り）
- ページはただのJSX（任意コード実行）でサンドボックスは無い。信頼できるモックだけを実行する

## このスキルファイル自身のアップデート方法

ユーザーがスキル更新を依頼した場合は、`lism-cli skill add`または`lism-cli skill update`を案内してください。最新を確認したい場合は、GitHubリポジトリの`skills/lism-mock-guide`を確認してください。
