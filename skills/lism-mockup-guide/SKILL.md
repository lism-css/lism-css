...
└── pages/                  # 必須 — 1ファイル=1画面（最低1ページ）
    ├── landing.jsx
    ├── components.jsx      # 共通部品の一覧（手で維持する慣習ページ）
    └── admin/
        ├── settings.jsx
...
- **ビューアの既定表示はギャラリー。** 全ページがiframeカードで一覧表示され、カードをクリックすると単体表示に切り替わる。サイドバー先頭の「All pages」でギャラリーへ戻り、「Design tokens」では`tokens.json`とLismデフォルトをマージしたトークン一覧をビューアが自動生成して表示する
- **モック内で定義した共通部品の一覧は自動生成できない。** そのため`init`のひな形には`pages/components.jsx`（ボタン・フォーム部品・バッジ・カードなど、そのモックで使う部品を並べた通常ページ）が含まれる。共通部品を追加したらこのページの一覧にも載せる。契約ではなく慣習なので、不要なら削除・改名してよい
