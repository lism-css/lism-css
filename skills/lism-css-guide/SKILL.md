...
0. **初期確認**: SKILL.mdだけで実装しない。実装対象に明らかに関係する最小限の詳細ファイルだけを先に開き、実装プランに「初期確認した資料」を列挙する。リンク表を眺めただけは確認済みにしない。
1. 目的別実装ガイドでPrimitive/コンポーネントの候補を選定する。
2. 実装前チェックを行い、初期確認した資料、使うPrimitive、コンポーネント、トークン、レスポンシブ方針を列挙した**実装プラン**を出す。未読のまま採用できない判断は🔁を付け、対応する「読む資料」を実装プランの判断行に紐づける。
3. 「資料確認トリガー」に従い、各操作をコードに書く手前で対応資料を読み、🔁を✅または⏸へ解消する。🔁のままコードへ反映しない。
4. ⏸が残る項目（px丸め・任意色・挙動変更・公開クラス変更など）は、その部分を実装する前にユーザー確認する。
5. 実装する。
6. 提出前セルフチェックで実装プランと実装を照合し、🔁の未解消・資料確認ログとの対応・差分・漏れを処理する。

C0–C8の詳細・出力形式・記入例は[`references/authoring.md`](./references/authoring.md)にまとめています。
...
- 構造は`<div>`+素のCSSよりPrimitiveを優先する。縦並びは`Stack`/`Flow`、横並びは`Cluster`/`Flex`、等幅列は`Columns`、メディア枠は`Frame`を先に検討する。
- `c--*`命名はBlockをcamelCase、Elementを`_`ひとつ、Modifierを`--`ふたつにする。`c--feature-card`や`__`は使わない。
- `c--*`のCSSは必ず`@layer lism-component`内に置く。
- トークン外のpx/rem/em値を勝手に丸めたり直書きしたりしない。丸め・新規トークン・直書き例外は⏸にする。
...
| 値照合付き | Figma/スクショ等のデザイン再現 | 通常+C5/C7を詳しく確認 | トークン差分表+CSS→Props移行表つき |
...
- `c--*`のBlockにハイフン、Elementに`__`が混ざっていないか。
- `c--*`のCSSは必ず`@layer lism-component`内にあるか。
...
| [`references/authoring.md`](./references/authoring.md) | 実装プランの作り方（C0–C8詳細・出力フォーマット・記入例） | 実装プランを作る/書式を確認 |
