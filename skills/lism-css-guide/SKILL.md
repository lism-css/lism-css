...
0. **実行レベル判定**: 変更規模から「事前チェック実行レベル」（不要/軽量/通常/値照合付き）を判定する。判定に迷う場合は一つ上のレベルを選ぶ。「不要」の場合、以降の手順1〜7は行わない（`.lism/`へのファイル作成もしない）。「軽量」の場合、手順7はチャット内の数行の確認に簡略化し、`.lism/review.md`は作らない。
