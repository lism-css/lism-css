...
Lism CSS が提供するクラス（`set--` / `is--` / `has--` / `l--` / `a--` / `u--` / Property Class）以外の、ユーザーが自分で定義するクラスは次の2分類で命名します。`b--` にできるのは [Block Class（`b--`）](#block-classb--)の3条件（サイト共通の繰り返し利用・ベーススタイルのCSS管理・ボタン/バッジ/カード級の粒度）をすべて満たす部品だけで、それ以外はすべて `c--` です。
...
「契約」を持つのは `b--` だけです。`b--` は「ベーススタイルが CSS 側にある」ことを名前が保証します。`c--` は契約のない自由地帯です。迷ったら `c--` で始め、3条件を満たす部品として CSS 管理へ切り替えたくなった時点で `b--` へリネーム昇格します。
