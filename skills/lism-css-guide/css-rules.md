...
<div class="c--myCard l--stack -g:20 -p:30 -bdrs:20 -bxsh:20 -bd">
  ...
</div>
```
...
カスタムCSS内でも、できる限り Lism のCSS変数（トークン）を使ってください。ただし、`padding`/`border-radius`/`font-size`/`color`などProperty Class/Propsへ移せる宣言は、CSSに書く前にマークアップ側へ移します。

```css
/* NG: Layer外・kebab-case・Propsへ移せる宣言をCSSへ直書き */
.c--my-card { padding: 24px; border-radius: 8px; }

/* OK: 意味クラスを残し、値はProps/Property Classへ移す */
/* <Stack class="c--myCard" p="30" bdrs="20">...</Stack> */
```
