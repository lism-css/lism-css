...
```
[customClass] [c--] [a--] [l--] [is--] [has--] [set--] [u--] [-]
```
...
| 5 | Trait Class 役割宣言（`is--`） | `is--wrapper`, `is--layer` |
| 6 | Trait Class 機能付与（`has--`） | `has--transition`, `has--gutter` |
| 7 | Set Class（`set--`） | `set--hov`, `set--shadow` |
| 8 | Utility Class（`u--`） | `u--cbox`, `u--trim` |
| 9 | Property Class（`-`） | `-p:20`, `-bgc:base-2` |
