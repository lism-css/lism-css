...
`--shc`（シャドウカラー: `hsl(220 4% 8% / 12%)`）と `--shsz--{N}`（シャドウサイズ）を組み合わせて構成される。要素ごとに `--shc` を上書きして再計算したい場合は、`set--var:bxsh` クラスを併用する。

| CSS変数 | 値 | 説明 |
|---------|-----|------|
| `--bxsh--10` | `var(--shsz--10) var(--shc)` | step 10 |
| `--bxsh--20` | `var(--shsz--20) var(--shc)` | step 20 |
| `--bxsh--30` | `var(--shsz--30) var(--shc)` | step 30 |
| `--bxsh--40` | `var(--shsz--40) var(--shc)` | step 40（最も濃い） |
