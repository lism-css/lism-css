...
Lism CSSでは、`line-height` は `var(--lh, calc(1em + var(--hl) * 2))` で算出される。既定は `--hl`（ハーフレディング）による fz 非依存の固定量管理で、`--lh`（unitless 比率）セット時はそちらが優先される。
...
`lh` は実 line-height（unitless 比率）を `--lh` で管理する独立トークン。比率は子要素へ継承され、各要素の font-size で再計算される。fz が画面サイズで大きく変わるグラフィカルなタイポグラフィで、余白感を比率のまま維持したい場合に使う。

| CSS変数 | 値 | 説明 |
| --- | --- | --- |
| `--lh--xs` | `1.25` | 基準 fz（16px）での `--hl--xs` 相当 |
| `--lh--s` | `1.5` | 基準 fz での `--hl--s` 相当 |
| `--lh--m` | `1.75` | 基準 fz での `--hl--base` 相当（デフォルト本文の行送り） |
| `--lh--l` | `2` | 基準 fz での `--hl--l` 相当 |
| `--lh--xl` | `2.25` | 基準 fz での `--hl--xl` 相当 |
