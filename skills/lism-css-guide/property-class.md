...
**`max-sz` の特殊クラス:**
- `-max-sz:full` — `max-inline-size: 100%`。`.has--gutter` 内では gutter 分を含めた全幅に拡張
...
ホバーエフェクト用のクラスです。Property Class 合成系 (`-hov:-{prop}`) と `set--var:hov` 連動系 (`-hov:in:*`) の 2 種類があります。

**`-hov:-{prop}` — ホバー時のプロパティ変更（Property Class 合成）**

`:hover` 擬似クラスで直接動作します（`set--var:hov` は不要）。`@media (any-hover: hover)` 内で定義され、タッチデバイスでは無効になります。
...
| `-hov:underline` | テキストに下線を表示 | `text-decoration: underline` |
| `-hov:neutral` | ニュートラルグレーを混合 | `color-mix(...)` |

`<Lism>` 継承コンポーネントでは、`hov` prop に文字列またはオブジェクトを渡すことで `-hov` クラス・`--hov-{prop}` 変数を出力できます。

| 指定 | 出力 | 用途 |
|------|------|------|
| `hov="-c"` / `hov="shadowUp"` | `-hov:-c` / `-hov:shadowUp` | 文字列はそのままクラス化（自動変換なし） |
| `hov="-c,-bxsh"` | `-hov:-c` + `-hov:-bxsh` | カンマ区切りで複数クラス指定 |
| `hov={{ c: 'red' }}` | `-hov:-c` + `--hov-c: var(--red)` | `-hov:-{prop}` に任意の値を割り当てる |
| `hov={{ shadowUp: true }}` | `-hov:shadowUp`（クラスのみ） | 値を省略してクラスのみ出力 |
| `hov={{ c: 'red', shadowUp: true }}` | `-hov:-c` + `--hov-c:…` + `-hov:shadowUp` | 値指定プロップとプリセット／自作クラスの併用 |

```jsx
// JSX: ホバーで色変更（set--var:hov なしで動作する）
<Link hov="-c">リンク</Link>
// → <a class="-hov:-c" href="...">リンク</a>

// カスタム値を指定
<Box hov={{ bgc: 'brand' }}>...</Box>
// → <div class="l--box -hov:-bgc" style="--hov-bgc: var(--brand)">...</div>

// 任意値指定とプリセット／自作クラスを同時出力
<Box hasTransition hov={{ bgc: 'brand', shadowUp: true }}>...</Box>
// → <div class="l--box has--transition -hov:-bgc -hov:shadowUp" style="--hov-bgc: var(--brand)">...</div>
```

**`-hov:in:*` — `set--var:hov` 連動のトランジションクラス**

`set--var:hov` がセットする `--_isHov` / `--_notHov` 変数を利用して動作します。`set--var:hov` と `has--transition` の併用が必要です。

| クラス | 効果 | 仕組み |
|--------|------|--------|
| `-hov:in:hide` | ホバー時にフェードアウト | `opacity: var(--_isHov, 0)` |
| `-hov:in:show` | ホバー時にフェードイン | `opacity: var(--_notHov, 0)` |
| `-hov:in:zoom` | ホバー時にズーム | `scale: var(--_isHov, 1.1)` |

```jsx
// set--var:hov + has--transition が必要
<Box set="var:hov" hasTransition>
  <Box hov="in:show">ホバーで表示</Box>
</Box>
```
