...
| 変数のセット | CSS 変数のセットアップのみを行う | `set--var:sh` / `set--var:hov` / `set--var:bdrsInner` |

`set--var:{name}` の `{name}` は、そのクラスがセットアップする CSS 変数の名称に対応します（例: `--sh--*` → `var:sh`、`--_isHov` / `--_notHov` → `var:hov`、`--bdrs--inner` → `var:bdrsInner`）。
