...
コンテンツ幅（`--contentSize`）の外側にはみ出させたい要素を配置するための `--bleed` 変数を提供します。
最外側の `is--container` を基準にしたコンテナクエリ単位（`100cqi`）で、コンテンツ幅の左右に残る余白を計算します。`--gutter-size` を下限とし、これより狭くなる場合はガター幅を返します。

`--contentSize` が未指定の場合は `--sz--m` を基準値として使用します。

使用例:
```html
<div class="is--container has--gutter">
  <div class="l--stack">
    <div class="set--bleed -contentSize:l">
      <!-- --bleed が定義された領域。`padding-inline: var(--bleed)` で wrapper の外側ガターまで広げられる -->
      <div style="padding-inline: var(--bleed)">...</div>
    </div>
  </div>
</div>
```
...
<Container hasGutter>
  <Stack>
    <Lism set="bleed" contentSize="l">
      <Lism ps="var(--bleed)">...</Lism>
    </Lism>
  </Stack>
</Container>
```
