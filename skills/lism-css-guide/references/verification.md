...
提出前チェックの実行を実装担当の本人から分離し、実装プランと検収結果を観測可能なファイルとして残すための規約です。実装フロー・チェック項目の正本は`SKILL.md`（実装フロー・提出前セルフチェック）、実装プランの中身と書式は[`authoring.md`](./authoring.md)を参照してください。
...
| `.lism/review.md` | 評価サブエージェント（または本人直列実行）の検収報告 | 提出前セルフチェック時 |
...
- OSのtempディレクトリは使わない（環境ごとにパスが変わり、ユーザーから観測できない）。
...
提出前セルフチェックを実装した本人だけで行うと、「正確に再現する」等のゴールへの執着や自分のプランへの一貫性バイアスから、例外の自己承認を見逃しやすくなります。サブエージェント／タスク委任機能が使える環境では、照合を読み取り専用の評価サブエージェントに委任します。使えない環境では、同じ照合を本人が`SKILL.md`「提出前セルフチェック」の順に直列実行します。

運用ルール:

- 評価サブエージェントは**実装プランではなくルールと照合する**。基準は`SKILL.md`の「最小ゲート」「判定記号」、`antipatterns.md`・`antipatterns-layout.md`、`property-class.md`。プランに焼き込まれた逸脱も、コードがルール違反であれば検出できる。
...
- **修正→再評価ループを終了条件にする**: 違反ゼロの評価報告が出るまで提出しない。違反を修正したら再評価を依頼する。
- 評価報告は`.lism/review.md`へ保存し、完了報告から必ず参照する（評価を実施したことをユーザーが観測できるようにする）。

### 評価サブエージェントへの指示テンプレ

```md
あなたはLism CSS実装の検収者です。実装の意図や経緯は考慮せず、ルール照合だけを行ってください。
...
### 補助検出器（コマンドが実行できる環境のみ）

評価時に`grep`等が使える場合は、次の検出を照合の入口に使ってよい（最終判定は資料との照合で行う）:
