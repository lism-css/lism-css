...
- **参照先**: `naming.md`、`css-rules.md#component-classc--`、`css-rules.md#独自プレフィックス`、`antipatterns.md#クラス名の命名ミス`。
...
- **照合・判定**: `tokens.md`に完全一致→✅。typo・既存規約上明確な置換→🔁（`tokens.md`で正規トークンを確認して置換し✅へ）。近似丸め・任意色・ブランド色・再現度の判断→⏸。`antipatterns.md`の「直書きしてよい例外」に該当→`✅例外`。
