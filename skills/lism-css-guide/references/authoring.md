...
判定記号（✅確定 / ⏸要ユーザー確認 / 🔁資料確認トリガー未通過 / 🆕新規定義 / ⬜例外）の定義は`SKILL.md`の「判定記号」を参照してください。🔁は「資料確認トリガー」の未通過項目で、対応操作をコードに書く手前で指定資料を読み✅または⏸へ解消します。
...
`SKILL.md`の「資料確認トリガー」に該当する操作をコードに書く手前で、対応資料をまだ読んでいなければ、その判断は🔁（未通過）にする。対応資料を読んで✅へ解消するか、判断できなければ⏸にする。🔁のままコードへ反映しない。

読んだ資料は「初期確認」「実装中」を区別して資料確認ログに記録する。提出前に、🔁が✅または⏸へ解消されたかを照合する。

## C0–C8 実装前チェック詳細

各Cは「列挙/照合・判定/決め方/参照先/よい例・避けたい例/事前に潰す失敗/参照するガイド」の観点で判断します。
...
- **参照先**: `primitive-class.md`、`primitives/l--*.md`、`trait-class.md`、`components-core.md`、`components-ui.md`、`antipatterns.md#レイアウト選択ミス`。
...
- **照合・判定**: 3+反復見込みかつ同じ意味のUI部品として安定→🆕コンポーネント化。1〜2回・局所・一時的重複→✅。Props設計が必要→⏸。
- **決め方**: CSSの`c--`新設でなくReact/Astroコンポーネント抽出を優先。`className`/`style`/`data-*`/ARIA/イベントハンドラの受け渡しを最初から設計する。
- **参照先**: `components-core.md`、`components-ui.md`、`css-rules.md#component-classc--`、`property-class.md`。
...
- **照合・判定**: プレフィックス後の名前が規約に合う→✅。ハイフンや`__`がある→🔁。サイト領域→🆕`z--`。ページ固有→🆕`p--`。公開API・CMS・外部JS・E2E依存→⏸。`.c--*` CSSがLayer外になる→🔁。
- **決め方**: `naming.md`に従う。再利用UIでない領域は`z--`、ページ固有は`p--`。`c--*` CSSを書く場合は必ず`@layer lism-component {}`内に置く。
- **参照先**: `naming.md`、`css-rules.md#component-classc--`、`css-rules.md#独自プレフィックス`、`antipatterns.md#クラス名の命名ミスkebab-case`。
- **よい例・避けたい例**: OK=`c--featureCard`、`c--featureCard_body`、既存命名がアンダースコア寄せなら`c--feature_card`。罠=`c--feature-card`、`c--hero__inner`、`c--feature-card__body`、サイトヘッダを`c--`にする。
- **よくあるNG例**: `c--feature-card`→`c--featureCard`、`c--hero__inner`→`c--hero_inner`、`c--myCard__body`→`c--myCard_body`。Block名のハイフン禁止、Element区切りは`_`ひとつ、Modifierは`--`ふたつ。
- **事前に潰す失敗**: kebab-case命名、BEM風`__`、`@layer lism-component`囲み忘れ、何でも`c--`、ゾーニングを`c--`にする。
- **参照するガイド**: `antipatterns.md`と`css-rules.md`。

### C4: 状態・バリエーション設計

- **列挙**: active/open/disabled/currentなどの状態、solid/outlineなどの見た目違い。
- **照合・判定**: 状態→🆕`data-*`/ARIA。見た目違い→🆕`c--name--variant`。正規Traitが当たる→✅。
...
- **参照するガイド**: `antipatterns.md`。

### C5: 値・トークン照合

- **列挙**: spacing/color/fz/radius/shadow/size/lh/ltsなど、使う予定の値をすべて列挙。
- **照合・判定**: `tokens.md`に完全一致→✅。typo・既存規約上明確な置換→🔁。近似丸め・任意色・ブランド色・再現度の判断→⏸。1px罫線・transform微調整・@media閾値→⬜。
- **決め方**: トークン値を流用。⏸はトークン候補と差分表を出して確認まで書かない。CSSへ書く前に、Lism Props/Property Classで表せる値かも同時に確認する。
...
- **参照するガイド**: `tokens.md`と「デザインデータ取り込みフロー」。
...
- **参照するガイド**: `property-class.md`と`tokens.md`。
...
- **事前に潰す失敗**: 既定値の重複指定。
- **参照するガイド**: 各`primitives/l--*.md`の既定CSS概要。

## 出力フォーマット

実装前チェックの成果物を実装プランと呼びます。各行に必ず判定記号（定義は`SKILL.md`参照）を付けます。🔁は対応操作をコードに書く手前で指定資料を読み✅または⏸へ解消します。⏸が残る項目はその部分を実装しないで先にユーザー確認します。
