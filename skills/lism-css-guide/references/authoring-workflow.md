...
Lism CSSで新規UI・セクション・コンポーネントを書く前に使う事前チェック詳細です。各Cは「列挙/照合・判定/決め方/参照先/よい例・避けたい例/事前に潰す失敗/参照するガイド」の7つの観点で判断します。

判定は新規実装用を使います: ✅確定 / ⏸要ユーザー確認 / 🔁参照して確定 / 🆕新規定義 / ⬜例外。
...
- **照合・判定**: 標準Primitiveで組める→✅。カラム系など迷う→🔁。`@lism-css/ui`で済む→✅または🔁。
...
- **よい例・避けたい例**: OK=`<Columns cols={3}>`、`<Frame ar="16/9">`。罠=`<div -d:grid gtc=...>`、手組みメディア枠、最外殻Wrapper。
...
- **照合・判定**: プレフィックスの後ろがcamelCaseで規約に合う→✅。サイト領域→🆕`z--`。ページ固有→🆕`p--`。公開API・CMS・外部JS・E2E依存→⏸。
...
- **よい例・避けたい例**: OK=`c--featureCard`、`c--featureCard_body`。罠=`c--feature-card__body`、サイトヘッダを`c--`にする。
...
- **決め方**: トークン値を流用。⏸はトークン候補と差分表を出して確認まで書かない。
...
- **参照するガイド**: `tokens.md`とデザインデータ取り込みフロー。
...
- **決め方**: baseの値を必ず置く。標準BPへ寄せる。同じ値の繰り返しは省略する。
- **参照先**: `responsive.md`、`trait-class/is--container.md`、`property-class.md`、`primitive-class.md#カラムレイアウト-primitive-の使い分けガイド`。
- **よい例・避けたい例**: OK=`p={{ base: '20', md: '30' }}`+祖先`isContainer`。罠=`gtc="repeat(3,1fr)"`固定、base抜け、container祖先なし。
...
- **照合・判定**: 1つの要素にだけ効く見た目の指定→✅マークアップへ。擬似クラス・擬似要素・状態切替・子孫セレクタ→✅CSSへ残す。
