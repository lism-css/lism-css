...
- [Element に `__` を使う](#element-に-__-を使う)
...
Lism CSSの`c--*` Element区切りは`_`ひとつ。BEMでよくある`__`は使わない。

| NG | OK |
|---|---|
| `c--hero__inner` | `c--hero_inner` |
| `c--featureCard__title` | `c--featureCard_title` |
...
| `<div class="c--featureGrid">`+`@container`で3列 | `<Columns cols={[1, null, 3]}>` | 等幅列とBP切替は`Columns`に任せる |
| `<div class="c--thumb">`+`aspect-ratio` | `<Frame ar="16/9">` | メディア枠は`Frame`に任せる |
