...
| `is--fullwide` | 親要素のサイズいっぱいに広げる。`has--gutter` 直下では gutter 分を加算して**その余白を突き抜ける**（旧 `-max-sz:full` の置き換え） |
| `is--bleed` | 直近の `is--container` 幅まで広げて中央配置する。`is--wrapper` の制約を越えて広がる（旧 `-max-sz:container` の置き換え） |
...
Lism コンポーネントでは `isContainer`, `isFullwide`, `isBleed`, `isLayer` 等の Props として利用できます（例: `<Lism isContainer>`）。

詳細は以下の個別ドキュメントを参照してください:
- [is--container](./trait-class/is--container.md)
- [is--wrapper](./trait-class/is--wrapper.md)
- [is--fullwide](./trait-class/is--fullwide.md)
- [is--bleed](./trait-class/is--bleed.md)
