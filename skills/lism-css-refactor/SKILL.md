...
2. **Pass1を実行**。class・Props・CSS宣言・style・@media・重複している束を1行ずつ洗い出し、洗い出し表を作る。
3. **Pass2–9を実行**。洗い出し表の各行に判定を付ける。判断に迷う行だけ`lism-css-guide`を参照する。
4. **修正案の差分を作る**。この時点ではまだユーザーに提示しない。
5. **Pass10を実行**。修正案を他人のPRとして見直し、提示前に元の見た目や動きを壊していないか確認する。
6. **修正案を提示する**。見直し後の修正案をdiff形式＋根拠つきで出す。⏸の項目はここでユーザーに確認し、勝手に確定しない。
7. **適用する**。ユーザーが実装を許可している場合だけ適用する。見た目・挙動を変えないことと、差分を小さくしたことを明記する。
8. **検証して報告する**。テスト/ビルド/型チェックを実行し、残ったリスクを報告する。

成果物の書式は[`references/output-format.md`](./references/output-format.md)、before→afterの実例は[`examples/`](./examples/markup.md)を参照してください。
