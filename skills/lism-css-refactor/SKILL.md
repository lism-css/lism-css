...
このスキルはguide（`lism-css-guide`）が同階層に導入済みであることを前提に、`../lism-css-guide/*`へrouteします。guideが未導入の場合は次のいずれかで最低限の判断材料を得てください。

- `lism skill add`でguideの導入を促す。
- MCPの`get_*`ツール（`get_tokens`/`get_guide`/`get_component`等）、または公式docsの`.md`URL（`https://lism-css.com/docs/overview.md`等）を参照する。
...
CLIが複数skill配布に対応後は`lism skill add`で更新できます。それまで、または最新を確認したい場合は、GitHubリポジトリの`skills/lism-css-refactor`を参照してください。
