...
- `@astrojs/markdown-satteri` / `satteri` — Sätteri, Astro 7's default Markdown processor, and its plugin API
- `github-slugger` — heading ID generation
- `pagefind` / `@pagefind/default-ui` — full-text search

In `astro.config.mjs`, `expressiveCode` is placed before `mdx`, and `markdown.processor` (`satteri()` from `@astrojs/markdown-satteri`) enables `features.directive: true` (parsing the `:::` syntax) and registers four custom transform plugins (converting labeled blocks into Callout and unlabeled ones into Alert, turning URL/`[[slug]]` paragraphs into link cards, turning inline `[[slug]]` into WikiLinks, and adding IDs to headings and anchor links to h2/h3). MDX is processed by the same `markdown.processor`. The sitemap integration also fills in each post's `lastmod`.
...
├── lib/             # TOC generation, year/month archives, OG images, sitemap lastmod, Sätteri plugins (`satteri/`), etc.
├── pages/           # Routing
...
`src/lib/satteri/directive.mjs` (an MDAST plugin) transforms the `:::` syntax so you can call `@lism-css/ui`'s `Callout` / `Alert`. Unsupported types and leaf directives like `::name` cause a build error (Sätteri silently drops unhandled directives, so this makes typos visible). Inline `:name` in body text, such as `1:2`, is left as plain text.
...
> Sätteri's directive parsing itself also works with `.md`, but because this plugin converts `:::` into JSX nodes (Astro components), it doesn't work in the `.md` render pipeline. `content.config.ts` is also scoped to `**/*.mdx`, so `.md` files aren't collected.
