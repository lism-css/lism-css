...
Post files are written in `.mdx`. Directive syntax like `:::note` is converted into a `<Callout>` by a Sätteri plugin. The available types are `alert` / `point` / `tip` / `warning` / `check` / `help` / `note` / `info`.
:::
