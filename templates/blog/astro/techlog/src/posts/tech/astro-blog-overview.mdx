...
`templates/blog/astro/techlog/`は、Lism CSSと`@lism-css/ui`を使った技術ブログ向けのAstroテンプレート。コードハイライト・記事内TOC・カテゴリ／タグ・年月アーカイブ・Pagefind全文検索・ダークモード・sitemap/robots.txtを揃えてある。

## 依存

AstroとLism CSS/`@lism-css/ui`に加え、以下を入れている。

- `@astrojs/mdx` — 記事は `.mdx` で書く
- `@astrojs/sitemap` — sitemap生成
- `astro-expressive-code` — コードハイライト
- `@astrojs/markdown-satteri` / `satteri` — Astro 7 既定の Markdown プロセッサ Sätteri と、そのプラグイン API
- `github-slugger` — 見出し ID の生成
- `pagefind` / `@pagefind/default-ui` — 全文検索

`astro.config.mjs`で`expressiveCode`を`mdx`より前に並べ、`markdown.processor`（`@astrojs/markdown-satteri`の`satteri()`）で`features.directive: true`（`:::`記法の解析）を有効にし、自前の変換プラグイン4つ（ラベルありをCallout・なしをAlertへ変換、URL/`[[slug]]`段落のリンクカード化、文中`[[slug]]`のWikiLink化、見出しへのID付与とh2/h3へのアンカーリンク追加）を登録している。MDXも同じ`markdown.processor`で処理される。さらにsitemap integrationで、記事ごとの`lastmod`も補完する。
...
├── lib/             # TOC生成・年月アーカイブ・OG画像・sitemapのlastmod・Sätteriプラグイン（`satteri/`）等
├── pages/           # ルーティング
...
`@astrojs/sitemap`でビルド時にサイトマップを生成する。`astro.config.mjs`の`site`はsitemap/robots.txtの基準になるので、公開前にデプロイ先ドメインへ書き換える。techlogは`src/posts/{category}/{slug}.mdx`に置いた記事を`/posts/{slug}/`で配信する都合上、sitemap integrationに`stripFirstSegment: true`を渡してカテゴリディレクトリを除いている。
...
`:::` 記法で `@lism-css/ui` の `Callout` / `Alert` を呼べるよう、`src/lib/satteri/directive.mjs`（MDAST プラグイン）で変換している。対応外の type や `::name` のような leaf directive はビルドエラーになる（Sätteri は未処理の directive を黙って捨てるため、typo に気付けるようにしている）。本文中の `1:2` のような `:name` はそのままテキストとして残る。
...
> Sätteri の directive 解析自体は `.md` でも動くが、このプラグインは `:::` を JSX ノード（Astro コンポーネント）に変換するため `.md` のレンダーパイプラインでは機能しない。`content.config.ts` も `**/*.mdx` に絞ってあるので `.md` は収集対象外。
...
```astro
<Flow class="c--content" data-pagefind-body>
...
- `PageLayout.astro` — `Layout` を基盤に、ページタイトル＋`<Flow as="article" class="c--content">` 本文をまとめる固定ページ用レイアウト
