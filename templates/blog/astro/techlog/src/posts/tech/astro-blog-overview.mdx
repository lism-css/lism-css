...
`astro.config.mjs`で`expressiveCode`を`mdx`より前に並べ、`markdown.remarkPlugins`に自前の変換プラグイン3つ（`remark-directive`の`:::`変換、URL/`[[slug]]`段落のリンクカード化、文中`[[slug]]`のWikiLink化）を登録している。さらにsitemap integrationで、記事ごとの`lastmod`も補完する。
