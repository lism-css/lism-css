...
:::note[このテンプレートは MDX + directive に対応]
記事ファイルは `.md` / `.mdx` の両方に対応しています。`:::note` のような directive 記法は Sätteri プラグインで `<Callout>` に変換されます。利用可能な type は `alert` / `point` / `tip` / `warning` / `check` / `help` / `note` / `info`。
:::
