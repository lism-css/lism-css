...
Lism CSS styles are split across several `@layer`s, and the order in which the layers are declared determines their priority. They are stacked in the order `lism-base` → `lism-block` → `lism-trait` → `lism-primitive` → `lism-custom` → `lism-utility`, and property classes sit outside the layers, so you can override styles with property classes without worrying about specificity.
...
There are also `has--{name}` classes that add a capability, such as `has--transition` and `has--gutter`.

## Your own classes (b-- / c--)

Classes you define in a project fall into two kinds: `b--{name}` (Block Class) and `c--{name}` (Custom Class). Use `b--` for basic parts reused across the site, such as buttons and badges, and write their base styles in `@layer lism-block`. Everything else — elements and components — uses `c--`.

For `c--`, move any declaration you can express with a property class into the markup, and leave only CSS-only declarations — pseudo-elements, state changes, descendant selectors, and the like — in `@layer lism-custom`. Simple hover changes such as a background color can be written with `-hov:*` classes.

```html
<span class="c--tag has--transition -fz:xs -px:10 -py:5 -bgc:base-2 -bdrs:10 -hov:-bgc" style="--hov-bgc: var(--divider)">React</span>
```

```css
@layer lism-custom {
  .c--tag[data-is-active] {
