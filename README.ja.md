...
- **CSS レイヤー構造** — `@layer`（lism-base → lism-primitive → lism-trait → lism-component → lism-custom → lism-utility）を使用した明確な詳細度管理。`lism-primitive` の内部は `layout` / `atomic` のサブレイヤーに分かれています。`lism-trait` は `is--` / `has--` の Trait クラス用レイヤー、`lism-component` は BEM 構造の `c--` コンポーネント用レイヤー、`lism-custom` はユーザー独自プレフィックスのクラス用レイヤーです。詳細度の衝突を最小限に抑えます。
