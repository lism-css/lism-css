...
- **CSS レイヤー構造** — `@layer`（lism-base → lism-primitives → lism-components → lism-custom → lism-utility）を使用した明確な詳細度管理。`lism-primitives` の内部は `trait` / `layout` / `atomic` のサブレイヤーに分かれています。`lism-components` は BEM 構造の `c--` コンポーネント用レイヤー、`lism-custom` はユーザー独自プレフィックスのクラス用レイヤーです。詳細度の衝突を最小限に抑えます。
