...
> 作成: 2026-06-22 / ステータス: guide 強化・refactor 新設・CLI 複数 skill 配布の実装完了（検証待ち。運用判断は §9-1 未決事項を参照）
...
> - **forward 系（前向き・§4-4a）**: ✅ 確定（新規定義・合意済み直書き例外は `✅新規`/`✅例外（…）` と行内注記）／🔁 資料確認トリガー未通過（資料を読んで ✅/⏸ へ解消）／⏸ 要確認。当初の 🆕（新規定義）/⬜（例外）は ✅ の行内注記へ統合済み
> - ⚠️ **記号衝突に注意**: ✅（触らない ↔ 確定）は系統で意味が異なる。⬜ は refactor 系のみで使う。両系統で共通なのは ⏸（要確認）のみ。衝突対策として、guide SKILL.md には forward 系の定義のみを載せ（refactor 系の表は置かない）、両スキルの SKILL.md がそれぞれ相手系統への注意書きを 1 行持つ。refactor 系の定義は refactor 側 SKILL.md を正本とする。
...
順序に意味がある（先に構造を決めないと、何に値を載せ・どこに状態を持たせ・何を命名するかが定まらない）。各パスは §5-3 と同型の 7 フィールド（**列挙 / 照合・verdict / 決め方 / route先 / OK・避ける罠 / 事前に潰す失敗 / guide依存**）。verdict: **✅ 確定 / 🔁 資料確認トリガー未通過 / ⏸ 要ユーザー確認**（実装済みの 3 記号。以下の FP 記述中に残る 🆕 は `✅新規`、⬜ は `✅例外（…）` の行内注記と読み替える）。**通常レベルでも該当しない FP は省略してよい**（核 = FP0/1/5/6 ＋ 該当 FP に絞り、形式埋めを避ける）。
...
3. **判定（3 段階）**:
   ...
   - **typo・既存規約上明確な置換**（`secondary`→`base-2` 等）→ tokens.md で確認のうえ正規トークンへ置換して ✅（🔁 のまま「自動置換」とは書かない。skill 側の 🔁 は「資料確認トリガー未通過」の意味に限定）
...
> **実装済みの集約**: SKILL.md では下記リストをそのまま並べず、「プロセス照合（🔁 解消・資料確認ログ対応）」＋「ルール照合（最小ゲート 6 項目の再確認 ＋ antipatterns.md TOC を開いて照合）」＋「個別確認（ui 置換・3 箇所以上のコンポーネント化・既存配置整合の 3 点のみ）」の 3 ブロックに畳んである。下記は設計時の全項目リスト（内容の正本は antipatterns.md 側）。
...
- **具体化（PR #459 では未着手）**: 個別クラス解説の `## Usage` を docs `.md` URL へ逃がす実作業と、削り幅（ゼロ／最小例／フル）の比較検証は [`plan-skill-guide-slimming.md`](./plan-skill-guide-slimming.md) / #460 で扱う。
...
- [ ] ★ SKILL.md 本体には「FP0–FP8 一覧＋実行レベル＋verdict 語彙＋§4-4b 接続」だけ置き、7 フィールド詳細・出力フォーマット表・記入見本は **guide 自身の `references/authoring.md`** へ逃がす（当初は `authoring-workflow.md`／`authoring-output-format.md`／`examples/authoring-plan.md` の 3 分割だったが、リンクの散らかり解消のため 1 ファイルに統合。資料確認トリガー表・verdict 語彙は SKILL.md を正本とし authoring.md では再掲しない）（§4-8 の 500 行維持・構成は §4-11）。**refactor 配下や本 plan には逃がさない**（plan は配布物でなく skill 実行時に参照不可／refactor へ置くと §5-6 の依存方向 refactor→guide が逆転）（P-forward）


### 4-11. guide オーバーホール後のディレクトリ構成

既存はフラット構成（`SKILL.md` ＋ `antipatterns.md` 等 ＋ `primitives/` `property-class/` `trait-class/` サブディレクトリ）。オーバーホールでは **既存ファイルを維持しつつ `references/` を追加**する（サブディレクトリ追加は既存パターンと整合）。当初は `references/` と `examples/` の 2 ディレクトリを新設したが、authoring 詳細を 1 ファイルへ統合した結果 `examples/` は不要になり廃止。

```
skills/lism-css-guide/
  SKILL.md                       # 索引（FP0–FP8 一覧／実行レベル／verdict 語彙〈forward のみ・refactor 系は注意書き 1 行〉／§4-4b 接続）。500 行以下を維持
  ...
  references/                    # ★新設: SKILL.md から逃がす authoring 詳細
    authoring.md                 #   FP0–FP8 の 7 フィールド詳細＋出力フォーマット表＋記入見本を一本化
```

> **MCP 注記**: `get_guide` は `GUIDE_TOPICS`（固定トピック→ファイル map）で配信し、現状 `naming.md`/`trait-class.md` 等も非公開。新設 `references/` は **MCP 公開したい場合のみ `packages/mcp/src/tools/get-guide.ts` の `GUIDE_TOPICS` に追加**する（しなくても既存の非公開ファイルと同じ扱いで動作に支障なし）。`get_overview` は `SKILL.md` を読むため、SKILL.md 刷新は MCP 出力に反映される。
...
4. **CLI 複数 skill 配布の一般化**（付録 A。既定挙動＝全スキル一括導入。§9-1 U3 確定）。物理 rename ＋ alias/移行は本オーバーホールのスコープ外（将来・§7）。
...
| 9 | examples 配置 | refactor `examples/` に独立（before→after は嵩張るため）。guide 側の記入見本は `references/authoring.md` に集約（`examples/` は廃止） | ✅ |
...
| 17 | forward verdict 語彙 | ✅ 確定 / 🔁 資料確認トリガー未通過 / ⏸ 要確認 の 3 記号（🆕/⬜ は ✅ の行内注記へ統合。`✅例外` にできるのは antipatterns「直書きしてよい例外」該当時のみ） | ✅ |
...
| U3 | CLI 複数 skill 配布のデフォルト挙動（付録 A） | ✅ **確定: 全スキルを一括導入**。引数なしの `lism skill add`/`update` は guide＋refactor を両方導入し、`check` も全レジストリスキルを対象にする。`lism skill add [name]` での個別指定も可。既存ユーザーの `update` 時に refactor が新規追加される点は一括導入方針として許容 |
...
5. ✅ **guide オーバーホール実装 完了**（SKILL.md 索引化、`references/authoring.md` 新設〈詳細・出力フォーマット・記入見本を一本化〉、antipatterns 増強、primitive 既定 CSS 概要、リンク補完）。
6. ✅ **`lism-css-refactor` 実装 完了**（SKILL.md、順序パス、退行集、出力フォーマット、examples）。
7. ✅ **CLI 複数 skill 配布の一般化 実装 完了**（付録 A の対象ファイルを更新）。
8. **次アクション**: 検証（`nr build` / `nr lint` / `nr typecheck` / `nr test` / `nr --filter @lism-css/mcp test`）と guide / refactor のリンク切れ再確認を行う。
...
- `commands/skill/add.ts` / `check.ts` / `update.ts`: スキルを loop（`lism skill add [name]` で個別指定可。**既定挙動＝全スキル一括導入（§9-1 U3 確定）**: 引数なしの `add`/`update` は guide＋refactor を両方導入し、`check` も全レジストリスキルを対象に確認する）
