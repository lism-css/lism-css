...
> 作成: 2026-06-22 / ステータス: 設計確定・実装未着手
...
| **verdict（判定）** | 棚卸しの各行に付ける処理判定（✅ 触らない／🔧 修正／⏸ 要確認／⬜ 残す） |
...
| 知識 | 共有 spine を保持 | 複製せず guide のローカル file へ route |
...
**(A) 実装フロー（厳守・forward）** — refactor の 0〜7（§5-2）と対称：
...
2. 実装する
3. 提出前セルフチェック（§4-4b）で Authoring Plan と実装を照合
4. ⏸（px 丸め等）はユーザー確認。勝手に確定しない
```
...
| 通常 | 新規 UI / コンポーネント / セクション | FP0〜FP8（FP2 は該当時のみ） | ドメイン別表（§出力フォーマット） |
| 値照合付き | Figma/スクショ等のデザイン再現 | 通常＋FP5 を厳密化 | 上記＋token 差分表（デザインデータ取り込みフロー） |

##### preflight パス（FP0–FP8）

順序に意味がある（先に構造を決めないと、何に値を載せ・どこに状態を持たせ・何を命名するかが定まらない）。各パスは §5-3 と同型の 7 フィールド（**列挙 / 照合・verdict / 決め方 / route先 / OK・避ける罠 / 事前に潰す失敗 / guide依存**）。verdict: **✅ 確定 / ⏸ 要ユーザー確認 / 🔁 参照して確定 / 🆕 新規定義 / ⬜ 例外（直書き許容）**。
...
- **照合・verdict**: 3+ 反復見込み→🆕 component 化して着手 ／ 1〜2 回・局所→✅ そのまま ／ 値差分・slot 要→⏸（Props 設計先行）
...
- **照合・verdict**: tokens.md にある→✅ ／ typo・存在しない値→🔁 正規トークンへ ／ px/rem/em 直書き・任意色推測・デザイン値置換→⏸（取り込みフローへ）／ 1px 罫線・transform 微調整・@media 閾値→⬜
...
- **列挙**: 各値の base 値・切替 BP（sm/md/lg）・レスポンシブ値を載せる要素の祖先 isContainer の有無・固定 Grid 化していないか
- **照合・verdict**: base 値あり・標準 BP・祖先 isContainer あり→✅ ／ base 抜け・xs 誤用→🔁 ／ 固定列をレスポンシブ化（`cols={[1,2,3]}`）→⏸（挙動変更）
...
- **事前に潰す失敗**: レスポンシブ抜け・固定 Grid・isContainer 祖先なし
...
- 物理名・`name` は `lism-css-guide` のまま。`description` だけ「実装時に起動されやすい coding 寄り」に変更する。
  - 例: `Lism CSS で UI やページを実装する時に使う実装ガイド。Primitive 選定・token 照合・Property Class/Lism Props 活用・レスポンシブ設計・アンチパターンセルフチェックを行う。`
- 将来の物理 rename は §7。
...
- [ ] ★ SKILL.md 本体には「FP0–FP8 一覧＋実行レベル＋verdict 語彙＋§4-4b 接続」だけ置き、7 フィールド詳細・出力フォーマット表はこの plan §4-4a／refactor `examples/` に逃がす（§4-8 の 500 行維持）（P-forward）


## 5. スキル B: `lism-css-refactor` 仕様

役割 = 既存コードの棚卸し・**挙動不変**リファクタ。ユーザーが明示起動する。知識は持たず、guide へ route する。
...
4. 修正案を diff 形式＋ルール出典つきで提示
   └ ⏸（px 丸め・色推測など要確認）はここでユーザー確認。勝手に確定しない【必須チェックポイント】
5. Self-review ゲート（common-mistakes.md で退行を再照合・§5-5）
6. ユーザーが実装を許可している場合だけ適用（挙動不変・最小 diff を明記）
7. 検証（テスト/ビルド/型チェック）と残リスクを報告
```
...
- **ただし forward 固有手順は別**: デザイン → Primitive/token/responsive 方針の列挙 preflight（§4-4a）は refactor パスから導けない。guide の authoring workflow は別途設計する（§10-2）。
...
> 開発中、refactor スキルはリポジトリの `skills/lism-css-refactor/` に置く。各ツールへの配布は §フェーズ 4（CLI 一般化）まで手動。それまでは `.claude/skills/` 等へ手動コピーで動作確認する。
...
| 10 | 適用前の対話チェックポイント（⏸ px 等） | 必須（§5-2 step 4） | ✅ |
...
| 14 | `check-ui-props` スキルとの接続 | refactor の重複/抽出パスと観点が重なる。重複させず参照で繋ぐ | 🟡 refactor 設計時 |
...
3. **次アクション**: §8 フェーズ 2「guide オーバーホール」へ着手（§4 全体を実 SKILL.md／参照 .md に反映）。
...
- `commands/skill/add.ts` / `check.ts` / `update.ts`: スキルを loop（既定=全スキル、`lism skill add [name]` で個別指定可）
