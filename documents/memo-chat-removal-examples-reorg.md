...
> 作成日: 2026-08-14 / 最終更新: 2026-08-15（issue化） / #550のChat移行作業（`34af1576`）中の議論から。**未着手。issue #557 で対応する**。まずは既存lism-ui全体のb--移行（#550）を完走させる。

## 現時点のステータス

- Chatは#550でb--移行済み（`34af1576`）のまま`@lism-css/ui`に**存続中**。`variant="think"`も標準スタイル化済み。
- ここに書いた再編はすべて未実施。**issue #557 として作成済み**（このメモが実行時の判断材料）。
