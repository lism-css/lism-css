...
| Tabs | **a11y改修（issue #87）実施済み・コミット前 / Props→CSS移行の要否確認は未着手** | issue #87対応: パネルの`aria-hidden`を`hidden`属性へ移行、タブへ`type="button"`・`id="{tabId}-{index}-tab"`・roving tabindexを追加、パネルへ`aria-labelledby`・`tabindex="0"`を追加。キーボード操作（自動アクティベーション）を`tabsKeyNav.ts`（共有純関数）で実装。**APGに合わせ既定（水平）は左右キーのみ**とし、上下キーはブラウザのスクロールに残す。tablistの`aria-orientation="vertical"`（`listProps`経由で指定）を検知した場合のみ上下キーへ切り替える。Home/Endは向きに関わらず有効。`aria-orientation`は属性のためレスポンシブに出し分けられず、縦横が切り替わるレイアウトは既定（水平・左右）のまま使う方針をdocsに明記。`setTabs`は「同tablist内の全タブを同期」方式へ全面書き換え（選択中不在でも復帰可・`closest()`化・`data-hasTabLink`死にコード削除）。ディープリンク`?lism-tab=`をReact版にも実装（useEffect方式）。`defaultIndex`は1始まり維持で範囲外は1へフォールバック。Root.astroの`btns &&`空配列バグ修正。**Tabの`as`は既定値`button`のまま受け付ける**（一度`as?: never`で型拒否したが、既定値だけで十分・変えて崩れるなら変えなければよい、という判断で撤回。ただしAstro版はRoot側transformが`<button>`前提のため実質変更不可である旨をコメントで明記）。Tab/Panelの`tabId`既定値は`'tab'`→`'__LISM_TAB_ID__'`へ変更し、transformの置換目印であることを明示（Accordionの`__LISM_ACC_ID__`と同じ規約）。テストは42件（setTabs/tabsKeyNav/transformTabitems/React Root）。docs ja/enへHTML例更新・「キーボード操作」「URLパラメータで初期タブを指定する」節を追加、`_tabs-keep-height.css`を`.b--tabs_panel[hidden]`ベースへ変更、MCP headings/snippet同期済み |
...
残りのProps→CSS移行はTabs（a11y改修はissue #87として対応済み。上記表を参照）。ModalはMCP `docs-index.json`の見出し同期のみ残る。
