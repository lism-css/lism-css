...
`patterns/` `_demo/` `preview/` `page-layout/` `og/` 等は対象外。
...
`article[data-pagefind-body]` が無いページは `ArticleNotFoundError` として skip されるが、それ以外の例外（rehype プラグインの TypeError、I/O 失敗、HTML パス不整合など）はそのまま rethrow されて build を失敗させる。これにより `llms.txt` が指す `.md` だけが密かに欠落する事故を防ぐ。
