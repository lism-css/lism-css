...
- [Layout 優先の原則](#layout-優先の原則-layout-isxxx-vs-trait-layout)
...
## Layout 優先の原則: `<Layout isXxx>` vs `<Trait layout="...">`

レイアウトとトレイトの両方の性質を持つ場合、以下の2つの書き方で同じ出力が得られます。
...
**`<Layout isXxx>` の形式で書いてください（Layout 優先）。** レイアウトコンポーネントを軸にして、トレイトを付加する書き方がコードの意図を明確にします。
