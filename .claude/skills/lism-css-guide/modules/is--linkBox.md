...
ボックス全体をクリック可能なリンク領域にする State モジュール。

## 基本情報

- クラス名: `is--linkBox`
- コンポーネント: `<LinkBox>`
- SCSSソース: https://raw.githubusercontent.com/lism-css/lism-css/main/packages/lism-css/src/scss/modules/state/_linkbox.scss
- ドキュメント（人間向け）: https://lism-css.com/docs/modules/is--linkbox/

## 専用Props

| Prop | 説明 |
|------|------|
| `href` | リンク先を指定。`href` があれば `<LinkBox>` 自体が `<a>` タグに、なければ `<div>` として出力される（**特殊挙動**） |

通常の `<Lism>` エイリアスコンポーネントと異なり、`<LinkBox>` だけは `href` 指定の有無で出力タグが動的に切り替わります。

## Usage

LinkBox には主に2通りの使い方があります。

### パターン1: `<LinkBox>` 自身を `<a>` にする

`href` を指定すると `<LinkBox>` 自体が `<a>` タグとして出力されます。シンプルなカード全体リンクに最適。

```jsx
<LinkBox href="/article/1" p="30" bgc="base" bd bdrs="30" hov="o">
...
```html
<a class="is--linkBox -hov:o -p:30 -bgc:base -bd -bdrs:30" href="/article/1">
