...
直下のコンテンツ幅を一括制御するモジュールクラス。`max-width` とセンタリングを担い、記事・セクションのコンテンツ幅の統一に使います。

## 基本情報

- クラス名: `is--wrapper`
- コンポーネント: `<Wrapper>`
- SCSSソース: https://raw.githubusercontent.com/lism-css/lism-css/main/packages/lism-css/src/scss/modules/state/_wrapper.scss
- ドキュメント（人間向け）: https://lism-css.com/docs/modules/is--wrapper/

## 使い方

`<Wrapper>` は `<Lism isWrapper>` のエイリアスです。`isWrapper` Prop は**どのコンポーネントからでも呼び出せる**ので、既存のレイアウトモジュールに後付けで幅制限を適用できます。
