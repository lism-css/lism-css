...
- **行間（half-leading）**: 全要素に `line-height: calc(1em + var(--hl) * 2)` を適用。`--hl` 変数で half-leading を調整できる設計
- **`body`**: `font-size: var(--fz--base)`, `font-family: var(--ff--base)`, `letter-spacing: var(--lts--base)`, `color: var(--text)`, `background-color: var(--base)`
- **見出し（`h1`〜`h6`）**: フォントサイズとウェイトが定義済み
- **リンク**: `color: var(--link-c, var(--link))`, `text-decoration` の管理
- **`code`**: `font-family: var(--ff--mono)`

### テーブル

テーブルセルは CSS 変数でカスタマイズ可能です。
...
- `disabled` 要素: `opacity: var(--o--n20)`（0.5）
- `:focus-visible`: outline-offset の管理
- リスト要素: クラスなし or Prop Class のみの場合、ブラウザデフォルトに revert
