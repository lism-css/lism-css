...
> モジュールの詳細な使い方は各セクション末尾のリンク先を参照してください。
...
| `u--` | Utility | 用途が明確なユーティリティ（`@layer lism-utility`） |
...
`@layer lism-utility` に属する、用途が明確なユーティリティクラスです。

| クラス | 用途 |
|--------|------|
| `u--trim` | half-leading のネガティブマージンで上下のテキスト余白を詰める |
| `u--trimChildren` | 子要素すべてに half-leading トリムを適用 |
| `u--cbox` | キーカラー（`--keycolor`）を使った色付きボックス（背景・ボーダー・テキスト色を自動生成） |
| `u--collapseGrid` | 境界線が重なり合うグリッド（`gap: var(--bdw)` + `box-shadow` で表現） |
| `u--expandedLink` | `::before` 疑似要素で親コンテナ全体をクリック可能にするリンク |
| `u--srOnly` | スクリーンリーダー専用（視覚的に非表示） |
| `u--snap` | スクロールスナップコンテナ（`--snapType`, `--snapAlign`, `--snapStop` でカスタマイズ） |
| `u--clipText` | テキストを背景グラデーションでクリップ（`background-clip: text`） |
