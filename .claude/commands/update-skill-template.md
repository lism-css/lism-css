...
| `set-class.md` | `set--`クラス (`set--plain`,`set--revert`,`set--var:bxsh`,`set--var:hov`,`set--var:bdrsInner`) の一覧と用途解説 | `packages/lism-css/src/scss/base/set/`, `packages/lism-css/src/scss/base/tokens/_shadow.scss` |
...
- `## 基本情報`（クラス名 / コンポーネント / SCSSソース / ドキュメント）。URL は以下の形式を維持し、誤って書き換えないこと：
    ...
    - ドキュメント（人間向け）: Layout/Atomic は `https://lism-css.com/docs/primitives/{lowercase}.md`、Trait は `https://lism-css.com/docs/trait-class/{lowercase}.md` — `{lowercase}` はクラス名の小文字版（例: `l--withSide.md` → `l--withside`）
- `## 専用Props`（該当プリミティブのみ）
...
| 確認したい内容 | 参照先 |
|---|---|
| set-- クラス一覧 | `packages/lism-css/src/scss/base/set/` 配下の SCSS、`packages/lism-css/src/scss/base/tokens/_shadow.scss`（set--var:bxsh） |
