...
| `lism-cli` | `@lism-css/cli` + `create-lism` | `packages/lism-cli/` + `packages/create-lism/` | `lism-cli@v` | `nr publish:cli` |

### lism-cli の特別ルール

`lism-cli` は `@lism-css/cli` と `create-lism` を **同時に同じバージョンで** publish する（`nr publish:cli` がまとめて処理する）。
...
- `create-lism` 側は `@lism-css/cli` を bundle で内包しているため、CLI 本体に依存変更があった場合も `create-lism` の `dependencies` 追従は不要
