...
6. **ダミーコンポーネントの不使用**: `/ui/` 以外の MDX の `<PreviewCode>` 内コードブロックで `<DummyText>` が使われていないこと。`<PreviewArea>` 内は許容。コード例では実際のテキスト・HTML要素を直接記述する。テキスト内容は `packages/lism-ui/src/components/DummyText/texts.ts` を参照
