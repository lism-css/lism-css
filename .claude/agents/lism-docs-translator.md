...
親エージェントから指定されたファイル群を英語に翻訳します。
各ファイルは「新規作成」（`en/` に未作成）または「更新」（`en/` に既存）のいずれかとして指示されます。

### 親エージェントからの個別ルール

親エージェントから個別の追加ルール・注意事項が渡された場合は、そちらを優先してください。
例えば `.astro` テンプレートの翻訳では、MDX 固有のルール（frontmatter 翻訳・内部リンクの `/en/` 化・import パス調整・MDX 用セルフチェックリスト等）は適用せず、渡された個別ルールに従います。その場合も「翻訳の品質基準」「用語対応表」「避けるべき直訳パターン」の共通基準は適用します。
...
- **内部リンク**: `/docs/...` は `/en/docs/...` に変更する（英語ページには `/en/` プレフィックスが付く）
...
- **外部 URL**: そのまま維持（内部リンクは上記の通り `/en/` プレフィックスを付ける）
