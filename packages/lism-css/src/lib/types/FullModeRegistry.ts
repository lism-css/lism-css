 ...
 * 型側へ反映するための拡張ポイント。
 *
 * full モードでは isVar 系（`bds` / `bdc` を除く）と border ショートハンド系を除く全 props が responsive に
 * なる（`props-full.ts` のランタイム挙動と一致）が、
 ...
 * `ta` 等の defaults では BP 非対応の props でも BP 指定（配列・オブジェクト記法）が型エラーにならなくなる。
 *
