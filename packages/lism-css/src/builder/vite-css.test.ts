...
import { lismCssVite } from './vite-css';

// resolveId フックを取り出す（Function 形式 / Object 形式の両対応）。
function getResolveId(plugin: ReturnType<typeof lismCssVite>) {
...
describe('lismCssVite resolveId', () => {
  test('lism-css/<entry>.css を自身の dist/css パスへ解決する', async () => {
    const resolveId = getResolveId(lismCssVite());
