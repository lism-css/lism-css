...
const generated: GeneratedCss = {
  entries: ['main', 'primitives/layout'],
  aliasMap: {
    'lism-css/main.css': '/proj/.lism-css/css/main.css',
    'lism-css/primitives/layout.css': '/proj/.lism-css/css/primitives/layout.css',
  },
...
describe('buildWebpackAlias', () => {
  test('CSS エントリは生成 CSS の絶対パスへ、config は完全一致($)で差し替える', () => {
    ...
    expect(alias['lism-css/primitives/layout.css']).toBe('/proj/.lism-css/css/primitives/layout.css');
...
describe('buildTurbopackAlias', () => {
  test('絶対パスを projectRoot 起点の ./ 相対パスへ変換する（Turbopack は絶対パスで解決失敗するため）', () => {
    ...
    expect(alias['lism-css/primitives/layout.css']).toBe('./.lism-css/css/primitives/layout.css');
