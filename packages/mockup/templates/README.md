...
- The default view is the **gallery**: every page is drawn in an `<iframe>` card
...
- The sidebar starts with **All pages** (back to the gallery) and **Design tokens**.
...
- The viewer cannot know which UI parts a mockup settled on, so `pages/components.jsx`
  covers that as an ordinary page you maintain yourself: it lists the buttons, form
  controls, badges and cards this mockup uses. It is a convention, not part of the
  contract — rename or delete it if it is not wanted, and add newly shared parts to
  it when it is kept.
...
│   ├── landing.jsx
│   ├── components.jsx      # hand maintained index of the UI parts in use
│   └── admin/
│       ├── dashboard.jsx
│       ├── settings.jsx
