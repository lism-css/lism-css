...
├── scripts/
│   └── generate-ai-assets.mjs # スキル MD → src/data/guides/ コピー
├── src/
│   ├── index.ts                # サーバー初期化・起動
│   ├── lib/
...
│   │   └── guides/             # ← generate:ai-assets で生成（.gitignore 対象）
│   │       ├── SKILL.md
│   │       ├── tokens.md
│   │       ├── property-class.md
│   │       ├── components-core.md
...
② .claude/skills/lism-css-guide/*.md（正本）
        │
        │  pnpm generate:ai-assets（コピー）
        ▼
③ packages/mcp/src/data/guides/*.md（生成物）
        │
        │  pnpm build（tsc + cp）
        ▼
④ packages/mcp/dist/data/guides/*.md（配布物）← サーバーが参照
```
