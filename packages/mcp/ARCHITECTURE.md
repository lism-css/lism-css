...
│   │   ├── schemas.ts          # Zod スキーマ定義（DocsEntrySchema, MetaInfoSchema）
│   │   ├── search.ts           # 検索ロジック（トークナイズ・クエリ展開・スコアリング・nextTool 付与）
│   │   ├── response.ts         # レスポンスフォーマット（Markdown / JSON / error / structuredContent）
...
│   │   ├── get-guide.ts        # トピック一覧（後述）から指定した Markdown ガイドを返却
...
│       ├── tools.test.ts
│       ├── convert-css.test.ts
│       └── docs-index.test.ts  # docs-index.json と実 MDX の整合性検証（モノレポ開発時のみ実行）
...
- 1 つの MDX ファイルに対して複数エントリ（セクション単位）を持てます。`sourcePath` の重複は、長いページを検索単位で分割するための意図的な設計です
- `sourcePath` の実在・収録漏れ・URL スラッグ変換の整合性は `src/tests/docs-index.test.ts` で機械的に検証されます
...
| `set-class` | set-class.md | Set クラス（set--plain, set--bxsh, set--hov 等） |
| `primitive-class` | primitive-class.md | Primitive クラスプレフィックス（l--, a--） |
| `trait-class` | trait-class.md | Trait クラス（is--, has--）: 役割宣言・機能付与クラス |
| `utility-class` | utility-class.md | ユーティリティクラス（u--trim, u--cbox 等） |
| `css-rules` | css-rules.md | CSS 設計方法論・Layer 構造・Block Class（b--）/ Custom Class（c--）・命名規則 |
| `naming` | naming.md | CSS 変数・クラスの命名規則、{prop} / {value} の省略ルール |
...
bin/lism-mcp.mjs
  └─► dist/index.js (main())
        ...
        │                          （name: 'lism-css', version は package.json から取得、
        │                            instructions で推奨ワークフローをクライアントへ宣言）
...
- search_docs と convert_css は `outputSchema` を宣言し、`structuredContent` で構造化データを返します（後方互換のため同じ JSON 文字列を `content` にも含める、MCP 仕様推奨のパターン）
- `notFound()` は「候補を提示して次の行動を促す正常応答」であり、`isError` を付けず `message` + `suggestions` 等のキーで返します（`isError: true` は `error()` のみ）
- search_docs の各結果には `nextTool`（そのページの詳細を取得するための推奨フォローアップツール呼び出し。例: `get_component(name: "l--flex")`）が付与されます
...
server.registerTool(
    ...
    ({ topic }) => markdownResponse(GUIDE_TOPICS[topic].files.map(loadMarkdown).join('\n\n'))
);
```
...
起動時に `preloadGuides()` がサブディレクトリ配下も含めて再帰的に全 `.md` ファイルをメモリキャッシュに載せ、ツール実行時のファイル I/O を排除します。キャッシュキーは `guidesDir` からの posix 相対パス（例: `SKILL.md`, `primitives/l--flex.md`）。
