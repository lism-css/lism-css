...
| `get_component` | コンポーネントの詳細を取得 | Markdown | `name`: コンポーネント名, `package?`: `"lism-css"` \| `"@lism-css/ui"` |
...
│   │   ├── get-component.ts    # components-ui.md / components-core.md から名前検索
...
起動時に `preloadGuides()` で全ファイルをメモリキャッシュに載せ、ツール実行時のファイル I/O を排除します。
