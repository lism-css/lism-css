# Lism CSS design mock — data contract

This directory is a **data directory** for `@lism-css/mock`. You only write data
...
```bash
npx @lism-css/mock check .   # validate this directory (agents: use this)
npx @lism-css/mock dev .     # start the preview server (humans, in a browser)
```
...
```
.
├── mock.config.json      # required — schema version + page metadata
...
- Adding a file is enough — pages are discovered from disk, not from
  `mock.config.json`. Two files with the same id (`foo.jsx` + `foo.tsx`) is an error.
...
Not allowed: API calls, authentication, persistence, business logic. A mock is a
...
`check` verifies the schema of `mock.config.json` and `tokens.json`, the import
...
Pages are plain JSX, so they run as ordinary code. Only run mocks you trust.
