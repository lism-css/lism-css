...
export interface MockRuntime {
  ...
  /** `serializeTokens()` の結果（`virtual:lism-mock/tokens.css` の中身）。 */
  tokensCss: string;
  /** 仮想モジュールが生成する動的 import 指定子の集合（境界チェックの照合用）。 */
  getPageSpecifiers(): ReadonlySet<string>;
  /** `mock.config.json` と `pages/` を読み直す。 */
  refreshPages(): void;
...
/** データディレクトリを検証して `MockData` を作る。 */
export async function loadMockData(dir: string): Promise<MockData> {
...
export async function prepareMockRuntime(dir: string): Promise<MockRuntime> {
  ...
  const tempDir = safeRealpath(fs.mkdtempSync(path.join(os.tmpdir(), 'lism-mock-')));
  ...
  let specifierSource: MockData['pages'] | null = null;
