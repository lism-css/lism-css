...
import { MockContractError, type MockTokens } from './types.js';
 ...
 */
export function validateTokens(raw: unknown, defaultTokens: Record<string, unknown>, file: string): MockTokens {
  if (!isPlainObject(raw)) {
    throw new MockContractError(`${TOKENS_FILENAME} must contain a JSON object of token groups (e.g. { "color": { "brand": "#333" } }).`, {
      file,
    });
  }
  ...
  for (const [group, values] of Object.entries(raw)) {
    ...
    if (!isPlainObject(values)) {
      throw new MockContractError(`${TOKENS_FILENAME}: "${group}" must be an object of token key / value pairs.`, { file });
    }
    ...
    for (const [key, value] of Object.entries(values)) {
      ...
      if (typeof value !== 'string' && typeof value !== 'number') {
        throw new MockContractError(`${TOKENS_FILENAME}: "${group}.${key}" must be a string or a number.`, { file });
      }
      entry[key] = value;
    }
    tokens[group] = entry;
...
export function readTokensFile(dataDir: string): { raw: unknown; file: string } | null {
  ...
  try {
    ...
    throw new MockContractError(`${TOKENS_FILENAME} is not valid JSON: ${(error as Error).message}`, { file });
  }
}

/** `tokens.json` を読み、default-config を正として検証した結果を返す。 */
export async function loadTokens(dataDir: string): Promise<MockTokens> {
...
export function writeConfigModule(dir: string, tokens: MockTokens): string {
  const file = path.join(dir, GENERATED_CONFIG_FILENAME);
  const body = `// Generated by @lism-css/mock from ${TOKENS_FILENAME}. Do not edit.\nexport default ${JSON.stringify({ tokens }, null, 2)};\n`;
