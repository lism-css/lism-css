...
function fail(error: unknown): void {
  if (error instanceof MockContractError) {
    console.error(pc.red(`[lism-mock] ${error.message}`));
    if (error.file) console.error(pc.dim(`  at ${error.file}`));
  } else {
    console.error(pc.red(`[lism-mock] ${error instanceof Error ? error.message : String(error)}`));
  }
  process.exitCode = 1;
}
...
program.name('lism-mock').description('Design mock preview CLI for Lism CSS').version(pkg.version);

program
  .command('init')
  .argument('[dir]', 'data directory (default: current directory)')
  .option('--force', 'overwrite existing files')
  .description('scaffold a mock data directory (sample pages + contract guide)')
