/**
 * `lism-mock dev` … ブラウザ確認用の dev サーバー。
 *
...
import { MOCK_CONFIG_FILENAME } from '../core/data-dir.js';
import { PAGE_EXTENSIONS, PAGES_DIRNAME } from '../core/pages.js';
import { getViewerDir, isInsideDir } from '../core/paths.js';
import { prepareMockRuntime, type MockRuntime } from '../core/runtime.js';
...
export function classifyDataEvent(dataDir: string, event: WatchEvent, file: string): ReloadKind | null {
  ...
  if (target === path.join(dataDir, MOCK_CONFIG_FILENAME)) return 'pages';
...
function reportWatchError(server: ViteDevServer, error: unknown): void {
  ...
  const file = error instanceof MockContractError ? error.file : undefined;
  const detail = file ? `${message}\n  at ${file}` : message;

  // 検証エラーでサーバーは落とさない。ターミナルとブラウザのオーバーレイの両方へ出し、
  // ユーザーがファイルを直せばそのまま復帰できるようにする。
  server.config.logger.error(pc.red(`[lism-mock] ${detail}`));
  server.ws.send({ type: 'error', err: { message: `[lism-mock] ${detail}`, stack: '' } });
}

/** 変更種別に応じて runtime を作り直し、仮想モジュールを invalidate してフルリロードする。 */
export async function applyDataChange(server: ViteDevServer, runtime: MockRuntime, kind: ReloadKind): Promise<void> {
  try {
    if (kind === 'tokens') {
...
/** データディレクトリの監視を仕掛ける。 */
export function watchDataDir(server: ViteDevServer, runtime: MockRuntime): void {
  const { dataDir } = runtime.data;
  server.watcher.add([path.join(dataDir, PAGES_DIRNAME), path.join(dataDir, MOCK_CONFIG_FILENAME), path.join(dataDir, TOKENS_FILENAME)]);
...
export async function createMockDevServer(dir: string, options: DevCommandOptions = {}): Promise<{ server: ViteDevServer; runtime: MockRuntime }> {
...
export async function devCommand(dir: string, options: DevCommandOptions = {}): Promise<void> {
  ...
  await server.listen();
  console.log(pc.green(`[lism-mock] preview server for ${runtime.data.dataDir}`));
