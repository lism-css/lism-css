...
beforeAll(async () => {
  ...
  writeFiles(projectDir, {
    // データディレクトリの親に同名パッケージを置いても、@lism-css/mock 側へ解決されることを確かめる。
    ...
    'data/mock.config.json': JSON.stringify({
      schemaVersion: 1,
      title: 'Integration',
...
describe('dev サーバー', () => {
  ...
  test('virtual:lism-mock/pages が列挙結果を供給する', async () => {
  ...
  test('virtual:lism-mock/tokens.css に tokens.json の内容が反映される', async () => {
  ...
  test('ページからの react は @lism-css/mock 側へ解決される（親の同名パッケージを見ない）', async () => {
  ...
  test('watch 対象の分類', () => {
    ...
    expect(classifyDataEvent(dataDir, 'change', path.join(dataDir, 'mock.config.json'))).toBe('pages');
