/**
 * `lism-mock check` … 非対話の検証（エージェントの自己確認用）。
 *
 * 保証するのは3点まで。
...
import { MockContractError, type MockData } from '../core/types.js';
...
function toCheckError(error: unknown): MockContractError {
  if (error instanceof MockContractError) return error;
