 ...
 *
 * 1. 信頼済みコード（`virtual:lism-mock/pages`）からのページ import … 列挙結果に含まれることを検証して許可
...
export type ImportDecision =
  ...
  /** 許可された bare import（`@lism-css/mock` 起点で解決する）。 */
...
function violation(dataDir: string, importerFile: string, source: string, reason: string): MockContractError {
  const where = isInsideDir(dataDir, importerFile) ? path.relative(dataDir, importerFile) : importerFile;
  return new MockContractError(`Forbidden import ${JSON.stringify(source)} in ${where}: ${reason}`, { file: importerFile });
}
...
/** import 1件を分類する。契約違反は `MockContractError` を投げる。 */
export function classifyImport(source: string, importer: string | undefined, ctx: ImportBoundaryContext): ImportDecision {
  ...
  if (isPageImporterId(importerFile)) {
    ...
    throw new MockContractError(`Unexpected import ${JSON.stringify(source)} from the generated page list.`);
  }
...
export interface ImportBoundaryOptions extends ImportBoundaryContext {
  /** 許可 bare import を解決するときの importer（既定は `@lism-css/mock` パッケージルート）。 */
...
export function importBoundaryPlugin(options: ImportBoundaryOptions): Plugin {
  ...
  return {
    name: 'lism-mock:import-boundary',
    enforce: 'pre',
    ...
    async resolveId(source, importer) {
      ...
      // 許可 bare import は `@lism-css/mock` 自身の位置から解決する。
      ...
      if (!resolved) {
        throw new MockContractError(
          `Cannot resolve ${JSON.stringify(decision.specifier)} from @lism-css/mock. Reinstall @lism-css/mock and try again.`,
          { file: importer }
        );
      }
      return resolved;
