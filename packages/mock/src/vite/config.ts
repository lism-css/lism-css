...
export type MockViteMode = 'dev' | 'build';

export interface MockViteConfigOptions {
  runtime: MockRuntime;
...
/** `from` から親方向へ辿って見つかる node_modules ディレクトリ（`@lism-css/mock` の依存ツリー）。 */
 ...
 *
 * ビューア・データディレクトリ・生成物の一時ディレクトリと、`@lism-css/mock` が所有する
...
export function createMockViteConfig({ runtime, viewerDir, mode }: MockViteConfigOptions): InlineConfig {
